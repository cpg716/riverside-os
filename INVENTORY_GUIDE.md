...
When staff record **line returns** (`POST /api/transactions/{id}/returns`), **takeaway** lines that are already **fulfilled** (picked up) default to **restocking**: `product_variants.stock_on_hand` increases by the returned quantity. **Special**, **Custom**, and **Wedding** order lines do not get an automatic floor restock (merchandise was never decremented at checkout the same way as takeaway). Optional per-line **`restock`** on the request overrides the default.
