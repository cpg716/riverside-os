//! Checkout: persist cart → `orders` + `order_items` in one transaction.
...
use crate::logic::order_returns::{self, ReturnLineInput};
...
impl IntoResponse for OrderError {
    fn into_response(self) -> Response {
        let (status, msg) = match self {
            OrderError::InvalidPayload(m) => (StatusCode::BAD_REQUEST, m),
            OrderError::NotFound => (StatusCode::NOT_FOUND, "Order not found".to_string()),
            OrderError::Unauthorized(m) => (StatusCode::UNAUTHORIZED, m),
            OrderError::Forbidden(m) => (StatusCode::FORBIDDEN, m),
            OrderError::BadGateway(m) => (StatusCode::BAD_GATEWAY, m),
            OrderError::Database(e) => {
                ...
                tracing::error!(error = %e, "Database error in orders");
                (
...
impl From<crate::logic::order_checkout::CheckoutError> for OrderError {
    fn from(e: crate::logic::order_checkout::CheckoutError) -> Self {
        match e {
            crate::logic::order_checkout::CheckoutError::InvalidPayload(m) => {
                OrderError::InvalidPayload(m)
            }
            crate::logic::order_checkout::CheckoutError::Database(d) => OrderError::Database(d),
        }
    }
}

impl From<suit_component_swap::SuitSwapError> for OrderError {
    fn from(e: suit_component_swap::SuitSwapError) -> Self {
        match e {
            suit_component_swap::SuitSwapError::NotFound => OrderError::NotFound,
            suit_component_swap::SuitSwapError::InvalidPayload(m) => OrderError::InvalidPayload(m),
            suit_component_swap::SuitSwapError::Inventory(i) => {
                OrderError::InvalidPayload(i.to_string())
            }
            suit_component_swap::SuitSwapError::Database(d) => OrderError::Database(d),
        }
    }
}

fn spawn_meilisearch_order_upsert(state: &AppState, order_id: Uuid) {
    let ms = state.meilisearch.clone();
    let pool = state.db.clone();
    if let Some(c) = ms {
        tokio::spawn(async move {
            crate::logic::meilisearch_sync::upsert_order_document(&c, &pool, order_id).await;
        });
    }
}
...
pub struct OrderDetailItem {
    ...
    pub quantity: i32,
    /// Units recorded on `order_return_lines` for this line.
...
impl OrderDetailResponse {
    /// Build customer-facing receipt data. When `order_item_ids` is `Some`, only those lines are
    /// included (must match at least one line or returns `InvalidPayload`).
    pub(crate) fn receipt_for_zpl_filtered(
        &self,
        order_item_ids: Option<&[Uuid]>,
    ) -> Result<receipt_zpl::ReceiptOrderForZpl, OrderError> {
            ...
            Some(ids) => {
                if ids.is_empty() {
                    return Err(OrderError::InvalidPayload(
                        "order_item_ids must list at least one line id.".to_string(),
                    ));
                }
                ...
                if v.is_empty() {
                    return Err(OrderError::InvalidPayload(
                        "No order lines matched order_item_ids for this order.".to_string(),
                    ));
                }
                v
            }
        ...
        Ok(receipt_zpl::ReceiptOrderForZpl {
            ...
            customer: self
                .customer
                .as_ref()
                .map(|c| receipt_zpl::ReceiptCustomerLine {
            ...
            items: selected
                ...
                .map(|it| receipt_zpl::ReceiptLineForZpl {
                    ...
                    salesperson_name:
                        crate::logic::receipt_privacy::staff_name_for_customer_receipt(
...
fn receipt_query_order_item_ids(
    params: &std::collections::HashMap<String, String>,
) -> Option<Vec<Uuid>> {
    let raw = params.get("order_item_ids")?;
...
#[derive(Debug, Deserialize)]
pub struct PatchOrderItemRequest {
...
pub fn router() -> Router<AppState> {
    Router::new()
        ...
        .route("/{order_id}/pickup", post(mark_order_pickup))
        .route("/{order_id}/review-invite", post(post_order_review_invite))
        .route("/{order_id}/audit", get(get_order_audit))
        .route("/{order_id}/refunds/process", post(process_refund))
        .route("/{order_id}/returns", post(post_order_returns))
        .route("/{order_id}/exchange-link", post(post_order_exchange_link))
        .route("/{order_id}/items", post(add_order_item))
        .route(
            "/{order_id}/items/{order_item_id}",
            patch(update_order_item).delete(delete_order_item),
        )
        .route(
        ...
        .route(
            "/{order_id}/receipt/send-email",
            post(post_order_receipt_send_email),
        )
        .route(
...
fn map_perm_err(e: (StatusCode, axum::Json<serde_json::Value>)) -> OrderError {
    ...
    let msg = v
        ...
        .to_string();
    match status {
        StatusCode::UNAUTHORIZED => OrderError::Unauthorized(msg),
        StatusCode::FORBIDDEN => OrderError::Forbidden(msg),
        _ => OrderError::Forbidden(msg),
    }
}

async fn register_session_is_open(pool: &sqlx::PgPool, sid: Uuid) -> Result<bool, OrderError> {
...
async fn authorize_order_read_bo_or_register(
    state: &AppState,
    headers: &HeaderMap,
    order_id: Uuid,
    ...
    if let Some(sid) = register_session_id {
        ...
        let ok = order_has_positive_payment_in_session(&state.db, order_id, sid)
            .await
...
async fn authorize_order_modify_bo_or_register(
    state: &AppState,
    headers: &HeaderMap,
    order_id: Uuid,
    register_session_id: Option<Uuid>,
) -> Result<Option<Uuid>, OrderError> {
    if let Some(sid) = register_session_id {
        if !register_session_is_open(&state.db, sid).await? {
            return Err(OrderError::Forbidden(
                "register session is not open".to_string(),
...
async fn get_order_detail(
    ...
    Query(q): Query<OrderReadQuery>,
    headers: HeaderMap,
) -> Result<Json<OrderDetailResponse>, OrderError> {
    authorize_order_read_bo_or_register(&state, &headers, order_id, q.register_session_id).await?;
...
async fn post_order_review_invite(
    State(state): State<AppState>,
    Path(order_id): Path<Uuid>,
    Query(q): Query<OrderReadQuery>,
    headers: HeaderMap,
    ...
    authorize_order_read_bo_or_register(&state, &headers, order_id, q.register_session_id).await?;
    podium_reviews::apply_post_sale_review_choice(&state.db, order_id, body.skip)
        .await
        .map_err(|e| match e {
            podium_reviews::ReviewInviteError::Db(d) => OrderError::Database(d),
            podium_reviews::ReviewInviteError::NotFound => OrderError::NotFound,
        })?;
...
async fn list_orders(
    ...
    headers: HeaderMap,
) -> Result<Json<PagedOrdersResponse>, OrderError> {
    ...
    let page =
        crate::logic::order_list::query_paged_orders(&state.db, &q, state.meilisearch.as_ref())
            .await
            .map_err(OrderError::Database)?;
    Ok(Json(page))
}
...
async fn patch_order(
    ...
    if let Some(status) = body.status {
        if status == DbOrderStatus::Cancelled {
            ...
            if refundable_pre > Decimal::ZERO {
                ...
                {
                    return Err(OrderError::Forbidden(
                        "orders.cancel or orders.void_sale required to cancel an order with no payments"
                            .to_string(),
                    ));
                }
            }
        ...
        if status == DbOrderStatus::Cancelled {
            loyalty_logic::reverse_order_accrual_in_tx(&mut tx, order_id)
                .await
                .map_err(OrderError::Database)?;
        }

        let upd = sqlx::query("UPDATE orders SET status = $1 WHERE id = $2")
            .bind(status)
            .bind(order_id)
            .execute(&mut *tx)
            .await?;
    ...
    let detail = load_order_detail(&state.db, order_id).await?;
    spawn_meilisearch_order_upsert(&state, order_id);
    Ok(Json(detail))
}

async fn mark_order_pickup(
    State(state): State<AppState>,
    Path(order_id): Path<Uuid>,
    headers: HeaderMap,
    Json(body): Json<PickupOrderRequest>,
) -> Result<Json<serde_json::Value>, OrderError> {
    let _ =
        authorize_order_modify_bo_or_register(&state, &headers, order_id, body.register_session_id)
            .await?;

    let mut tx = state.db.begin().await?;
    if body.delivered_item_ids.is_empty() {
        sqlx::query("UPDATE order_items SET is_fulfilled = TRUE WHERE order_id = $1")
            .bind(order_id)
            .execute(&mut *tx)
            .await?;
    } else {
        sqlx::query(
            "UPDATE order_items SET is_fulfilled = TRUE WHERE order_id = $1 AND id = ANY($2)",
        )
        .bind(order_id)
    ...
    if remaining_unfulfilled == 0 {
        sqlx::query("UPDATE orders SET status = 'fulfilled'::order_status, fulfilled_at = CURRENT_TIMESTAMP WHERE id = $1")
            .bind(order_id)
            .execute(&mut *tx)
            .await?;
    }

    // For Special/Custom orders: the item physically arrives from the vendor and goes
    ...
    let status_after: DbOrderStatus = sqlx::query_scalar("SELECT status FROM orders WHERE id = $1")
        .bind(order_id)
        .fetch_one(&mut *tx)
        .await?;
    ...
    if let Err(e) = loyalty_logic::try_accrue_for_order(&state.db, order_id).await {
        tracing::error!(error = %e, order_id = %order_id, "loyalty accrual failed after pickup");
    }
...
async fn get_order_audit(
    ...
    Query(q): Query<OrderReadQuery>,
    headers: HeaderMap,
) -> Result<Json<Vec<OrderAuditEvent>>, OrderError> {
    authorize_order_read_bo_or_register(&state, &headers, order_id, q.register_session_id).await?;
    let rows = sqlx::query_as::<_, OrderAuditEvent>(
        r#"
...
async fn list_refunds_due(
    State(state): State<AppState>,
    headers: HeaderMap,
) -> Result<Json<Vec<RefundQueueRow>>, OrderError> {
    middleware::require_staff_with_permission(&state, &headers, ORDERS_REFUND_PROCESS)
        .await
        .map_err(map_perm_err)?;
    let rows = sqlx::query_as::<_, RefundQueueRow>(
        r#"
        SELECT id, order_id, customer_id, amount_due, amount_refunded, is_open, reason, created_at
...
async fn process_refund(
    ...
    let row: Option<RefundQueueRow> = sqlx::query_as(
        ...
        SELECT id, order_id, customer_id, amount_due, amount_refunded, is_open, reason, created_at
        FROM order_refund_queue
        WHERE order_id = $1 AND is_open = TRUE
        ORDER BY created_at DESC
        LIMIT 1
        FOR UPDATE
        "#,
    ...
    let current_paid: Decimal =
        sqlx::query_scalar("SELECT amount_paid FROM orders WHERE id = $1 FOR UPDATE")
            .bind(order_id)
            .fetch_one(&mut *tx)
            .await?;
    ...
    if wants_card_refund {
        if let Some(ref iid) = stripe_intent {
            ...
            let rf = stripe::Refund::create(&state.stripe_client, cp)
                .await
                .map_err(|e| OrderError::InvalidPayload(format!("Stripe refund failed: {e}")))?;
    ...
    if method_l.contains("gift") {
        ...
        .await
        .map_err(|e| match e {
            gift_card_ops::GiftCardOpError::Db(d) => OrderError::Database(d),
            gift_card_ops::GiftCardOpError::BadRequest(m) => OrderError::InvalidPayload(m),
        })?;
    }
    ...
    sqlx::query(
        r#"
        INSERT INTO payment_allocations (transaction_id, target_order_id, amount_allocated, metadata)
        VALUES ($1, $2, $3, $4)
        "#,
    )
    .bind(payment_tx_id)
    ...
    let new_paid: Decimal = sqlx::query_scalar("SELECT amount_paid FROM orders WHERE id = $1")
        .bind(order_id)
        .fetch_one(&mut *tx)
        .await?;
    if new_paid.is_zero() {
...
async fn get_order_receipt_zpl(
    State(state): State<AppState>,
    Path(order_id): Path<Uuid>,
    ...
    authorize_order_read_bo_or_register(&state, &headers, order_id, register_session_id).await?;
    let detail = load_order_detail(&state.db, order_id).await?;

    let item_ids = receipt_query_order_item_ids(&params);
    ...
    Response::builder()
        ...
        .map_err(|e| {
            ...
            OrderError::InvalidPayload("failed to build response".to_string())
        })
}

async fn get_order_receipt_html(
    State(state): State<AppState>,
    Path(order_id): Path<Uuid>,
    Query(params): Query<std::collections::HashMap<String, String>>,
    headers: HeaderMap,
...
struct SendOrderReceiptEmailBody {
    ...
    #[serde(default)]
    gift: bool,
    /// Subset of `order_items.id` to include; when empty, all lines are used.
    #[serde(default)]
...
fn map_podium_order_err(e: crate::logic::podium::PodiumError) -> OrderError {
    OrderError::BadGateway(format!(
...
async fn post_order_receipt_send_email(
    State(state): State<AppState>,
    Path(order_id): Path<Uuid>,
    Query(q): Query<OrderReadQuery>,
    headers: HeaderMap,
    ...
    let item_ids = if body.order_item_ids.is_empty() {
        None
    } else {
        Some(body.order_item_ids.as_slice())
    };
...
async fn post_order_receipt_send_sms(
    State(state): State<AppState>,
    Path(order_id): Path<Uuid>,
    Query(q): Query<OrderReadQuery>,
    headers: HeaderMap,
    ...
    if let Some(b64_raw) = body.png_base64.as_ref() {
        ...
        if !b64.is_empty() {
            let png = general_purpose::STANDARD.decode(b64).map_err(|_| {
                OrderError::InvalidPayload("Invalid base64 for receipt image.".to_string())
            })?;
            if png.is_empty() {
                return Err(OrderError::InvalidPayload(
...
pub(crate) async fn load_order_detail(
    ...
    let items = sqlx::query_as::<_, OrderItemRow>(
        ...
        SELECT
            ...
            COALESCE((
                ...
                FROM order_return_lines orx
                WHERE orx.order_item_id = oi.id
            ...
            CASE
                ...
                THEN (NULLIF(TRIM(oi.size_specs->>'original_unit_price'), ''))::numeric(14,2)
                ELSE NULL
    ...
    let payment_methods_summary: String = sqlx::query_scalar(
        r#"
        SELECT COALESCE(
            STRING_AGG(DISTINCT pt.payment_method, ', ' ORDER BY pt.payment_method),
            '—'
        )
        FROM payment_allocations pa
        INNER JOIN payment_transactions pt ON pt.id = pa.transaction_id
...
async fn insert_order_activity_log_tx(
...
async fn log_order_activity(
    ...
    if is_customer_milestone {
        if let Some(cid) = customer_id {
            ...
            .bind(cid)
            .bind(format!("Order {order_id}: {summary}"))
            .execute(db)
            .await?;
        }
    }
...
async fn post_order_returns(
    ...
    let staff_id =
        authorize_order_modify_bo_or_register(&state, &headers, order_id, q.register_session_id)
            .await?;
    ...
    order_returns::apply_order_returns(&state.db, order_id, staff_id, inputs)
        .await
        .map_err(|e| match e {
            order_returns::OrderReturnError::Db(d) => OrderError::Database(d),
            order_returns::OrderReturnError::BadRequest(m) => OrderError::InvalidPayload(m),
        })?;
...
async fn post_order_exchange_link(
    State(state): State<AppState>,
    Path(order_id): Path<Uuid>,
    Query(q): Query<OrderReadQuery>,
    headers: HeaderMap,
    ...
    if body.other_order_id == order_id {
        return Err(OrderError::InvalidPayload(
            "other_order_id must differ from this order".to_string(),
        ));
    }

    if let Some(sid) = q.register_session_id {
        authorize_order_modify_bo_or_register(&state, &headers, order_id, Some(sid)).await?;
        authorize_order_modify_bo_or_register(&state, &headers, body.other_order_id, Some(sid))
            .await?;
    } else {
        middleware::require_staff_with_permission(&state, &headers, ORDERS_MODIFY)
            .await
    ...
    .bind(order_id)
    .bind(body.other_order_id)
    .fetch_one(&mut *tx)
    .await?;
    if n != 2 {
        return Err(OrderError::InvalidPayload(
            "one or both orders were not found or are cancelled".to_string(),
        ));
    }

    let gid = Uuid::new_v4();
    sqlx::query("UPDATE orders SET exchange_group_id = $1 WHERE id = $2 OR id = $3")
        .bind(gid)
        .bind(order_id)
        .bind(body.other_order_id)
        .execute(&mut *tx)
    ...
    log_order_activity(
        ...
        json!({
            ...
            "other_order_id": body.other_order_id,
        }),
    )
    .await?;
...
async fn add_order_item(
    ...
    Path(order_id): Path<Uuid>,
    headers: HeaderMap,
    Json(body): Json<AddOrderItemRequest>,
) -> Result<Json<OrderDetailResponse>, OrderError> {
    middleware::require_staff_with_permission(&state, &headers, ORDERS_MODIFY)
        .await
        .map_err(map_perm_err)?;
    ...
    let wedding_row: Option<Option<Uuid>> =
        sqlx::query_scalar("SELECT wedding_member_id FROM orders WHERE id = $1")
            .bind(order_id)
            .fetch_optional(&mut *tx)
            .await?;
    ...
    let fulfillment =
        crate::logic::order_fulfillment::persist_fulfillment(wedding_member_id, body.fulfillment)
            .map_err(|m| OrderError::InvalidPayload(m.to_string()))?;
    sqlx::query(
        r#"
...
async fn update_order_item(
    State(state): State<AppState>,
    Path((order_id, order_item_id)): Path<(Uuid, Uuid)>,
    headers: HeaderMap,
    Json(body): Json<PatchOrderItemRequest>,
) -> Result<Json<OrderDetailResponse>, OrderError> {
    middleware::require_staff_with_permission(&state, &headers, ORDERS_MODIFY)
        .await
        .map_err(map_perm_err)?;
    ...
    let normalized_fulfillment = match body.fulfillment {
        Some(f) => Some(
            crate::logic::order_fulfillment::persist_fulfillment(wedding_member_id, f)
                .map_err(|m| OrderError::InvalidPayload(m.to_string()))?,
        ),
        None => None,
    };

    let mut qb = sqlx::QueryBuilder::<sqlx::Postgres>::new("UPDATE order_items SET ");
    ...
    log_order_activity(
        ...
        "item_updated",
        "Order line edited",
        json!({ "order_item_id": order_item_id, "quantity": body.quantity, "unit_price": body.unit_price, "fulfillment": normalized_fulfillment }),
    )
    .await?;
    let detail = load_order_detail(&state.db, order_id).await?;
    Ok(Json(detail))
...
async fn post_suit_component_swap(
...
) -> Result<Json<SuitSwapOutcome>, OrderError> {
    let staff_id_for_event: Option<Uuid> = if let Some(reg_sid) = body.register_session_id {
        ...
        authorize_order_modify_bo_or_register(&state, &headers, order_id, Some(reg_sid)).await?;
        ...
        if !staff_has_permission(&eff, ORDERS_MODIFY) {
            return Err(OrderError::Forbidden(
                "missing permission orders.modify".to_string(),
            ));
        }
        if !staff_has_permission(&eff, ORDERS_SUIT_COMPONENT_SWAP) {
            return Err(OrderError::Forbidden(
                "missing permission orders.suit_component_swap".to_string(),
            ));
        }
        Some(staff.id)
    };
...
async fn checkout(
    ...
    middleware::require_pos_register_session_for_checkout(&state, &headers, payload.session_id)
        ...
        .map_err(|(status, axum::Json(v))| {
            ...
            if status == StatusCode::UNAUTHORIZED {
                OrderError::Unauthorized(msg)
            } else {
                OrderError::InvalidPayload(msg)
            }
        })?;

    use crate::logic::order_checkout::{execute_checkout, CheckoutDone};
    ...
    match outcome {
        CheckoutDone::Idempotent { order_id } => {
            spawn_meilisearch_order_upsert(&state, order_id);
        ...
        CheckoutDone::Completed {
            ...
            for detail in price_override_audit {
                let _ = log_staff_access(
                    ...
                    operator_staff_id,
                    "price_override",
                    json!({ "order_id": order_id, "detail": detail }),
                )
                .await;
            }
            ...
            let accrual_res = loyalty_logic::try_accrue_for_order(&state.db, order_id).await;
            ...
            if let Ok(url_raw) = std::env::var("RIVERSIDE_WEBHOOK_URL") {
                ...
                if !target_url.is_empty() {
                    ...
                    tokio::spawn(async move {
                        ...
                        {
                            tracing::warn!(error = %e, order_id = %order_id, "Webhook dispatch failed");
                        } else {
                            tracing::info!(order_id = %order_id, "Webhook successfully dispatched");
                        }
                    });
                }
            }
...
async fn staff_id_active(pool: &sqlx::PgPool, id: Uuid) -> Result<bool, OrderError> {
    let ok: bool =
...
async fn patch_order_attribution(
    State(state): State<AppState>,
    Path(order_id): Path<Uuid>,
    ...
    let admin = pins::authenticate_pos_staff(&state.db, code, body.manager_pin.as_deref())
        .await
        .map_err(|_| {
            OrderError::Unauthorized("valid manager cashier code and PIN required".to_string())
        })?;
    let eff =
    ...
    for line in &body.line_attribution {
        if let Some(sid) = line.salesperson_id {
            if !staff_id_active(&state.db, sid).await? {
                return Err(OrderError::InvalidPayload(format!(
    ...
    if status == "cancelled" {
        return Err(OrderError::InvalidPayload(
            "cannot edit attribution on cancelled orders".to_string(),
        ));
    }

    let mut primary_touched = false;
    if let Some(pid) = body.primary_salesperson_id {
        sqlx::query("UPDATE orders SET primary_salesperson_id = $1 WHERE id = $2")
            .bind(pid)
            .bind(order_id)
            .execute(&mut *tx)
            .await
    ...
    for line in &body.line_attribution {
        ...
        sqlx::query(
            ...
            INSERT INTO order_attribution_audit (
                order_id, order_item_id, prior_salesperson_id, new_salesperson_id,
