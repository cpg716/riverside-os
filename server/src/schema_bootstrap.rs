...
const STAFF_ERROR_EVENT_STATUS_BOOTSTRAP_SQL: &str =
    include_str!("../../migrations/170_staff_error_event_status_compat.sql");
...
pub async fn ensure_core_schema(pool: &PgPool) -> Result<(), sqlx::Error> {
    ...
    sqlx::raw_sql(STAFF_ERROR_EVENT_STATUS_BOOTSTRAP_SQL)
        .execute(pool)
        .await?;
    Ok(())
}
