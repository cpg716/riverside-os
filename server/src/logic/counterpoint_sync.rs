...
fn counterpoint_catalog_variant_sku(cp_key: &str, sku: &str) -> String {
    ...
    deterministic_counterpoint_recovery_sku(source_key)
        .unwrap_or_else(|| format!("CP-{}", stable_counterpoint_numeric_suffix(source_key)))
}
...
fn reserve_unique_counterpoint_recovery_sku(
    counterpoint_item_key: &str,
...
fn normalized_alias_option_values(values: &[String]) -> Vec<String> {
    values
        ...
        .filter(|value| !value.chars().all(|ch| ch == '*' || ch == '_'))
        .collect()
}
...
fn clean_counterpoint_variation_label(
    cp_key: &str,
    variation_label: Option<&str>,
) -> Option<String> {
    let mut values = option_values_from_counterpoint_item_key(Some(cp_key));
    if values.is_empty() {
        values = option_values_from_variation_label(variation_label);
    }
    let values = normalized_alias_option_values(&values);
...
fn recover_counterpoint_inventory_skus(
    ...
    let mut used_generated_skus = sku_counts
        .keys()
        .filter(|sku| is_counterpoint_recovered_sku(sku))
        .cloned()
        .collect::<HashSet<_>>();
    ...
    for row in &mut payload.rows {
        ...
        let Some(cp_key) = row.counterpoint_item_key.as_deref().and_then(|key| {
            reserve_unique_counterpoint_recovery_sku(key, &mut used_generated_skus)
        }) else {
            continue;
        };
    ...
    for row in &mut payload.rows {
        ...
        if !is_grid || row.cells.is_empty() {
            ...
            let Some(generated_sku) =
                reserve_unique_counterpoint_recovery_sku(&row.item_no, &mut used_generated_skus)
            else {
                continue;
            };
        ...
        for cell in &mut row.cells {
            ...
            let Some(generated_sku) = reserve_unique_counterpoint_recovery_sku(
                &cell.counterpoint_item_key,
                &mut used_generated_skus,
            ) else {
                continue;
            };
...
pub async fn record_counterpoint_import_preflight(
    ...
    for row in &payload.counts {
        ...
        let effective_required = row.required;
...
async fn load_counterpoint_import_source_count_rows(
    ...
    let rows: Vec<(
        ...
        WHERE import_run_id = $1
          AND entity_key <> 'receiving_history'
        ORDER BY entity_key
        "#,
    )
    .bind(import_run_id)
...
async fn build_counterpoint_import_run_reconciliation(
    ...
    for source in source_counts {
        ...
        let counts_match = count_difference == 0;
        let passed = counts_match || !source.required;
        rows.push(CounterpointSnapshotReconciliationRow {
            ...
            note: if counts_match {
                "Current import run provenance matches the Bridge source count.".into()
            } else if !source.required {
                "Optional Counterpoint source area is not required for go-live proof.".into()
            } else {
...
pub async fn build_counterpoint_import_command_center(
    ...
    let latest_import_status = latest_import_run.as_ref().map(|run| run.status.as_str());
    let latest_import_completed = latest_import_status == Some("completed");
    let latest_import_failed = latest_import_status == Some("failed");
    let snapshot_reconciliation = match latest_import_run.as_ref() {
    ...
    let (proof_scope, proof_scope_note) = if latest_import_completed {
        ...
        (
            "running_import_run".to_string(),
            "The Bridge import is still running. Landed proof remains pending until the run completes.".to_string(),
        )
    ...
    let required_entity_keys = source_counts
        .iter()
        .filter(|row| row.required)
        .map(|row| row.entity_key.clone())
        .collect::<Vec<_>>();
    let latest_completed_import_run_id = latest_import_run
        .as_ref()
        .filter(|run| run.status == "completed")
        .map(|run| run.id);
    let open_exception_count: i64 = match latest_completed_import_run_id {
        Some(import_run_id) if !required_entity_keys.is_empty() => sqlx::query_scalar(
            r#"
            SELECT COUNT(*)::bigint
            FROM counterpoint_import_exceptions
            WHERE import_run_id = $1
        ...
        .bind(import_run_id)
        .bind(&required_entity_keys)
        .fetch_one(pool)
        .await
        .unwrap_or(0),
        _ => 0,
    };
    let fallback_landed_exception_count: i64 = match latest_completed_import_run_id {
        Some(import_run_id) => sqlx::query_scalar(
            r#"
            SELECT COUNT(*)::bigint
            FROM counterpoint_import_exceptions
            WHERE import_run_id = $1
    ...
    let required_snapshot_rows = snapshot_reconciliation
        .iter()
        .filter(|row| row.required)
        .collect::<Vec<_>>();
    let import_proof_passed = latest_import_completed
        && !required_snapshot_rows.is_empty()
        && required_snapshot_rows.iter().all(|row| row.passed);
...
pub async fn list_counterpoint_import_exceptions(
    ...
    sqlx::query_as(
        r#"
        SELECT
            id, import_run_id, entity_key, source_key, severity, reason_code, message,
            suggested_fix, fallback_landed, ros_table, ros_id, source_payload,
            status, created_at, updated_at
        FROM counterpoint_import_exceptions
        WHERE ($3::uuid IS NULL OR import_run_id = $3)
        ORDER BY
...
pub async fn ignore_counterpoint_import_exception(
    ...
    Ok(Some(CounterpointImportExceptionResolveResult {
        ...
        reason: "Issue deleted from active review.".into(),
        ros_table: existing_ros_table,
        ros_id: existing_ros_id,
    }))
}
...
pub async fn build_counterpoint_transaction_reconciliation_snapshot(
    ...
    Ok(CounterpointTransactionReconciliationSnapshot {
        generated_at: Utc::now(),
        disclaimer: "Read-only Counterpoint ticket transaction sanity check. Tax authority depends on the imported Counterpoint tax columns and any legacy rows may require reimport/backfill; this is not full accounting reconciliation or financial close.".into(),
...
async fn upsert_variant(
    ...
    let variant_id = if let Some((vid, _, old_stock)) = existing {
        ...
        .bind(&import_sku)
        .bind(barcode)
        .bind(clean_variation_label.as_deref())
...
pub struct CounterpointGiftCardMetadataRepairRequest {
    ...
    #[serde(skip)]
    pub expected_original_value_repair_count: Option<i32>,
    #[serde(skip)]
    pub expected_original_value_repair_digest: Option<String>,
}
...
pub struct GiftCardMetadataRepairSummary {
    ...
    pub would_repair_original_values: i32,
    pub would_repair_original_value_total: Decimal,
    pub original_value_repair_digest: String,
...
fn gift_card_repair_digest(mut fingerprints: Vec<String>) -> String {
    fingerprints.sort();
    let mut hasher = Sha256::new();
...
async fn execute_counterpoint_gift_card_metadata_repair(
    ...
    let expected_original_value_repair_count = request.expected_original_value_repair_count;
    let expected_original_value_repair_digest = request.expected_original_value_repair_digest;
    if mode == CounterpointGiftCardMetadataRepairMode::Apply {
        ...
        let expected_original_count = expected_original_value_repair_count.ok_or_else(|| {
            CounterpointSyncError::InvalidPayload(
                "gift card metadata repair apply requires the reviewed original-value repair count"
                    .to_string(),
            )
        })?;
        if expected_original_count < 0 {
            return Err(CounterpointSyncError::InvalidPayload(
                "gift card metadata repair original-value repair count cannot be negative"
                    .to_string(),
            ));
        }
        let original_digest = expected_original_value_repair_digest
            .as_deref()
            .map(str::trim)
            .filter(|digest| {
                digest.len() == 64
            ...
            .ok_or_else(|| {
                CounterpointSyncError::InvalidPayload(
                    "gift card metadata repair apply requires the reviewed original-value repair digest"
                        .to_string(),
                )
            })?;
        if original_digest != original_digest.to_ascii_lowercase() {
            return Err(CounterpointSyncError::InvalidPayload(
                "gift card metadata repair original-value repair digest must be lowercase hexadecimal"
                    .to_string(),
            ));
        }
    }
    ...
    let mut repair = GiftCardMetadataRepairSummary {
        ...
        would_repair_original_values: 0,
        would_repair_original_value_total: Decimal::ZERO,
        original_value_repair_digest: String::new(),
    ...
    let mut original_value_repair_fingerprints = Vec::new();
    ...
    for row in rows {
        ...
        let source_original_value = row.original_value.ok_or_else(|| {
            CounterpointSyncError::InvalidPayload(format!(
                "gift card {code} is missing its Counterpoint original value"
            ))
        })?;
        if source_original_value <= Decimal::ZERO || source_original_value < current_balance {
            return Err(CounterpointSyncError::InvalidPayload(format!(
                "gift card {code} has invalid Counterpoint original value {source_original_value} for current balance {current_balance}"
            )));
        }
        let original_value_changed = stored_original_value != Some(source_original_value);
        if original_value_changed {
            repair.would_repair_original_values += 1;
            repair.would_repair_original_value_total += source_original_value;
            original_value_repair_fingerprints.push(format!(
                "{gift_card_id}|{code}|{}|{source_original_value}",
        ...
        if mode == CounterpointGiftCardMetadataRepairMode::Apply {
            ...
            if original_value_changed {
                sqlx::query(
                    ...
                    VALUES (
                        $1,
                        'counterpoint_gift_card_original_value_repaired',
                        jsonb_build_object(
                            'gift_card_id', $2::text,
                            'gift_card_code', $3::text,
                            'previous_original_value', $4::text,
                            'corrected_original_value', $5::text,
                            'current_balance', $6::text,
                            'reason', $7::text
                        )
                    )
                    "#,
    ...
    repair.payment_repair_digest = gift_card_repair_digest(payment_repair_fingerprints);
    repair.original_value_repair_digest =
        gift_card_repair_digest(original_value_repair_fingerprints);
    ...
    if mode == CounterpointGiftCardMetadataRepairMode::Apply
        && (expected_original_value_repair_count != Some(repair.would_repair_original_values)
            || expected_original_value_repair_digest.as_deref()
                != Some(repair.original_value_repair_digest.as_str()))
    {
        return Err(CounterpointSyncError::InvalidPayload(format!(
            "gift card original values changed after preview: reviewed {} repair(s) with digest {}, current {} repair(s) with digest {}; refresh preview and review again; no changes were committed",
            expected_original_value_repair_count.unwrap_or_default(),
            expected_original_value_repair_digest.as_deref().unwrap_or("missing"),
            repair.would_repair_original_values,
            repair.original_value_repair_digest
        )));
    }
...
async fn find_existing_pos_transaction_for_counterpoint_payment(
    ...
    let candidate_ids: Vec<Uuid> = sqlx::query_scalar(
        ...
        WHERE t.customer_id = $1
          ...
          AND (t.counterpoint_ticket_ref IS NULL OR t.counterpoint_ticket_ref <> $5)
          AND (
              (
                  NOT COALESCE(t.is_counterpoint_import, false)
...
pub async fn execute_counterpoint_ticket_batch(
    ...
    let mut bulk_line_state_taxes = Vec::new();
    let mut bulk_line_local_taxes = Vec::new();
    ...
    for tkt in &payload.rows {
        ...
        if tkt.lines.is_empty() {
            record_counterpoint_import_exception(
                ...
                serde_json::json!({
                    ...
                    "counterpoint_tax_total": tkt.tax_total,
            ...
            )
        {
            ...
            bulk_line_prices.push(*unit_price);
            bulk_line_costs.push(cost);
            bulk_line_state_taxes.push(*state_tax);
            bulk_line_local_taxes.push(*local_tax);
            bulk_line_reasons.push(line.reason_code.clone());
            bulk_line_vendor_refs.push(vendor_ref.clone());
            bulk_line_size_specs.push(size_specs);
    ...
    if !bulk_line_txn_ids.is_empty() {
        ...
        .bind(&bulk_line_state_taxes)
        .bind(&bulk_line_local_taxes)
...
fn counterpoint_open_doc_line_subtotal(lines: &[TicketLineRow]) -> Decimal {
    lines
        .iter()
...
fn counterpoint_line_source_prices(
    lines: &[TicketLineRow],
) -> (Vec<Decimal>, Vec<Option<Decimal>>) {
...
fn split_counterpoint_tax_amount(unit_price: Decimal, tax_amount: Decimal) -> (Decimal, Decimal) {
    if tax_amount.is_zero() {
        return (Decimal::ZERO, Decimal::ZERO);
    }

    let sign = if tax_amount < Decimal::ZERO {
        -Decimal::ONE
    } else {
        Decimal::ONE
    };
    let tax_abs = tax_amount.abs();
    let state_abs =
        rounded_counterpoint_money((unit_price.abs() * Decimal::new(4, 2)).min(tax_abs));
    let local_abs = rounded_counterpoint_money(tax_abs - state_abs);
    (state_abs * sign, local_abs * sign)
}

fn explicit_counterpoint_line_taxes(
    lines: &[TicketLineRow],
    unit_prices: &[Decimal],
) -> Option<Vec<(Decimal, Decimal)>> {
    let has_explicit_tax = lines.iter().any(|line| {
        line.state_tax
            .or(line.local_tax)
            .or(line.tax_amount)
    ...
    Some(
        lines
            ...
            .zip(unit_prices.iter())
            .map(|(line, unit_price)| {
                let state_tax = line.state_tax.unwrap_or(Decimal::ZERO);
                let local_tax = line.local_tax.unwrap_or(Decimal::ZERO);
                if !state_tax.is_zero() || !local_tax.is_zero() {
                    return (
                        rounded_counterpoint_money(state_tax),
                        rounded_counterpoint_money(local_tax),
                    );
                }
                line.tax_amount
                    .filter(|amount| !amount.is_zero())
                    .map(|amount| split_counterpoint_tax_amount(*unit_price, amount))
...
fn counterpoint_effective_total_from_tax(
    total_price: Decimal,
    subtotal: Decimal,
    tax_total: Decimal,
) -> Decimal {
    ...
    let total_minus_subtotal = rounded_counterpoint_money(total_price - subtotal);
    if total_minus_subtotal.abs() <= Decimal::new(1, 2) {
        rounded_counterpoint_money(subtotal + tax_total)
    } else {
        total_price
    }
}
...
fn counterpoint_open_doc_plausible_tax(subtotal: Decimal, candidate_tax: Decimal) -> bool {
    ...
    let rate = candidate_tax / subtotal;
    rate >= Decimal::new(3, 2) && rate <= Decimal::new(12, 2)
}
...
fn infer_counterpoint_open_doc_tax(
    ...
    amount_paid: Decimal,
    lines: &[TicketLineRow],
) -> Option<(Decimal, Vec<(Decimal, Decimal)>)> {
    let subtotal = rounded_counterpoint_money(counterpoint_open_doc_line_subtotal(lines));
    if subtotal <= Decimal::ZERO {
        return None;
    }

    let header_tax = rounded_counterpoint_money(total_price - subtotal);
    let tender_tax = rounded_counterpoint_money(amount_paid - subtotal);
    let inferred_tax = if counterpoint_open_doc_plausible_tax(subtotal, header_tax) {
        header_tax
    } else if total_price <= subtotal + Decimal::new(1, 2)
        && counterpoint_open_doc_plausible_tax(subtotal, tender_tax)
    {
        tender_tax
    } else {
        return None;
...
fn counterpoint_import_line_financials(
    total_price: Decimal,
    amount_paid: Decimal,
    tax_total: Option<Decimal>,
    lines: &[TicketLineRow],
    ...
    let (source_unit_prices, source_original_prices) = counterpoint_line_source_prices(lines);
    ...
    if let Some(line_taxes) = explicit_counterpoint_line_taxes(lines, &source_unit_prices) {
        let tax_total: Decimal = line_taxes
            .iter()
            .zip(lines.iter())
            .map(|((state, local), line)| Decimal::from(line.quantity) * (*state + *local))
            .sum();
        return (
            counterpoint_effective_total_from_tax(total_price, source_subtotal, tax_total),
            source_unit_prices,
            source_original_prices,
            line_taxes,
        );
    }

    if let Some(tax_total) = tax_total.filter(|amount| *amount > Decimal::ZERO) {
        let line_taxes =
            allocate_counterpoint_open_doc_line_taxes(lines, &source_unit_prices, tax_total);
        return (
            counterpoint_effective_total_from_tax(total_price, source_subtotal, tax_total),
            source_unit_prices,
            source_original_prices,
    ...
    if let Some((effective_total_price, inferred_line_taxes)) =
        infer_counterpoint_open_doc_tax(total_price, amount_paid, lines)
    {
        return (
            effective_total_price,
            lines.iter().map(|line| line.unit_price).collect(),
            vec![None; lines.len()],
            inferred_line_taxes,
        );
    }
    ...
    (
        ...
        source_unit_prices,
        source_original_prices,
        vec![(Decimal::ZERO, Decimal::ZERO); lines.len()],
    )
}
    ...
    for doc in &docs {
            ...
            )
        {
            ...
            sqlx::query(
                ...
                VALUES (
                    $1, $2, $3, $4, $5::fulfillment_type, $6, $7, $8, $9, $10, 0, 0,
                    $11, $12, $13, 'ready_for_pickup'::order_item_lifecycle_status, $14, $15, $16
                )
                "#,
            )
            .bind(transaction_id)
...
mod tests {
    ...
    #[test]
    fn counterpoint_variation_labels_skip_placeholders_and_preserve_slash_sizes() {
        assert_eq!(
            clean_counterpoint_variation_label("I-101583|BLACK||", Some("BLACK / * / *")),
            Some("BLACK".into())
        );
        assert_eq!(
            clean_counterpoint_variation_label(
                "I-100201|4630-414 SEAFOA|19.5|32/33",
                Some("4630-414 SEAFOA / 19.5 / 32/33")
            ),
            Some("4630-414 SEAFOA / 19.5 / 32/33".into())
        );
        assert_eq!(
            clean_counterpoint_variation_label(
                "I-100161|385-473430 717|36 R|**",
                Some("385-473430 717 / 36 R / **")
            ),
            Some("385-473430 717 / 36 R".into())
        );
        assert_eq!(
            option_values_from_variation_label(Some("4630-414 SEAFOA / 19.5 / 32/33")),
            vec!["4630-414 SEAFOA", "19.5", "32/33"]
        );
    }
    ...
    #[tokio::test]
    async fn counterpoint_import_preflight_does_not_require_receiving_history() {
        let _guard = SNAPSHOT_RECONCILIATION_TEST_LOCK.lock().await;
        let pool = connect_test_db().await;
        ensure_counterpoint_import_first_tables(&pool).await;
        let mut counts = realistic_import_preflight_counts();
        ...
        .fetch_one(&pool)
        .await
        .expect("count stored receiving source rows");
    ...
    #[tokio::test]
    async fn counterpoint_command_center_gates_current_required_import_proof_only() {
        let _guard = SNAPSHOT_RECONCILIATION_TEST_LOCK.lock().await;
        let pool = connect_test_db().await;
        ensure_counterpoint_import_first_tables(&pool).await;
        ...
        for row in &mut counts {
            row.source_count = 0;
            row.required = row.entity_key == "customers";
            row.status = Some("ok".into());
            row.message = None;
            row.suspicious_min_count = Some(0);
            if row.entity_key == "customers" {
                row.source_count = 1;
            }
        }
        let stale_run = record_counterpoint_import_preflight(
            ...
            CounterpointImportPreflightPayload {
                ...
                bridge_version: Some("test".into()),
                ros_base_url: Some("http://127.0.0.1:3000".into()),
                source_fingerprint: Some(format!("stale-command-center-{unique}")),
                import_first: true,
                staging_enabled: false,
                dry_run: false,
                startup_issues: vec![],
        ...
        .await
        .expect("record current preflight");
        ...
        .await
        .expect("start current import run");
        sqlx::query(
            r#"
            INSERT INTO counterpoint_import_provenance (
                import_run_id, entity_key, source_key, source_row_hash, ros_table, ros_id
            )
            VALUES ($1, 'customers', $2, 'fixture-hash', 'customers', $3)
            "#,
        )
        .bind(import_run.import_run_id)
        .bind(format!("CURRENT-{unique}"))
        .bind(Uuid::new_v4())
        .execute(&pool)
        .await
        .expect("insert current proof provenance");
        ...
        .await
        .expect("complete current import run");

        let command_center = build_counterpoint_import_command_center(&pool, true)
            .await
            .expect("build command center");
        assert!(command_center.ready_for_import);
        assert!(command_center.ready_for_go_live_review);
        assert_eq!(command_center.open_exception_count, 0);
        let receiving_row = command_center
            .snapshot_reconciliation
            .iter()
            .find(|row| row.key == "receiving_history");
        assert!(receiving_row.is_none());

        let current_exceptions =
            list_counterpoint_import_exceptions(&pool, Some(import_run.import_run_id), 100, 0)
                .await
                .expect("list current exceptions");
        assert!(current_exceptions.is_empty());

        let _ =
            sqlx::query("DELETE FROM counterpoint_import_exceptions WHERE import_run_id = ANY($1)")
                .bind(&vec![stale_run.import_run_id, import_run.import_run_id])
                .execute(&pool)
                .await;
        let _ = sqlx::query("DELETE FROM counterpoint_import_runs WHERE id = ANY($1)")
            .bind(&vec![
                stale_run.import_run_id,
    ...
    async fn counterpoint_command_center_shows_partial_failed_import_progress() {
        ...
        .await
        .expect("record failed-proof preflight");
        let import_run = start_counterpoint_import_run(
            &pool,
            CounterpointImportRunStartPayload {
                preflight_import_run_id: Some(preflight.import_run_id),
        ...
        .await
        .expect("start failed-proof import run");
        ...
        sqlx::query(
            ...
            VALUES ($1, 'customers', $2, 'failed-proof-hash', 'customers', $3)
            "#,
        )
        .bind(import_run.import_run_id)
        .bind(format!("FAILED-PROOF-{unique}"))
        .bind(Uuid::new_v4())
        .execute(&pool)
        .await
        .expect("insert partial failed provenance");
        complete_counterpoint_import_run(
            &pool,
            CounterpointImportRunCompletePayload {
                import_run_id: import_run.import_run_id,
                failed: true,
                error_message: Some("catalog SQL made no progress".into()),
                totals: Some(serde_json::json!({ "customers": 1 })),
            },
        )
        .await
        .expect("fail import run");

        let command_center = build_counterpoint_import_command_center(&pool, true)
            .await
            .expect("build failed-proof command center");
        assert_eq!(command_center.proof_scope, "failed_import_run");
        assert!(command_center.recommendation.contains("IMPORT FAILED"));
    ...
    async fn counterpoint_catalog_import_lands_generated_sku_for_missing_barcode() {
        ...
        let generated_sku =
            deterministic_counterpoint_recovery_sku(&item_no).expect("generated sku");
        let payload = CounterpointCatalogPayload {
    ...
    #[test]
    fn open_doc_tax_inference_treats_tender_subtotal_gap_as_tax() {
        let lines = vec![TicketLineRow {
            ...
            description: Some("Gruppo Bravo Suit (46306)".into()),
            reason_code: None,
        }];

        let (total, taxes) =
            infer_counterpoint_open_doc_tax(Decimal::new(26000, 2), Decimal::new(28275, 2), &lines)
                .expect("infer open-doc tax from tender subtotal gap");
    ...
    fn historical_payment_only_ticket_does_not_reprice_merchandise_from_tender() {
        ...
        let (total, unit_prices, original_prices, _) = counterpoint_import_line_financials(
            Decimal::ZERO,
            Decimal::new(13000, 2),
            None,
            &lines,
        ...
        assert_eq!(unit_prices, vec![Decimal::new(26000, 2)]);
        assert_eq!(original_prices, vec![None]);
    }

    #[test]
    fn counterpoint_import_financials_preserves_explicit_line_tax_and_discount() {
        let lines = vec![TicketLineRow {
            ...
            state_tax: None,
            local_tax: None,
            tax_amount: Some(Decimal::new(569, 2)),
            original_unit_price: None,
            discount_amount: Some(Decimal::new(1000, 2)),
            description: Some("Mantoni Classic Fit DrShirt".into()),
            reason_code: None,
        }];
        ...
        assert_eq!(original_prices, vec![Some(Decimal::new(7500, 2))]);
        assert_eq!(taxes, vec![(Decimal::new(260, 2), Decimal::new(309, 2))]);
    }

    #[test]
    fn counterpoint_import_financials_allocates_header_tax_total() {
        let lines = vec![TicketLineRow {
            sku: Some("B-1471081".into()),
            counterpoint_item_key: Some("I-102120|M20001-1|16.5|2/3".into()),
            lin_seq_no: Some(1),
    ...
    async fn counterpoint_gift_card_metadata_repair_preserves_balance() {
        ...
        let source_original_value = Decimal::new(7500, 2);
        ...
        let payload =
            |mode,
             balance,
             expected_payment_repair: Option<(i32, String)>,
             expected_original_value_repair: Option<(i32, String)>| {
                let (expected_payment_repair_count, expected_payment_repair_digest) =
                    expected_payment_repair
                        ...
                        .unwrap_or((None, None));
                let (expected_original_value_repair_count, expected_original_value_repair_digest) =
                    expected_original_value_repair
                        .map(|(count, digest)| (Some(count), Some(digest)))
                        .unwrap_or((None, None));
                CounterpointGiftCardsPayload {
                    rows: vec![CounterpointGiftCardRow {
                        ...
                        balance,
                        original_value: Some(source_original_value),
                    ...
                    metadata_repair: Some(CounterpointGiftCardMetadataRepairRequest {
                        ...
                        expected_original_value_repair_count,
                        expected_original_value_repair_digest,
                    }),
                }
            };
        ...
        assert_eq!(preview_repair.would_repair_original_values, 1);
        assert_eq!(
            preview_repair.would_repair_original_value_total,
        ...
        let reviewed_original_value_repair = (
            preview_repair.would_repair_original_values,
            preview_repair.original_value_repair_digest.clone(),
        );
        ...
        let mut unaudited_apply = payload(
            ...
            Some(reviewed_original_value_repair.clone()),
        );
        ...
        let applied = execute_counterpoint_gift_card_batch(
            ...
            payload(
                ...
                Some(reviewed_original_value_repair),
            ),
        )
        .await
        ...
        let repaired: (String, bool, Decimal, Decimal, DateTime<Utc>, DateTime<Utc>, Option<Uuid>) = sqlx::query_as(
            "SELECT card_kind::text, is_liability, current_balance, original_value, created_at, expires_at, customer_id FROM gift_cards WHERE id = $1",
        )
        .bind(gift_card_id)
        .fetch_one(&pool)
        .await
        ...
        assert_eq!(repaired.3, source_original_value);
        assert_eq!(repaired.4, issued_at);
        assert_eq!(repaired.6, Some(customer_id));
        assert_eq!(
            repaired.5,
        ...
        let original_value_audit: JsonValue = sqlx::query_scalar(
            r#"
            SELECT metadata
            FROM staff_access_log
            WHERE staff_id = $1
              AND event_kind = 'counterpoint_gift_card_original_value_repaired'
              AND metadata->>'gift_card_id' = $2
            "#,
        )
        .bind(repair_staff_id)
        .bind(gift_card_id.to_string())
        .fetch_one(&pool)
        .await
        .expect("load gift card original-value repair audit");
        assert_eq!(
            original_value_audit
                .get("corrected_original_value")
                .and_then(JsonValue::as_str),
            Some("75.00")
        );
        ...
        let repeated = execute_counterpoint_gift_card_batch(
            ...
            payload(
                ...
                Some((
                    repeated_preview_repair.would_repair_original_values,
                    repeated_preview_repair.original_value_repair_digest,
                )),
            ),
        )
        .await
        ...
        let repeated_repair = repeated.metadata_repair.expect("repeated repair summary");
        assert_eq!(repeated_repair.would_reclassify_payments, 0);
        assert_eq!(repeated_repair.would_repair_original_values, 0);

        sqlx::query(
            "DELETE FROM staff_access_log WHERE staff_id = $1 AND event_kind IN ('counterpoint_gift_card_payment_classification_repaired', 'counterpoint_gift_card_original_value_repaired')",
        )
        .bind(repair_staff_id)
        .execute(&pool)
        .await
