...
pub async fn run_integration_heartbeat(
    ...
    let (qbo_status, qbo_detail) = if !qbo_h.configured {
        ("CAUTION".to_string(), "QBO integration not configured".to_string())
    ...
    let (podium_status, podium_detail) = if !podium_h.configured {
        ("CAUTION".to_string(), "Podium integration not configured".to_string())
    ...
    record_status(pool, "podium", &podium_status, &podium_detail, podium_has_failed).await?;

    // 3. Shippo
    let shippo_h = shippo::health_check(http_client).await;
    let (shippo_status, shippo_detail) = if !shippo_h.configured {
        ("CAUTION".to_string(), "Shippo integration not configured".to_string())
    ...
    record_status(pool, "shippo", &shippo_status, &shippo_detail, shippo_has_failed).await?;

    // 4. Weather
    let weather_h = weather::health_check(http_client).await;
    let (weather_status, weather_detail) = if !weather_h.configured {
        ("CAUTION".to_string(), "Weather integration not configured".to_string())
    ...
    record_status(pool, "weather", &weather_status, &weather_detail, weather_has_failed).await?;

    // 5. Fal.ai
    let fal_h = fal_sidecar::health_check(http_client).await;
    let (fal_status, fal_detail) = if !fal_h.configured {
        ("CAUTION".to_string(), "Fal.ai integration not configured".to_string())
    ...
    let (nu_status, nu_detail) = match nuorder_client_from_pool(pool).await {
        ...
        Err(_) => {
            ("CAUTION".to_string(), "NuORDER integration not configured".to_string())
        }
    };
    ...
    let (ms_status, ms_detail) = if let Some(client) = meilisearch_client {
        ...
        ("CAUTION".to_string(), "Meilisearch not configured".to_string())
    };
...
async fn record_status(
    ...
    let prev: Option<(String,)> = sqlx::query_as(
        "SELECT status FROM integration_alert_state WHERE source = $1"
    )
    .bind(source)
    .fetch_optional(pool)
    .await?;
