...
fn resolve_rosie_speech_python_path() -> PathBuf {
    ...
    // 2. Probe uv tool install locations (platform-specific)
    #[cfg(windows)]
    {
        if let Ok(local_app) = std::env::var("LOCALAPPDATA") {
            // uv tool install puts python here on Windows
            ...
            // Fallback dedicated venv (attempt 3 in installer)
    ...
    // 3. Last-resort default (dev machine path)
    PathBuf::from(if cfg!(windows) {
        r"C:\Users\Default\.local\share\uv\tools\sherpa-onnx\Scripts\python.exe"
    } else {
