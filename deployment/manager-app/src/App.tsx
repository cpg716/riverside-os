...
export default function App() {
  ...
  const [dbPasswordSaving, setDbPasswordSaving] = useState(false);
  ...
  const saveDatabaseCredentials = async () => {
    if (dbPasswordSaving) return;
    setDbPasswordSaving(true);
    try {
      await saveMainHubConfig('main-hub');
      setLogs(prev => [...prev, { level: 'success', text: 'PostgreSQL admin password saved to deployment config.' }]);
      await refreshPgStatus();
    } catch (e) {
      setLogs(prev => [...prev, { level: 'error', text: `Could not save PostgreSQL admin password: ${e}` }]);
    } finally {
      setDbPasswordSaving(false);
    }
  };
                ...
                <div>
                  ...
                  <div className="p-4 rounded-xl border border-zinc-200 bg-white space-y-4 shadow-sm">
                    ...
                    <div className="rounded-lg border border-zinc-100 bg-zinc-50 p-3">
                      <label className="block text-[11px] font-semibold uppercase tracking-wide text-zinc-600 mb-1">
                      ...
                      <div className="flex flex-col gap-2 sm:flex-row">
                        <input
                          ...
                          value={dbPassword}
                          onChange={(e) => setDbPassword(e.target.value)}
                          placeholder="Enter the postgres admin password"
                          className="min-w-0 flex-1 rounded-lg border border-zinc-200 px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-brand-500"
                        />
                        <button
                          type="button"
                          onClick={() => void saveDatabaseCredentials()}
                          disabled={dbPasswordSaving}
                          className="rounded-lg bg-brand-600 px-4 py-2 text-xs font-bold text-white transition-all hover:bg-brand-700 disabled:opacity-50"
                        >
                          {dbPasswordSaving ? 'Saving...' : 'Save & Check'}
                        </button>
                      </div>
                      <p className="mt-1 text-[11px] text-zinc-500">
                        Saved to deployment config and reused by Database Health, migrations, repair, reset, and Main Hub update actions.
                      </p>
                    </div>
                    {pgStatus === null ? (
                ...
                <div className="mb-4 max-w-md">
                  ...
                  <div className="flex gap-2">
                    <input
                      ...
                      className="min-w-0 flex-1 px-3 py-2 border rounded-lg text-sm focus:ring-2 focus:ring-brand-500 outline-none"
                    />
                    <button
                      type="button"
                      onClick={() => void saveDatabaseCredentials()}
                      disabled={dbPasswordSaving}
                      className="rounded-lg bg-brand-600 px-3 py-2 text-xs font-bold text-white transition-all hover:bg-brand-700 disabled:opacity-50"
                    >
                      {dbPasswordSaving ? 'Saving...' : 'Save'}
                    </button>
                  </div>
                  <p className="mt-1 text-[11px] text-zinc-500">Saved before Main Hub update and reused by status checks, migrations, repair, and reset actions.</p>
                </div>
