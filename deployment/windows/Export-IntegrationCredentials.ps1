...
try {
  ...
  $exists = & $psql $databaseUrl -tAc "SELECT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'integration_credentials');"
  if (($exists -join "").Trim() -ne "t") {
    throw "integration_credentials table does not exist. Run migrations first."
  }

  $count = & $psql $databaseUrl -tAc "SELECT count(*) FROM integration_credentials;"
  ...
  & pg_dump --data-only --table=integration_credentials --no-owner --no-privileges --column-inserts $databaseUrl > $OutPath
