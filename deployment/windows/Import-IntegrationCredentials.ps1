...
try {
  $tableExists = & $psql $databaseUrl -tAc "SELECT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'integration_credentials');"
  if (($tableExists -join "").Trim() -ne "t") {
    Write-Warning "integration_credentials table does not exist yet. Skipping credential import."
    return
  }

  $existingCount = & $psql $databaseUrl -tAc "SELECT count(*) FROM integration_credentials;"
  ...
  if ($existingCount -ne "0" -and $Force) {
    ...
    & $psql $databaseUrl -c "TRUNCATE TABLE integration_credentials;" 2>&1 | Write-Host
  }

  Write-Host "Importing integration credentials from $SqlPath..."
  & $psql $databaseUrl -v ON_ERROR_STOP=1 -1 -f $SqlPath 2>&1 | Write-Host
  if ($LASTEXITCODE -ne 0) {
    throw "psql import failed with exit code $LASTEXITCODE."
  }

  $importedCount = & $psql $databaseUrl -tAc "SELECT count(*) FROM integration_credentials;"
