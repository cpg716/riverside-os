...
### Fixed
- **Streamlined POS Navigation and Workspaces**: POS now keeps five top-level destinations, groups supporting work and operations without a scrolling rail, reduces duplicate dashboard and empty-cart summaries, and moves secondary sale tools into a responsive **More Actions** dialog. POS Customers and Orders load focused 25-record/action-first views while Back Office retains the full management surfaces.
...
- **Partial Order-Item Cancellation and Refund Truth**: Customer Orders now provides a dedicated **Cancel Order Items** review for unfulfilled Special, Custom, Wedding, and Layaway lines. The live Transaction ledger previews how much cancellation credit reduces the unpaid balance, the revised Order total, any remaining balance, and only the actual overpayment eligible for refund. Cancelled lines remain distinct from customer returns, procurement-bearing lines are blocked for resolution, true refunds continue through multi-source Pay and verified Helcim settlement, and Return / Exchange rejects unfulfilled Order merchandise.
...
- **Orders Pickup Cart Review**: Loading some or all open items from Customer Orders now returns staff to the Register Cart for review instead of immediately opening Payment. Historical balances are no longer staged automatically; staff can add new items, explicitly add a full or partial payment when intended, or complete an already-paid pickup without a new tender.
