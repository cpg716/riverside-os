...
- **Private Windows Build Transport**: Mac-to-Main Hub candidate builds now upload their PowerShell launcher as a temporary file instead of passing it through Windows' command-line length limit, remain compatible with Windows PowerShell 5.1 and its DPAPI assembly, sign non-interactively without exposing private key material on the command line, initialize the installed Visual Studio C++ environment and linker, stage Tauri's verified prerequisites before workspace checks, and use short isolated package staging to stay within legacy Windows path limits.
- **Immutable Average-Cost Migration Recovery**: Migration 207 is restored byte-for-byte to the checksum already recorded by existing databases, while its later historical line-repair audit table now lands through new migration 213. Startup continues to fail closed on genuine migration drift without rewriting the migration ledger or changing inventory and financial records.
