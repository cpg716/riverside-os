...
- **Release Asset Set Reconciliation**: Published replacement assets now exclude an obsolete, unreferenced Register NSIS executable left by the superseded same-version build; the live updater and build manifests point only to the signed `d9e68018` MSI, and candidate promotion removes prior core Register installer variants that are absent from the verified candidate.
...
- **v0.95.0 Release Evidence**: Recorded published commit `d9e68018`, exact-commit Lint/Playwright, successful Windows/macOS candidates, verified promotion, the final 28-asset release set, all ten exact-SHA updater/build manifests, the ROSIE-preserving short-SHA Windows deployment package, and the remaining physical Main Hub verification hold.
