...
### Fixed
- **Server-Native Governed Insights**: Native Insights no longer requires a Cube Core container, shared reporting secret, sidecar port, or separate reporting login. Riverside validates the model-produced report specification, maps it only to approved `reporting.*` fields, and executes a bounded read-only query with a statement timeout inside the authenticated server. Settings now reports whether the required reporting views are ready and can show store-specific reporting guidance without exposing database credentials.
