...
- **Podium Customer Name Preservation and RMS Surname Repair**: Podium contact reconciliation no longer lets a combined display name or one-letter provider value overwrite an established Riverside first or last name. Explicit, complete provider fields may still update a profile, and meaningful display names can replace only the original `Podium Contact` placeholder or fill a genuinely blank field. An audited, fail-closed migration restores the 287 RMS-linked surnames whose exact account, creation, phone, provider-payload, and latest-report evidence proves the prior truncation, then queues those corrected profiles through the existing Podium contact-sync path.
