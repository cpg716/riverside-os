...
### Fixed
- **Private Windows Build Transport**: Mac-to-Main Hub candidate builds now upload their PowerShell launcher as a temporary file instead of passing it through Windows' command-line length limit, remain compatible with Windows PowerShell 5.1 and its DPAPI assembly, sign non-interactively without exposing private key material on the command line, initialize the installed Visual Studio C++ environment and linker, and stage Tauri's verified sidecar and transient generated-resource directory before checking the Windows workspace.
