...
### Fixed
- **Private Windows Build Transport**: Mac-to-Main Hub candidate builds now upload their PowerShell launcher as a temporary file instead of passing it through Windows' command-line length limit, keep the install-contract validator parse-safe under Windows PowerShell 5.1, and initialize the installed Visual Studio C++ environment and linker for non-interactive Rust builds.
