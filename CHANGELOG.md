...
- **Reliable Full-Suite Release Timing and Local Build Identity**: Required GitHub Playwright shards now have a 60-minute ceiling for the complete standard suite. The local server launcher also supplies the current commit as an explicit Cargo build input, so local version metadata refreshes after non-Rust commits instead of retaining a stale build SHA.
