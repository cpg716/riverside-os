...
### Fixed
- **Private Windows Build Transport**: Mac-to-Main Hub candidate builds now upload their PowerShell launcher as a temporary file instead of passing it through Windows' command-line length limit, keep the install-contract validator parse-safe under Windows PowerShell 5.1, initialize the installed Visual Studio C++ environment and linker, and stage Tauri's verified sidecar and transient generated-resource directory before checking the Windows workspace.
