...
### Added
- **Private Mac-to-Main-Hub Windows Release Lane**: Committed Riverside source can now travel directly from the Mac to an isolated, low-priority Windows build workspace over LAN or Tailscale SSH for Windows-native validation, exact-build packaging, protected Tauri updater signing, and explicit guarded promotion. Promotion verifies artifact hashes, reuses the existing pre-migration backup and rollback installer, requires exact Main Hub database/search readiness, and only then publishes the signed workstation update from the Main Hub. Normal validation/package requests remain non-deploying, store-hours execution is blocked by default, and GitHub is not required for this private release path.
- **Auditable Retail and Sale Price History**: Every parent and SKU Retail/Sale mutation now writes exact before/after override and effective prices to an append-only database ledger, including authenticated staff context for Product Hub edits and explicit automated-source attribution for imports and integrations. Product Timeline exposes the history for staff review, while a governed reporting view supports business-date, product, SKU, price-kind, actor, source, and price-change reporting.
- **Riverside-Wide Simplify, Enhance, Optimize Audit**: A production-data, read-only audit now records the staff UX standard, prioritized findings, evidence boundaries, implemented source remediations, and remaining mutation, responsive, and store-hardware certification work across every Riverside OS staff area. The staff-facing pass prioritizes clear next actions, meaningful icons and status graphics, progressive disclosure, retained results, accessible controls, and authoritative operational state while allowing Settings and development diagnostics to remain detailed.
- **Authoritative Transaction Records and Pickup Readiness**: Back Office now provides a plain-language Transaction Records archive distinct from logistical Fulfillment Orders. Order lists visibly retain but lock rows during refresh, use bounded summary hydration, and prune actions that do not apply; Pickup Queue rows and summary totals now use the same server-owned eligibility, balance, lifecycle, and reason-code model.
- **Reviewed Customer and Wedding Batch Work**: Customer group and wedding-party changes now show the selected target before writing and use permission-checked server batch endpoints with explicit complete or partial results. Customer Hub and Add Customer protect meaningful drafts, provide compact graphical navigation and stable loading states, and keep common staff actions beside customer identity.
- **Direct Register Camera and Parked-Sale Access**: Register now exposes camera scanning beside product search through the existing authoritative scan endpoint. **Resume Parked Sale** lives in the three-by-three More Actions panel and opens the audited parked-sale picker, where staff can resume, remove, or close without changing the current Cart.
...
- **Pick Up Later Fulfillment**: Register fully sells an exact, available in-stock variation to a linked Customer without sending it through procurement or receiving. Checkout removes it from inventory and recognizes revenue and commission on the sale day, while a Ready-for-Pickup Fulfillment Order tracks Riverside custody until the later audited pickup releases the item without posting those financial events twice.

### Fixed
- **Wedding Party Customer and Register Connection**: New Party and Party Management now search authoritative ROS Customers before adding a member, quick-create unmatched people from separate First and Last names, use a consistent member-type list including Groom, Groomsman, Father, Child, and Other, and persist the selected Customer ID instead of discarding it. Staff can save a new Groom or managed member and open that exact wedding context in Register immediately; the normal audited checkout remains the only financial write.
- **Private Windows Cube Packaging**: Exact Cube Core loopback enforcement now counts the pinned server listen anchor as a complete ordinal substring, so the package still fails closed on zero or multiple matches without misreading the anchor's individual characters as separators.
- **Private Windows Build Transport**: Mac-to-Main Hub candidate builds now upload their PowerShell launcher as a temporary file instead of passing it through Windows' command-line length limit, remain compatible with Windows PowerShell 5.1 and its DPAPI assembly, sign non-interactively without exposing private key material on the command line, initialize the installed Visual Studio C++ environment and linker, stage Tauri's verified prerequisites before workspace checks, use short isolated package staging to stay within legacy Windows path limits, and initialize the existing machine-local account without misresolving it through the `WORKGROUP` pseudo-domain.
- **Immutable Average-Cost Migration Recovery**: Migration 207 is restored byte-for-byte to its canonical checksum, while its later historical line-repair audit table lands through migration 213. Deployment migration tooling now recognizes only the exact known expanded migration-207 checksum, requires the exact immutable migration-213 companion, and atomically normalizes that one historical ledger row before continuing; all other migration drift remains fail-closed.
- **Podium Inbox Customer Contact Details**: Linked Inbox conversations now show the customer's formatted phone number and Riverside customer number in both the conversation list and the open-thread header, and the open thread can launch its exact provider conversation in Podium. Ambiguous Podium identities remain fail-closed for staff review instead of being renamed from message wording.
- **Counterpoint Joint Household Names**: Counterpoint customer names written as `Sam & Renee Smith` now remain one ROS household profile with `Sam & Renee` in First name and `Smith` in Last name. The parser handles both split source fields and an unsplit `NAM`, while a source-proven, append-only audited migration corrects existing Counterpoint-created rows without linking, merging, adding, or deleting customer profiles.
- **Selected-Line Shipping Release**: Shipping now loads the complete fulfillment row required by its readiness and financial guards, so staff receive the correct blocked-or-released result instead of an internal server error.
- **Staff-Attributed Customer Merge History**: Completing a duplicate-customer merge now writes an **Account merge** event to both the surviving Customer History and the retained inactive account. Each event names both customer codes and records the authenticated staff member who completed the merge, while the existing fail-closed linked-history checks and atomic merge transaction remain authoritative.
- **Legally Reconciled New York Sales Tax Reporting**: The fixed report and approved conversational Insights dataset now use Riverside's Completed/Fulfilled reporting basis instead of reclassifying historical lines from current catalog data. Paid takeaway/picked-up/fulfilled Transactions enter on their store-local completion date, settled refund and exchange events reduce the date on which they settled, and finalized completed-sale evidence is selected once without a Counterpoint/ROS cutoff. Gross, taxable, nontaxable, state tax, local tax, and total tax all come from saved financial evidence and reconcile to the cent or both report paths fail closed. The report never reruns the tax engine, reads a Z-close snapshot, or shows the generic Counterpoint banner.
- **Average and Last Cost Separation**: Riverside now retains last invoice/source cost separately from average/WAC cost. Product Hub shows both values, receiving records invoice unit as Last Cost while updating Average Cost, and valuation, frozen COGS, margins, below-cost controls, and employee cost-plus pricing continue through the effective average-cost path. Fail-closed ROS-only repair scripts apply reviewed Counterpoint current and historical `AVG_COST` evidence with per-product and per-Transaction-line audit records; no Bridge or inventory reimport is used.
- **Podium Customer Name Preservation and Proven Name Repair**: Podium contact reconciliation no longer lets a combined display name or one-letter provider value overwrite an established Riverside first or last name. Explicit, complete provider fields may still update a profile, and meaningful display names can replace only the original `Podium Contact` placeholder or fill a genuinely blank field. Audited, fail-closed migrations restore the 287 RMS-linked surnames and 72 Counterpoint couple names whose exact source and provider evidence proves the prior truncation, then queue those corrected profiles through the existing Podium contact-sync path. Literal Counterpoint records such as `TEMPLATE` and `DELETE` remain unchanged pending individual review.
- **Complete RMS Tender Reporting and Required Program Choice**: Register now requires staff to explicitly choose **Standard** or **90 Day** for every RMS Charge and the API rejects missing or mismatched program/tender combinations. Daily Sales, emailed Daily Financial Reports, and Z-Reports keep all supported payment-day tender rows visible even at zero and report Standard and 90 Day RMS charges separately. The RMS Charge workspace uses the same staff labels, while a fail-closed migration corrects the two verified historical 90 Day charges and their closed Z snapshots without changing amounts, allocations, dates, customers, sessions, or statuses.
- **Release Publication Waits for the Full Playwright Window**: Windows and macOS release gates now wait up to 110 minutes for the same-commit Playwright workflow, covering its build phase and 60-minute blocking-shard ceiling without publishing early or failing after the previous 45-minute polling limit.
- **Grouped-Rail Release Coverage and Refund Handoff**: Required browser tests now open secondary workspaces through the same **More workspaces** disclosure used by staff, recognize the action-focused Notifications label, and wait for the sign-in gate or a restored shell before navigation. Voided Transaction Records also keep **Process Refund** available when the authoritative void record still has a refundable amount, even after returned lines reduce the current Transaction total to zero.
- **Date-Scoped Daily Sales Order Counts**: Daily Sales now derives new Special and Custom Order counts from authoritative initial booking events inside the selected store-local range. Historical open work and later amendments no longer inflate Today, custom-date, print, CSV, or Z-Report counts.
- **Calmer, Faster Daily Staff Workspaces**: Customers, Weddings, Alterations, Appointments, Inventory, Online Store, Communications, Payments, Reports, Insights, Staff, Schedule, Tasks, Help, and the shared shell now prioritize primary work, distinguish initial/loading/updating/degraded states, remove duplicated or fabricated summaries, group provider incidents, and preserve advanced evidence through progressive disclosure. Shared drawers and repeated row controls also receive focused accessibility and route-ownership corrections.
- **Stable Insights Print and PDF Previews**: Insights now captures the complete rendered chart as a deterministic PNG before opening print preview, uses landscape pages for visual reports, repeats table headings, keeps rows together where possible, and preserves every returned result row. Chart animation is disabled for capture, missing dimensions use a readable **Unassigned** label, and preview failures remain visible instead of silently opening an incomplete report.
- **Focused Register Cart and One-Screen Z Review**: The Register now keeps Staff, Salesperson, and Store Date/Time at the top, leaves Customer and Wedding identity in the right rail instead of duplicating it above the Cart, and provides a three-by-three More Actions panel with a dedicated **Resume Parked Sale** picker. The final Z-Report review is one wide, non-scrolling readiness workspace with drawer, tender, deposit, note, and audit-count facts; operational recovery detail remains preserved in recovery, diagnostics, reporting, and the close audit instead of filling the Z screen.
- **Fast Mailbox Opening and Calm Recovery Sync**: Mailbox now loads compact conversation summaries before fetching only the selected email body, keeps a clear skeleton workspace visible while the list arrives, and exposes unmatched Customer mail as an instant compact search-toolbar filter. Checkout-recovery mirroring now limits concurrent requests and briefly backs off unchanged failed records, preventing a large stale local queue from flooding the Main Hub and delaying unrelated workspaces.
- **Safe Recovery After Main Hub Restores or Local Database Resets**: Checkout and receipt recovery payloads that reference a Transaction no longer present on the current Main Hub are preserved as blocked evidence with an explicit Manager-review explanation. Riverside leaves the invalid database link empty instead of triggering repeated foreign-key 500 errors, while retaining the original claimed Transaction identity inside the recovery payload for reconciliation or authorized dismissal.
- **Reliable Full-Suite Release Timing and Local Build Identity**: Required GitHub Playwright shards now have a 60-minute ceiling for the complete standard suite. One local dev-stack launcher supplies the same current commit to the API and Vite as an explicit build input, so local version metadata refreshes after non-Rust commits without creating a false client/server mismatch during startup.
- **Reliable Nightly Playwright and Split-Refund Z-Close**: Split refund sources now remain one return/refund activity with every tender component retained, preventing valid multi-tender refunds from creating duplicate Daily Sales identities that block Z-close. The required GitHub Playwright gate runs the complete standard suite across four shards, while focused local commands remain available for fast development validation.
