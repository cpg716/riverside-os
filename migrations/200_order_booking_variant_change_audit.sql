-- Preserve zero-value Order line changes in booked Daily/Z activity. Financial
-- deltas remain exact, while a same-price product, variant, fulfillment, or
-- line-kind change still leaves an auditable amendment event.
...
BEGIN
    ...
    IF OLD.quantity IS DISTINCT FROM NEW.quantity
        ...
        OR OLD.fulfillment IS DISTINCT FROM NEW.fulfillment
        OR OLD.custom_item_type IS DISTINCT FROM NEW.custom_item_type
    THEN
        ...
        INSERT INTO transaction_line_booking_events (
            ...
            jsonb_build_object(
                ...
                'old_fulfillment', OLD.fulfillment,
                'new_fulfillment', NEW.fulfillment,
                'old_custom_item_type', OLD.custom_item_type,
                'new_custom_item_type', NEW.custom_item_type
            )
        );
    END IF;
...
COMMENT ON FUNCTION record_transaction_line_booking_event() IS
    'Records signed booked financial deltas and zero-value Order line identity or fulfillment changes for Daily/Z audit output.';
