...
**Other hourly bundles (same pattern):** `wedding_soon_bundle`, `order_due_stale_bundle`, `pickup_stale_bundle`, `appointment_soon_bundle`, `integration_health_failed_bundle`, `gift_card_expiring_soon_bundle`, `special_order_ready_to_stage_bundle`, `negative_available_stock_bundle`. **Tasks:** `task_due_soon_bundle:{assignee_staff_id}:{yyyy-mm-dd}` — due-today/tomorrow instances are pre-materialized for working staff, then prior day’s assignee bundles for that date are deleted and recreated.
