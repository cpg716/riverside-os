...
Status: **Canonical Shipping / Shippo / Shipments Hub reference**. Cross-cutting Podium and notification behavior starts at [Podium Integration](staff/podium-integration-staff-manual.md).
...
| Mechanism | Notes |
|-----------|--------|
| **Settings -> Integrations -> Geoapify** | Encrypted Geoapify API key for protected staff address typeahead. Geoapify searches nationwide U.S. addresses, ranks Western New York and exact street-address matches first, and never exposes the key to the browser. |
...
Staff address entry uses Geoapify for nationwide typeahead with Western New York ranking. Selecting a result immediately fills the available street, secondary address, city, state, and ZIP fields; broad city/state/postcode-only suggestions are filtered out so operators see usable street-address candidates. Address autocomplete does not call Shippo. Shippo is used later when a shipping workflow requests rates, purchases a label, or explicitly tests the configured origin.
