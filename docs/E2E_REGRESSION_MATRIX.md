...
- **Required:** save the GitHub repository (`owner/repo`) and read-only Actions token in **Settings → ROS Dev Center**.
...
- Store only in **Settings → ROS Dev Center**; do not place in client or server `.env` files.
...
| **`visual-baselines.spec.ts`** | Viewport screenshots: register closed, QBO, dark inventory, customers, operations | **Opt-in release lane**: runs when **`E2E_RUN_VISUAL=1`** (local or CI). By default this suite is skipped in the standard release command to avoid release-blocking snapshot drift from fonts/layout/render differences. For v0.50 GOLD it was certified separately on 2026-05-14 with the E2E API/DB env and refreshed canonical snapshots. |
...
| **`offline-recovery-contract.spec.ts`** | 4xx checkout replay retention as blocked recovery, visible repair actions, nonblocking authorized close, and immutable audited close evidence outside the financial Z-Report | Production hardening audit contract |
...
| 2026-04-25 | Added production hardening audit contracts for checkout tender financials, tax, commission, inventory, offline recovery, QBO, and register close; local release gate reported **181 passed, 7 skipped, 0 failed**. Superseded by the v0.50 GOLD evidence on 2026-05-14. |
