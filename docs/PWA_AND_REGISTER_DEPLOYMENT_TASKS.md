...
- [x] Target Riverside version is now **`v0.70.6`** across root, client/PWA, server, Tauri, and Deployment Manager metadata.
...
- [ ] Latest `main` **Lint Checks** must be green on the `v0.70.6` release commit before calling the current head release-ready.
- [ ] Latest `main` **Playwright E2E** must be green on the `v0.70.6` release commit before calling the current head release-ready. Use the release-certification evidence plus a fresh CI run after commit/push.
