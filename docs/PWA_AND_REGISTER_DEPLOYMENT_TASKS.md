...
### A.1 Current release artifact status (2026-06-03)

- [x] Target Riverside version is now **`v0.85.5`** across root, client/PWA, server, Tauri, and Deployment Manager metadata.
...
- [x] The Windows deployment package workflow publishes `RiversideOS-v0.85.5-Windows-Deployment.zip` containing the server binary, web bundle, register installer, Deployment Manager, migrations, and PowerShell scripts.
...
- [ ] Latest `main` **Lint Checks** must be green on the `v0.85.5` release commit before calling the current head release-ready.
- [ ] Latest `main` **Playwright E2E** must be green on the `v0.85.5` release commit before calling the current head release-ready. Use the release-certification evidence plus a fresh CI run after commit/push.
