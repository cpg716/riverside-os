...
3. **Runtime Diagnostics**: Confirm the station's resolved API base, strict-production status, Helcim readiness, Shippo mode, native Insights readiness, help-search mode, weather mode, backup path, and station lifecycle governance without exposing any secrets.
...
- **Insights**: the workspace shows whether approved report data is ready. If not, use the normal Main Hub update or repair process; there is no separate reporting password.
