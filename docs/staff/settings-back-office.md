...
- **Counterpoint import:** Use **Import & Proof** first: confirm the Bridge is connected, run the Bridge import, then review landed rows, exceptions, and blockers. **Sent** means the Bridge posted rows to ROS; **Landed** means ROS wrote and linked those rows for proof. Use **CSV Cleanup** after import data exists in ROS to load the Lightspeed and Counterpoint CSV references, generate a review pack, import reviewed JSON, and apply only approved cleanup. Use **Customer Duplicates** after customers land. **Support Diagnostics** is for recovery, optional prepared packages, and deeper troubleshooting, not the normal go-live path.
