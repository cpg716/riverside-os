...
- **Counterpoint bridge:** Status, **Inbound staging** toggle, queue **Apply/Discard**, bridge sync token, and **Maps**. The Command Center includes **Bridge connection status** with token, accepted heartbeat, host, phase, and version so staff can tell whether ROS has accepted the Bridge, not just whether the Bridge app is open locally. To prevent console spam when you are away from the store, the bridge status panel will stop checking automatically after **3 failures**. Use the **[Reconnect]** button to resume monitoring. If saving the token warns that `RIVERSIDE_CREDENTIALS_KEY` is missing, use the Windows deployment package repair tool on the Backoffice / Server PC. Bridge `health 503` means Riverside has no token configured; bridge `health 401` means the token in `C:\counterpoint-bridge\.env` does not match Riverside.
