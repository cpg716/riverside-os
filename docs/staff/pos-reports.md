...
**Reports** answers **“How is this drawer doing right now?”** — tender buckets, expected vs actual cash context, and mid-shift checks. It is **not** **Back Office → Insights**, which builds custom store-wide reports from plain-language requests (see [insights-back-office.md](insights-back-office.md)).
...
2. POS → **Reports**.
3. View **Daily Sales** to see a chronological timeline of activity. Use **Booked** when comparing what was rung during the drawer/session, and **Completed** when reviewing recognized takeaway and pickup activity. Search covers the full selected date range by customer, contact detail, Customer #, Transaction number, tender, item, or SKU. The screen states the exact activity-record match count and offers **Load more audited activity** when another page exists. An activity record may be a sale, payment, or another audited event; do not read that count as unique transactions. If one checkout funds both a new sale and an existing order, Daily Sales and the Z-Report show one checkout record: its merchandise lines remain the sale, and a **Payment on Order** section names each public Order number, allocated amount, and remaining order balance. **Total Paid Today** and the tender lines show the full amount collected once. A payment-only checkout remains its own **Payment on Order** activity and does not claim that merchandise was taken. The public target Order number is used whenever one exists, falling back to the ROS Transaction number only when no Order number is available. Daily Sales also lists **Pickups Today** separately with the customer, Transaction Record, and picked-up items so pickup work is visible without counting it as a new sale. **Check numbers** are displayed next to the payment method for all check transactions.

Fulfillment is written for staff as **Takeaway**, **Special Order**, **Custom Order**, **Wedding Order**, **Layaway**, or **Pickup** on screen and in CSV output. Do not use internal underscore values as operating labels.

When staff change merchandise on an existing open Transaction, Booked Daily Sales groups the same-day added, changed, and cancelled lines into one **Order Edited** activity. This also applies when the original Transaction came from Counterpoint: the original imported sale is not counted again, but today's ROS edit is. **Salesperson** remains the order's primary salesperson, while **Edited by** identifies the staff member who made the change. A no-net-change swap shows **$0.00** for the edit and never says money was paid unless a tender actually moved.
4. Review the Daily quick-look boxes for the operating totals: Transactions, subtotal before tax, tax, cash, Credit Card Total, deposits, new orders, picked-up orders, RMS payments, RMS charge, merchandise subtotal, new appointments, new layaways, picked-up dollars, and discounts. **Subtotal before tax** and **Net Sales** include alteration charges while the separate **Alterations** box keeps them visible for reconciliation; shipping remains outside those subtotals. Dollar boxes that summarize grouped work include the count in parentheses.
5. For payment-only activity, use **Receipt** to reprint the payment-event receipt from that business day. It shows the payment receipt Transaction number plus the public Order or Transaction number that received the money, amount paid, customer, Customer #, phone, totals, and remaining balance; it does not rebuild the target order's lifetime purchase receipt. Internal allocation keys are never customer receipt identifiers. RMS Charge payments show as **RMS Payment** activity, not retail merchandise.
...
4. The final Z-report includes cash/check review, all supported tender methods, a Credit Card Total that includes CC/Card Reader, Card Present, Card Manual, Card Not Present, saved-card, and signed card-credit/refund entries, per-register Cash Total / CC Total / Checks Total, Cash Deposit, Checks for Deposit, combined Total Deposit, the Quick Look business boxes, transaction detail, pickups for the day, and the QBO journal-entry preview. Card refunds reduce the Credit Card Total rather than being omitted. Hosted HelcimPay.js entries are labeled **Card Not Present**; only externally recorded/manual card entries are labeled **Card Manual**. Operational close warnings remain available in their audited workspaces and are not printed on the financial Z-Report. It does not include non-sale inventory activity.
...
| Card or recovery warning remains at close | Use **Review** in Z-close or **POS → Payments** and record the terminal outcome; ordinary close remains available | Manager checks Back Office Payments → Health and verifies the issue remains open there; operational issues do not print on the financial Z-Report |
