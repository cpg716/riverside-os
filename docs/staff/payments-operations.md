...
- **Saved Card** charges a Helcim-saved card token for the selected customer. The request is tied to the open sale, and ROS reuses the same Helcim idempotency key if the response is interrupted, so retrying does not intentionally create a second charge. ROS only applies the saved-card payment after Helcim returns an approved result for the requested amount. ROS shows masked card details when Helcim returns them, but staff should never copy or expose the token. If the result is unresolved, keep the sale open and retry the Saved Card action for that same sale; do not add a manual payment unless Payments Health confirms that Helcim did not charge the card.
...
The **Payment Status** panel shows whether a terminal transaction was sent, is waiting for approval, approved, declined, canceled, expired, or returned an error code. If the terminal approved but ROS has not attached the payment yet, use **Recover payment** before retrying the card or changing tender. An approved provider payment stays tied to the customer and checkout that started it; Riverside will not carry it into a different customer's sale. If the original sale cannot be completed immediately, use **Clear Sale** to release the local Register, then review the approval in **Payments Health** before running that card again. Clear Sale remains available when provider state is retained even if the cart has no items; it does not delete or dismiss the processor evidence. If the card was truly declined, use **Retry card** to clear the declined attempt and send a new ROS-tracked request to the terminal. If a same-amount card attempt is declined as a possible duplicate and the customer then uses a different card that approves on the terminal, recover the Helcim approval in ROS before completing the sale. Do not complete the sale until the Helcim attempt is approved and appears as an applied payment.
...
Approved card payments are locked to the checkout identity and cannot be removed, parked, or transferred to another customer. Staff may use **Clear Sale** when the local Register must move on, but the provider approval remains visible in Payments Health until it is safely linked, recovered, refunded, or otherwise resolved through an audited workflow. If an approved Helcim charge is missing from an existing customer order, a payment-resolution manager must open **Payments → Health**, select **Recover Order Payment**, enter the exact `TXN-######` target, document the reason, and complete the typed confirmation. ROS validates the live order balance and processor evidence, then records the payment, allocation, updated balance, Helcim match, and audit history together without charging the card again.
