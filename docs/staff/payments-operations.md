...
- **Card Reader** first verifies that Helcim is listening, then sends the sale amount to the selected terminal for tap, insert, or swipe. If the reader is not listening, ROS sends no payment and leaves the sale ready to retry after the reader is awake, signed in, and on its ready screen.
- **Card Not Present** is for phone orders. It opens the public HTTPS ROS handoff page; select **Open Helcim Card Entry** on that page to render the secure HelcimPay.js card form. Keep the payment drawer open and complete the sale only after the approved payment appears in the ledger. If Helcim approves but the page says ROS could not attach it, select **Retry Approval** on the same page; ROS verifies and reuses that approval instead of charging the card again. A decline or closed card-entry window is finalized against the same attempt before ROS says the ledger is ready to retry. If ROS cannot save that outcome, use **Recover payment** and do not start another card attempt. Each handoff is bound to one checkout, customer, amount, and provider attempt. Completing or clearing that sale resets the drawer; an old approval is rejected from the next sale and remains in Payments Health for audited recovery or refund. Do not type card numbers or CVV into ROS notes, references, search fields, or support chats.
- **Saved Card** charges the selected customer's Helcim-saved card. The browser receives only masked card details and opaque customer/card ids; ROS resolves the provider token on the server. The request is tied to the open sale, and any automatic replay uses the same Helcim idempotency key and payload only during ROS's short safe-retry period. If the result is unresolved, keep the sale open and use **Recover payment**; after the safe-retry period, Payments Health must reconcile the exact provider result. Do not select another tender or start a new card request until ROS confirms a definitive result.
...
Terminal selection is in the checkout drawer header. Register #1 defaults to **Terminal 1**, Register #2 defaults to **Terminal 2**, and Register #3/#4 must choose an available terminal. A green dot means the selected terminal is configured and unreserved in ROS; it is not a live reader heartbeat. ROS checks that Helcim is listening immediately when staff send the payment. A red dot means configuration, routing, or ROS terminal availability needs attention.

The **Payment Status** panel shows whether a terminal transaction was accepted for delivery, is waiting for approval, approved, declined, canceled, expired, or returned an error code. **Accepted does not mean the sale reached the reader or was approved**: confirm the amount appears on the physical reader. If the terminal approved but ROS has not attached the payment yet—or a timeout/5xx left the result unknown—use **Recover payment**. While that outcome is unresolved, Riverside blocks another card request, alternate tender, sale completion, and local-only clearing. A live pending attempt that reached a terminal or secure card entry cannot be released locally; **Clear Sale** stays unavailable until ROS attaches the approval or recovers a definitive decline/cancel. An abandoned Card Not Present setup with no Helcim checkout token, payment ID, transaction ID, or processor event is different: ROS marks it failed after the initialization window and permits a retry because card entry never opened and no charge was possible. An approved provider payment stays tied to the customer and checkout that started it and cannot carry into a different customer's sale. If the card was definitively declined, use **Retry card** to clear the declined attempt and send a new ROS-tracked request. Do not complete the sale until the Helcim attempt is approved and appears as an applied payment.

A normal newly accepted card request appears as **Waiting for card** or **Checking** in Payment Status; it is not labeled as Checkout Recovery. If no final outcome arrives after 15 seconds, ROS emphasizes **Card outcome review** without clearing or duplicating the request. Before reporting a terminal as in use, the Main Hub refreshes the existing pending attempt and releases the route only after Helcim supplies a final result. If staff confirm cancellation on the physical terminal, ROS releases the old request and unlocks alternate tenders, but staff must still verify the reader has returned to its ready screen before retrying Card Reader.
...
When a new checkout finds an earlier request on its terminal, ROS refreshes that exact attempt without importing its payment into the new sale. A verified final result releases the route immediately. If the physical request ended without approval because it was canceled, timed out, or the reader restarted, confirm the reader is idle and use **Reader stopped / no approval** directly in Payment Status or Restore. ROS checks Helcim again, releases the blocked attempt, retains it for reconciliation, and unlocks Card Reader plus every other allowed tender.
