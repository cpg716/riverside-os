...
Payments Operations does not start standalone Helcim refunds or reverses. Open the original ROS Transaction Record and use its authorized return/refund workflow so provider money movement, returned merchandise, the negative payment, and the refund queue remain one auditable action. Staff never type Helcim invoice, payment, or transaction IDs into ROS.
...
- **Card Reader** sends the sale amount once, directly to the Register's assigned terminal for tap, insert, or swipe. ROS does not run the Settings diagnostic ping immediately before a live purchase. If Helcim returns **Device Not Listening**, that purchase request was rejected; wake or sign in to the reader and retry from the still-open sale. If one card is declined and the Helcim reader displays **Try Again**, leave the ROS checkout open and let the customer try again on the reader. ROS keeps that exact invoice pending and records a later unique approval; do not select Card Reader again or enter Manual Card.
- **Card Not Present** is for phone orders. It opens the public HTTPS ROS handoff page; select **Open Helcim Card Entry** on that page to render the secure HelcimPay.js card form. Keep the payment drawer open and complete the sale only after the approved payment appears in the ledger. If Helcim approves but the page says ROS could not attach it, select **Retry Approval** on the same page; ROS verifies and reuses that approval instead of charging the card again. A decline or closed card-entry window is finalized against the same attempt before ROS says the current checkout is ready to retry. If ROS cannot save that outcome, use **Recover payment** for that checkout. Each handoff is bound to one checkout, customer, amount, and provider attempt. Completing or clearing that sale resets the drawer; an old approval is rejected from the next sale and remains in Payments Health for audited recovery or refund without locking later sales. Do not type card numbers or CVV into ROS notes, references, search fields, or support chats.
- **Saved Card** charges the selected customer's Helcim-saved card. The browser receives only masked card details and opaque customer/card ids; ROS resolves the provider token on the server. The request is tied to the open sale, and any automatic replay uses the same Helcim idempotency key and payload only during ROS's short safe-retry period. If the result is unresolved, keep that checkout open and use **Recover payment**; after the safe-retry period, Payments Health reconciles the exact provider result. The unresolved history does not disable another permitted tender or a later sale.
- **Card Refund** appears only inside the authorized return/refund flow for a ROS Transaction Record with an original linked Helcim payment. Staff do not enter Helcim invoice, payment, or transaction IDs. ROS chooses the supported Reverse/Refund action from the original processor record. If Helcim requires the original debit card at a terminal, complete that action in Helcim and use the Manager Access external-refund recording step; do not use a standalone ROS provider route.
...
- These are different tenders everywhere in ROS: **Card Not Present** must show a Helcim approval, while **Manual Card** must show the external approval/reference entered by staff. Sale Complete, receipts, Transaction History, Payments Health, reports, and refunds must keep those labels and evidence separate.

Terminal selection is in the checkout drawer header. Riverside assigns one Register to one terminal; a live terminal is not shared between Register checkouts. A green dot means the assigned terminal is configured and unreserved in ROS; it is not a live reader heartbeat. A red dot means configuration, routing, or ROS terminal availability needs attention.

The **Payment Status** panel shows whether the current terminal transaction was accepted for delivery, is waiting for approval, offering **Try Card Again**, approved, declined, canceled, expired, or returned an error code. **Accepted does not mean the sale reached the reader or was approved**: confirm the amount appears on the physical reader. **Try Card Again** means Helcim declined one card but is still processing the same invoice; ROS keeps the sale Not Ready and watches for the final result. If the terminal approved but ROS has not attached the payment yet—or a timeout/5xx left the result unknown—use **Recover payment** for that checkout. Repeating Card Reader on that same checkout returns the existing request instead of sending a second purchase. A confirmed approval remains tied to the customer and checkout that started it and cannot carry into a different customer's sale. Earlier unresolved attempts remain visible in Payments Health, but they do not disable another permitted tender, Clear Sale, or a later checkout. Starting the later Card Reader payment replaces the old ROS routing reservation while retaining the old attempt's provider evidence for reconciliation.

A normal newly accepted card request appears as **Waiting for card** or **Checking** in Payment Status; it is not labeled as Checkout Recovery. If no final outcome arrives after 15 seconds, ROS emphasizes **Card outcome review** without duplicating the request. Staff should recover the current checkout when practical. If that checkout is abandoned, its attempt remains Payments Health evidence rather than becoming a persistent Register lock.

Before ROS sends Card Reader, Card Not Present, or Saved Card, it compares the current Register build identity with the Main Hub. If they differ, ROS sends nothing to Helcim and names both build prefixes. Update or restart the Register, confirm the Main Hub is on the package's exact SHA, and retry from the open sale.
...
| Terminal decline with Try Again | Sale remains Not Ready while ROS watches the same invoice; the customer's later unique approval attaches without Manual Card. | |
| Final terminal decline | Sale remains unpaid after the reader ends the request; cashier can then retry or choose another tender without duplicate payment. | |
...
Webhook receipt alone does not record a payment in ROS. Normal provider history and Manager-approved external card entries are not payment issues. Live pending attempts do not expire locally; legacy `expired` or unknown attempts remain unresolved until recovery proves a final provider result. Staff review is required whenever processor evidence may represent money movement that ROS has not safely attached or dismissed.

Unlinked Helcim approvals are reported during register close and in Payments Health so staff can repair the link or add a review note. A review note records what staff checked; it does not hide or financially resolve an approval that is still unlinked. These items do not block closing the register or printing the final Z-Report.
...
Riverside uses one-to-one Register/terminal assignments:

- **Register #1** defaults to **Terminal 1**.
- **Register #2** defaults to **Terminal 2**.
- A terminal is never shared by two live Register checkouts.
...
If a new checkout finds an earlier ROS request on its assigned terminal, starting Card Reader releases that earlier routing reservation and retains the attempt in Payments Health. ROS never imports the earlier payment into the new sale. A verified approval remains recoverable only to its original checkout; it is not discarded, moved, or used as the new customer's tender.
...
Standalone refunds and reverses are not available from **POS → Payments**. Start from the original Transaction Record so ROS verifies the remaining refundable amount and Helcim batch state, chooses a full open-batch reverse or closed-batch/partial refund, and commits the provider approval with the negative sales ledger action.

If Helcim definitively declines the refund or ROS blocks it before dispatch, ROS keeps the refund state unchanged and records the failed provider attempt for review. A network/5xx response that may have reached Helcim stays unresolved. ROS may replay the same request/key only during its short safe-retry period; after that, use Payments Health for exact reconciliation and do not submit another refund.
