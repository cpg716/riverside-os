...
6. **Wrong size after adding?** Tap the **product name** on the line (not only the SKU chip). If that style has multiple sizes/options, the same **variation picker** opens again so you can **swap** the line without removing it. Use **Back** or tap a completed choice to correct an earlier selection before confirming the price. If there is only one SKU, a **review / price** panel may open instead.
...
13. **Manager PIN Override**: If a discount exceeds your role's limit (e.g., >10%) or you attempt to **Void All** lines, the **Manager PIN** modal will appear. Have a manager select their name and enter their PIN; this authorizes the action for the current sale without changing your login.
14. Confirm the **line total** matches what you told the customer.
