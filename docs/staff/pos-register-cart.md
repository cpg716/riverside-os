...
8. **Recalling a Transaction:** If a customer has items to pick up or a previous transaction to resume, tap **Transactions** next to the customer search bar. This opens **Customer Transactions**. Payments apply to the existing **Transaction Record (TRX)**; copying unfulfilled items starts a new register sale and does not pay the original record.
