...
5. **Fulfillment choice:** Use **Take Now** when the customer is leaving with the exact item. Use **Pick Up Later** only after attaching the customer and only when Riverside is holding that exact in-stock variation; it becomes Ready for Pickup under Orders and reduces available stock without removing the item from on-hand stock. Use **Order** when the item still needs vendor or wedding fulfillment. If Pick Up Later says the item is unavailable, do not force it—use Order instead.
...
**Fulfillment Orders:** Pick Up Later, Special, Custom, and Wedding lines are **Fulfillment Orders**. **Pick Up Later** reserves an exact on-hand unit immediately and starts Ready for Pickup; Special, Custom, and Wedding lines normally wait for their lifecycle and receiving work. **Takeaway** items decrement stock at sale time. **Layaway** is separate from Orders and remains its own workflow.
