...
8. **Reviewing Open Orders:** If a customer has items to pick up or an existing order balance to pay, tap **Orders** next to the customer search bar. This opens **Customer Orders**. Payments apply to the existing order; copying unfulfilled items starts a new register sale and does not pay the original order.
