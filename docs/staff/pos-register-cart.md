...
- **It stays** if you switch to another **POS sidebar** tab (Dashboard, Inventory, Settings, etc.), **Exit POS mode** to Back Office, or refresh — until you **clear the sale**, or the saved snapshot does not match the current register session (e.g. different lane/session). Closing the Register, including automatic close after inactivity, immediately removes the local draft. Clearing or completing the sale also resets **Cashier for this sale** while leaving the Register session open.
...
1. **Scan** a barcode, choose **Camera** beside product search, or **type** SKU or keywords in the search field. Camera scanning uses the device's rear camera when available and stays open for additional items. Exact SKUs, product barcodes, and active barcode aliases add the verified variation directly; close the scanner to review an unknown, name-only, or ambiguous match in product search. Typed suggestions begin after a short pause; press **Enter** when you want an immediate lookup.
2. **Single match:** the line may drop in automatically.
3. **Multiple matches:** a **product / variation picker** opens. **Item to Build** stays at the top with the product and every completed choice. Select the correct **size / color / style**. Text options are alphabetical, numeric options use natural number order, and apparel aliases follow merchandise order such as **SMALL**, **MED**, **LG**, **XL**, and **2XL**. Use **Back** to return one step, or tap a completed choice under **Item to Build** to edit from that point. On the pricing review, enter an approved **Line discount %** when needed and verify the unit price before **Add to Sale**. The price appears once until it is adjusted; then **Regular unit price** and **Final unit price** appear together for comparison. **Line pricing** is attached directly above the full-size PIN pad as one calculator surface. The completed options, calculator, **Back**, and **Add to Sale** remain together in one fixed screen with no scrolling or repeated product summary. At the first step, **Back** returns to the Cart without adding an item.
...
5. **Fulfillment choice:** Use **Take Now** when the customer is leaving with the exact item. Use **Pick Up Later** only after attaching the customer and only when Riverside is holding that exact in-stock variation. It is fully sold, removed from inventory, recognized, and commissioned that sale day, while remaining Ready for Pickup under Orders until the customer collects it. Use **Order** when the item still needs vendor or wedding fulfillment. If Pick Up Later says the item is unavailable, do not force it—use Order instead.
6. **Custom Orders:** One of the three primary fulfillment types. The known Custom garment SKUs automatically book as **Custom** orders:
   - `100` HSM Custom Suit
   - `105` HSM Custom Sport Coat
   - `110` HSM Custom Slacks
   - `200` Individualized Custom Shirt
   When one of these SKUs is added, the **Custom Order** window opens so you can confirm the garment type, enter the **Sale Price**, add **Need By Date** or **Rush** details, and capture the main vendor-form references. In the cart, these lines automatically default to **ORDER (Custom)** fulfillment kind, stay custom when toggled or shipped (retaining their custom status instead of defaulting to special orders), and display with a prominent yellow **ORDER (Custom)** badge. **Vendor cost is not entered at booking**. The actual cost is entered later, when the garment is received. These items stay tracked as **Fulfillment Orders**.
7. **Wrong size after adding?** Tap the **product name** on the line (not only the SKU chip). If that style has multiple sizes/options, the same **variation picker** opens again so you can **swap** the line without removing it. Use **Back** or tap a completed choice to correct an earlier selection before confirming the price. If there is only one SKU, a **review / price** panel may open instead.
8. **Cart Item Toggles:** Each line in the cart includes **Take Now**, **Pick Up Later**, the applicable **Order** choice, and **Gift Wrap** (blue wrapping icon). Tapping one updates the fulfillment requirement immediately.
9. **Line Tax Type:** Merchandise lines show a small **Standard**, **Clothing**, or **Footwear** tax pill when applicable. Tap it only to correct the tax type for this sale when the item was misclassified and the customer is waiting. The correction is saved to the Transaction Record audit for that line; it does not change the inventory product setup.
   The action grid order is **Customer Orders**, **Wedding Manager**, **Return / Exchange**, **Start Alteration**, **More Actions**, then **Clear Sale**. The selected Customer and Wedding stay in the right rail instead of being repeated above the Cart. Staff, Salesperson, and Store Date/Time remain locked in the top strip. Choose **More Actions** for Custom Order, Gift Card loads, RMS or Staff Account payments, Layaway, Order options, **Resume Parked Sale**, and Park Sale. The nine actions use a three-by-three grid. Selecting one closes More before the chosen workflow opens, so closing that workflow returns directly to the Cart.
   On a portrait tablet or phone, choose **Customer & Pay** at the bottom to open the Customer, totals, keypad, and Pay workspace. Choose **Back to cart** to return to scanning and line review. Desktop workstations keep both areas visible together.
   Choose **More Actions → Resume Parked Sale** to see unfinished sales on the current Register. Review the Customer, draft reference, item count, and subtotal, then choose **Resume Sale**, delete the parked sale, or close the popup.
10. **Reviewing Open Orders:** If a customer has Pick Up Later, Special, Custom, or Wedding items to pick up or an existing balance to pay, tap **Orders** next to the customer search bar. This opens **Customer Orders** for unfulfilled order work. Payments apply to the linked Transaction Record. For pickup, select the lines leaving with the customer; ROS loads those pickup lines into the register cart so staff can add new purchases before tapping **Complete Pickup**.
11. **Cancelling unfulfilled Order items:** Open **Customer Orders** from the Register toolbar, choose **View Order Details**, and select **Cancel Item** directly below **Update Item**. Enter the reason and select **Review Balance & Refund**. Riverside applies the item credit to the remaining Order balance first; only payment above the revised total becomes a refund. Received or Ready-for-Pickup goods stay on hand while their customer reservation is released. If a refund exists, Riverside moves it to Pay for multi-source settlement. Do not load unfulfilled items through Return / Exchange.
12. **Fulfillment Requirements (Rush/Due Date):** Tap the **Zap (Options)** icon in the tool rail to open the **Transaction Review** screen. Here you can set the **Need By Date** and toggle **Rush** status for the whole sale. Use the separate **Ship current sale** action in the cart when the customer wants delivery; that flow captures the address, carrier rate or manual shipping charge, and shipment tracking together.
13. **Quantity and price:** Use the on-screen **numpad**.
...
14. **Manager PIN Override**: If a discount exceeds your role's limit (e.g., >10%) or you attempt to **Void All** lines, the **Manager PIN** modal will appear. Have a manager select their name and enter their PIN; this authorizes the action for the current sale without changing your login.
15. Confirm the **line total** matches what you told the customer.
...
**Staff Account purchases and payments:** Staff Account is for employees whose Staff profile is linked to their personal customer profile. For an employee purchase, attach that linked customer and choose **Staff Account** in checkout; the merchandise still follows normal tax rules, including clothing/footwear exemption rules and any taxable standard items. To pay down an existing employee Staff Account balance, use **Staff Pay** in the register toolbar. That adds a **STAFF ACCOUNT PAYMENT** line, which is a balance payment only: no new merchandise revenue, no new sales tax, and no quantity/discount changes.

**Fulfillment Orders:** Pick Up Later, Special, Custom, and Wedding lines are **Fulfillment Orders**. **Pick Up Later** fully sells and removes the exact on-hand unit from inventory immediately, then starts Ready for Pickup as custody work; Special, Custom, and Wedding lines normally wait for their lifecycle and receiving work. **Take Now** also decrements stock at sale time. **Layaway** is separate from Orders and remains its own workflow.

**Shipping without an order:** A customer can buy an in-stock item at the Register and have it shipped without turning the merchandise line into a Special/Custom/Wedding fulfillment order. Use **Ship current sale** before payment. Riverside records the Transaction Record delivery method as **Ship**, stores the address and shipping fee from the carrier rate or manual shipping charge, and creates the shipment record for follow-up. Manual shipping charges post as customer-charged shipping, not merchandise. This is separate from shipping existing open order work from the Orders/Shipments workflow.

**Shipping fee for existing orders:** To charge delivery for open order work, select the customer, open **Ship current sale**, enter or rate-shop the shipping charge, then select the matching Transaction Record(s) under **Link existing orders**. The Register can complete this as a shipping-only transaction. The shipping fee posts to shipping income, while the original Transaction Records remain the merchandise/fulfillment records.
...
2. If the search has no match, use **Add Customer** at the top of the results. The Add Customer drawer is the same intake used in Back Office and will pre-fill the name, phone, or email you typed.
   - After the drawer opens, the register search results close and stay behind the customer form. Complete or cancel the customer form before returning to register search.
...
If the loaded customer has no email, Register opens **Please add customer email**. Ask for the address and choose **Save email**. Choose **Skip** when the customer is not ready to answer; the prompt may appear the next time that account is loaded. Choose **Customer declined** only after an explicit refusal, because that choice permanently suppresses the prompt for the account. Saving an address does not grant marketing consent. Riverside attributes saved emails to the signed-in staff member in Back Office → Reports → **Email Collection Report**.

When adding or completing a customer profile at the register, the **Address** field searches U.S. street addresses after you start typing and ranks Western New York matches first. Selecting a suggestion immediately fills **Address**, **Address line 2** when available, **City**, **State**, and **ZIP**. Review or edit any field manually as needed. Customer address entry uses Geoapify directly and does not depend on Shippo.
...
4. **Integrated card payments:**
   ...
   - While the customer inserts or taps a card and completes PIN prompts, the checkout stays **Waiting for Card** and the final action stays **Not Ready**. Riverside allows about two minutes before showing recovery guidance and never enables **Ready to Save** until the current checkout has a confirmed approval, decline, cancellation, or expiry.
   - If Helcim has already approved a simple take-now sale and the Main Hub connection drops before the save completes, select the green **Ready to Save** box once. ROS saves the exact sale and approval locally, prints a **PAYMENT APPROVED — PENDING SYNC** receipt, and posts the same sale automatically when the Main Hub reconnects. Do not run the card again. Shipping, pickups, orders, exchanges, alterations, and other fulfillment workflows must remain open for live recovery because they require additional Main Hub actions.
   - **Card Refund** appears only when ROS already has the original Helcim payment reference for the refund. For a refund-only return, choose **Original Card** and **Apply Refund**; ROS sends the Helcim refund immediately and keeps **Record Sale** unavailable until the approved provider refund ID and committed ROS refund-ledger event are verified. Exchange remainders stay staged until the replacement Transaction and inventory linkage are recorded, then use the same verified processor. Staff do not enter Helcim invoice, provider, or transaction IDs.
   ...
   - Refunds and exchange remainders may use multiple payment methods just like any other Transaction. Add each refund source and amount until the Payment Status is **Ready**. ROS records the sources under one return event; if a processor source succeeds and a later source fails, retrying resumes the unfinished source without duplicating the completed one.
   - An exchange can apply its return credit to replacement merchandise, a payment on an existing Transaction Record, or both. A Transaction payment added after return lines stays in the cart and posts through the normal payment allocation and reporting path.
...
5. **Audited Tax Exemptions:**
   ...
   - **Reason Recorded**: Select the applicable reason (e.g. Resale, Exempt Org). The selected reason is recorded on the Transaction Record; the optional note can hold a certificate, ID, or other reference.
   - Taxes will be struck through, and the **Balance Due** will update automatically. Single-line tax badges can still be changed independently in the cart; the server recognizes the selected line status and recalculates that line's tax without blocking a valid sale because of a stale client tax value.
...
7. **Order deposit:** On an Order, Custom, Wedding, or Layaway Transaction, Payment shows a compact status strip: green **DEPOSIT MET**, red **NEED 25% DEPOSIT**, or red **NO DEPOSIT**. For **NO DEPOSIT** or another below-minimum exception, use the inline **Manager Access** action. That approval is limited to the exact checkout and creates no fake payment. Every payment that leaves a balance is called and recorded as a **Deposit**; a payment that closes the balance is **Payment in Full**. Any takeaway merchandise must still be paid in full. For one payer funding several wedding members, use **Wedding Deposit** from the Register toolbar and select only the intended beneficiaries.
8. **Combined New Purchases & Pickups**: Cashiers can process a new takeaway purchase and release/pickup an existing order, layaway, or alteration (even with outstanding balances) in a single register flow:
   - **Load the Pickup**: Attach the customer, click **Orders**, select the Transaction Record, choose only the lines being picked up, and tap **Continue with Pickup**. When money remains due, **Pay at Pickup?** shows the exact balance. Choose **Pay Balance Now** to stage the balance and pickup together before tender, or **Skip Payment for Now** to leave it open. Customer Orders then closes and returns you to the Cart with the selected items (flagged with their original transaction line IDs).
   ...
   - **Payment or Manager Override**: **Pay Balance Now** continues into the normal tender workflow as part of the pickup. If staff choose **Skip Payment for Now** and existing payments do not cover the selected items plus merchandise already released, Complete Pickup requires the audited **Manager Access** override. If coverage is sufficient, the pickup can finish without a new tender.
   - **Complete Pickup / Receipt**: Use **Complete Pickup** or finish payment. The Sale Complete screen opens so staff can print, text, or email the pickup receipt showing the picked-up items. If every item is picked up but money remains due, the physical pickup is recorded while the Transaction Record stays **Open** and payable until its balance reaches `$0.00`; it must not disappear as Fulfilled.
   - **Behind the Scenes**: To maintain clean financial ledgers, tax compliance, and salesperson commission rules, the system splits this into two actions automatically upon checkout:
     - A **new Transaction Record** is created only for today's takeaway items and any payment staff explicitly added.
     - A **fulfillment update** is executed on the *original* transaction to mark the loaded items as fulfilled/picked up.
9. **Customer identity on receipts:** When a customer is attached, printed, emailed, and text receipts show the customer’s full name, phone, and Customer #. Confirm the attached customer before completing the sale so the receipt identity is correct.
10. **Store date and time** next to **Salesperson** is a live clock in the store’s receipt timezone. If a sale must be recorded under an earlier store-local date/time, select the date/time and complete the **Manager Access** approval prompt. The label changes to **Backdated sale**. This applies only to the current sale and resets after checkout, cart clear, or customer change. The transaction business date is backdated, but every tender remains on its actual processing day for drawer, card-batch, deposit, and QBO reconciliation. The receipt is marked **BACKDATED SALE**; the normal printed receipt timestamp remains the time the server records checkout.
11. **Receipt:** after tender completes, a **receipt** screen opens on top of the cart — **Print receipt** uses the Epson receipt printer configured in **Printers & Scanners**, and **Email receipt** / **Text receipt** use the standard receipt content. Receipts list each item with the product name, variation/size description when present, SKU, quantity, and price. Gift card payments also show the masked card number and the card balance **after that payment**. Register #1 opens the attached cash drawer only for **CASH** and **CHECK** sales. You can view or edit **phone** and **email** on that screen and **save** them to the customer record when allowed. Close that screen when done; only then does the register treat the sale as fully finished for “next customer” flow. Offer **bag tag** / label prints if your store uses them.

**RMS Charge (private label credit on a sale):** Use the single **RMS CHARGE** payment button, not separate RMS / RMS90 buttons. After selecting it, POS checks the linked RMS Charge account, shows only **masked** account choices, and places the required program selection first. Every RMS Charge customer always has both **Standard RMS** and **RMS 90** available; the cashier must select the program confirmed in R2S for that sale. The selection records the matching tender code for distinct QBO reporting. Enter the approval/reference, then complete R2S follow-up. The receipt prints RMS Charge, saved plan label, masked account, and reference number when available. For the quick cashier workflow, see **[POS RMS Charge](pos-rms-charge.md)**.
...
| **Complete Sale** disabled | Missing customer, missing Salesperson on sale lines, zero-total line, open modal, or an Order is below its 25% deposit minimum | Read the hint; add payment, or use **Override Deposit** with Manager Access when store policy permits |
