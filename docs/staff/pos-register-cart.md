...
8. **Combined New Purchases & Pickups**: Cashiers can process a new takeaway purchase and release/pickup an existing order, layaway, or alteration (even with outstanding balances) in a single register flow:
   - **Load the Pickup/Payment**: Attach the customer, click **Orders**, select the transaction, and choose the lines being picked up. This populates the cart with the selected pickup items (flagged with their original transaction line IDs) and adds an **Order Payment** line for any remaining balance.
   ...
   - **Collect Payment**: Settle the total balance due (covering both the new takeaway purchase and the historical order balance). If the selected pickup was already fully paid, checkout recognizes the prior paid/deposit amount and stays at **$0.00 Balance Due** without requiring a new tender.
   ...
   - **Behind the Scenes**: To maintain clean financial ledgers, tax compliance, and salesperson commission rules, the system splits this into two actions automatically upon checkout:
     - A **new Transaction Record** is created for today's takeaway items and the order balance payment.
...
**RMS Charge (private label credit on a sale):** Use the single **RMS CHARGE** payment button, not separate RMS / RMS90 buttons. After selecting it, POS checks the linked RMS Charge account, shows only **masked** account choices, and then opens a required plan-selection step. The system does **not** silently default the plan for the cashier. This is the current manual RMS workflow: confirm the account/program outside ROS, enter the reference number when available, and complete R2S follow-up. The receipt prints RMS Charge, saved plan label, masked account, and reference number when available. For the quick cashier workflow, see **[POS RMS Charge](pos-rms-charge.md)**.
