...
8. **Line Tax Type:** Merchandise lines show a small **Standard**, **Clothing**, or **Footwear** tax pill when applicable. Tap it only to correct the tax type for this sale when the item was misclassified and the customer is waiting. The correction is saved to the Transaction Record audit for that line; it does not change the inventory product setup.
   The action grid keeps the five most common workflows visible, with **Customer Orders** first. Choose **More Actions** for Custom Order, Gift Card loads, RMS or Staff Account payments, Layaway, Order options, Park Sale, and Clear Sale. Selecting one closes More before the chosen workflow opens, so closing that workflow returns directly to the Cart.
...
8. **Combined New Purchases & Pickups**: Cashiers can process a new takeaway purchase and release/pickup an existing order, layaway, or alteration (even with outstanding balances) in a single register flow:
   - **Load the Pickup**: Attach the customer, click **Orders**, select the transaction, choose only the lines being picked up, and tap **Continue with Pickup**. Customer Orders does not add the historical balance automatically. When money will remain due after staged payments, choose **Go Back / Add Payment** to collect it now or explicitly choose **Pick Up Without Payment**. Customer Orders then closes and returns you to the Cart with the selected items (flagged with their original transaction line IDs).
   ...
   - **Payment or Manager Override**: Use **Add Payment** only when the customer is intentionally paying the historical order balance. If existing payments do not cover the selected items plus merchandise already released, collect the shortage or use the audited **Manager Access** override. If coverage is sufficient, pickup stays at **$0.00 Balance Due** without requiring a new tender.
