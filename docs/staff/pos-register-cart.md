...
8. **Line Tax Type:** Merchandise lines show a small **Standard**, **Clothing**, or **Footwear** tax pill when applicable. Tap it only to correct the tax type for this sale when the item was misclassified and the customer is waiting. The correction is saved to the Transaction Record audit for that line; it does not change the inventory product setup.
   The action grid keeps the five most common workflows visible. Choose **More Actions** to open the register-tools panel for Customer Orders, Gift Card loads, RMS or Staff Account payments, Layaway, Order options, Park Sale, and Clear Sale; close the panel to return to the compact grid.
...
7. **Order deposit:** On an Order, Custom, Wedding, or Layaway Transaction, every payment that leaves a balance is called and recorded as a **Deposit**. **Record Sale** stays neutral and unavailable until at least 25% of the current Transaction total is paid. If store policy permits less—including `$0.00` down—select **Override Deposit** at the top of Payment and obtain Manager Access. That approval is limited to the exact checkout and creates no fake payment. Any takeaway merchandise must still be paid in full. A payment that closes the balance is shown as **Payment in Full**, not Deposit. For one payer funding several wedding members, use **Wedding Deposit** from the Register toolbar and select only the intended beneficiaries.
