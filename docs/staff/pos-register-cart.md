...
5. **Wrong size after adding?** Tap the **product name** on the line (not only the SKU chip). If that style has multiple sizes/options, the **variation picker** opens again so you can **swap** the line without removing it; if there is only one SKU, a **review / price** panel may open instead.
6. **Quantity and price:** use the on-screen **numpad**; **%** discount and **$** override only if your role allows; if blocked, call a manager for **override**.
...
4. **Special / wedding orders (when shown):** The balance card may include **Deposit release (ledger)**. Use the same keypad, then **Apply deposit** (below **Apply payment**) to record the deposit amount for accounting. **Split deposit (wedding party)** opens wedding lookup in **group pay** mode so one payer can split amounts across members (same as **Wedding** → party → **Enter Group Pay**). If your sale is **order lines only** (no take-home items) and you are **not** using a split wedding payout list, you may be able to tap **Complete Sale** with **only** a deposit set — follow store policy.
