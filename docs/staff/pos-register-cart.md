...
10. **Cancelling unfulfilled Order items:** Open **Orders** from the POS navigation, open the customer's Order, and select **Cancel Order Items** beside the **Items** heading (or **Cancel Ordered Item** beside one eligible line). Review how the selected item credit affects the existing unpaid balance before confirming. Only a payment above the revised Order total becomes a refund; if one exists, Riverside moves it to Pay for multi-source settlement. Do not load unfulfilled items through Return / Exchange.
