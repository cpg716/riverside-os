...
3. Select a row to read **initial value**, **remaining balance**, **status**, **customer link**, and **recent activity**. For Counterpoint-originated cards, the guarded repair restores the source original issue amount rather than treating the remaining balance as the initial value. The detail window stays centered in the current workspace even after the card list has been scrolled.
