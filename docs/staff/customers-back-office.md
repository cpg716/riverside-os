...
4. For mailing address, start typing **Address line 1**. ROS searches only after you edit the field, searches U.S. addresses, and ranks Western New York matches first. Select the correct result to immediately fill **Address line 1**, **Address line 2** when available, **City**, **State**, and **Postal code**. Municipality prefixes such as **Town of** are omitted from City. Review or edit any field manually as needed. Customer address entry uses Geoapify directly and does not depend on Shippo.
