...
5. Ensure the Podium app has all required scopes enabled: `read_locations`, `read_messages`, `write_messages`, `read_phones`, `read_reviews`, `write_reviews`, `read_users`, `read_contacts`, and `write_contacts`. The Podium person approving Riverside must also be an administrator with product access to Messages, Calls, Contacts, and Reviews. Existing connections must use **Reconnect Podium Account** once after scopes or approving-user permissions change. If Podium refuses `read_phones` during authorization, Podium must enable Calls / Read phones for the Riverside developer app.
...
2. Search or use the **Open**, **Needs reply**, **Unread**, or **Closed** filter, then select a customer thread. Linked customer rows show the name, formatted phone number, and Riverside customer number; the open conversation header repeats all three so staff can verify the recipient before replying. Riverside loads the complete cursor-paged provider conversation set before applying Podium's open/closed state, so the Open count matches Podium after a successful pull. The chronological thread opens on the right with messages, Podium call cards, and review cards when a published review is attributable to that customer's Riverside review invitation.
...
9. Use **Open Customer** when the conversation needs profile, Transaction, Fulfillment Order, or wedding follow-up. Use **Open in Podium** to open the exact provider conversation in a new browser tab when Podium has supplied its conversation ID.
...
17. Use **Refresh** to reload the Riverside copy. Open **Status** and use **Pull from Podium** when messages are missing; that action asks Podium for the complete current conversation set and cursor-paged history for new or changed conversations. **ROS webhook receiving** means the local signing and ingest configuration is present. **Processing current** separately confirms accepted deliveries are not stuck or failed. **Pulling history** is normal progress and does not raise the needs-attention warning by itself. **History current** means stored provider histories are marked complete. **Historical gap recorded** shows the exact number of stored histories not marked complete but does not imply live messages are failing. Retry the pull once only when older messages are actually missing; **History pull failed** remains an actionable warning for IT.
...
| **Podium consent page says something went wrong** | Do not retry repeatedly; check whether the Podium app has all scopes enabled (`read_locations`, `read_messages`, `write_messages`, `read_phones`, `read_reviews`, `write_reviews`, `read_users`, `read_contacts`, `write_contacts`) | IT / Podium support |
| **Update Webhook returns `403 Invalid scopes for the given event types`** | Use **Reconnect Podium Account** once so the token receives `read_phones`, then retry the webhook update | If it still fails, Podium must enable Calls / Read phones for the Riverside developer app |
...
| **A known greeting appears under Podium Contact** | Open the customer profile and read the Podium sync status. **Conflict / ambiguous_identity** means more than one customer matches the provider identity | Correct the duplicate phone/email ownership, then sync again; never rename or merge from the greeting alone |
| **A customer surname appears as only an initial after Podium reconciliation** | Open the Riverside customer profile and verify the RMS/Counterpoint identity; do not repeatedly reconcile or manually guess the surname | IT deploys the guarded name-sync repair and reviews its retained audit evidence |
...
| **Podium Inbox shows old conversations, misses an open conversation, or reports a historical gap** | Click **Pull from Podium** once; use **Refresh** only to reload the Riverside copy | IT checks the displayed historical-gap count or pull-failure detail, OAuth scopes, saved provider location, complete provider cursor sync, and the message-history response |
