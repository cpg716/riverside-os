...
| **Bag Tag** | A 2x1 thermal label generated for individual fulfillment items. |
| **Balance due** | Amount still owed on a Transaction Record after payments and credits. |
...
| **Checkout / Complete Sale** | Finalizing tenders so the Transaction Record is paid or partially paid per policy. |
...
| **Deposit (Transaction)** | A partial payment made at checkout for tailored or Special/Custom/Wedding order items. |
| **Discount event** | Time-boxed merchandising discount with eligible SKUs; may apply automatically at POS when rules match. |
| **Disbursement (wedding)** | Splitting one payment across wedding party members’ Transaction Record balances via the checkout payload — see [abstracts/wedding-group-pay.md](abstracts/wedding-group-pay.md). |
...
| **Fulfillment / pickup** | Marking transaction lines or fulfillment-order work as physically given to the customer; may affect stock for **stocked** lines. |
...
| **Need By Date** | The hard customer deadline for Special, Custom, Wedding, or alteration work. |
| **Open Order** | Unfulfilled Special, Custom, or Wedding work that appears in **Orders → Open Orders**. Use **Transaction Records** for the whole sale and payment history. |
| **Open register** | An active till session (**register session**) required for many POS money actions. |
| **Order** | Only unfulfilled **Special**, **Custom**, or **Wedding** fulfillment work. It is not the whole sale. |
...
| **Recalc** | Server recomputation of Transaction Record totals, tax, or balances after a change. |
...
| **Reserved stock** | Units physically in the store but promised to Special, Custom, or Wedding fulfillment work until pickup. |
...
| **Transaction Record** | The complete sale and financial ledger: booked items, takeaways, gift cards, alterations, deposits, payments, refunds, receipts, balances, and linked fulfillment work. Visible numbers start with **TXN-**. |
...
| **Wedding Order** | Wedding-party fulfillment work linked to a Transaction Record and wedding member. |
