# Riverside OS v0.85.0 Printable Deployment Guide
...
4. Configure all integrations (QBO, Helcim, Shippo, Podium, Meilisearch, Constant Contact, Counterpoint, NuORDER, Metabase, ROSIE, and auxiliary APIs).
...
```text
https://github.com/cpg716/riverside-os/releases/tag/v0.85.0
```
...
- [ ] `Riverside.POS_0.85.0_x64_en-US.msi.sig`
- [ ] `latest.json`
- [ ] `riverside-updater-build-manifest.json`
- [ ] `RiversideOS-Deployment-Manager_0.85.0_universal.dmg` (macOS remote management)

Expected release identity:

```text
Version: 0.85.0
Deployment package: RiversideOS-v0.85.0-Windows-Deployment.zip
```
...
- Apply migrations.
- Download the pinned ROSIE LLM model (Gemma E4B, ~5.4 GB) and install the sherpa-onnx Python voice stack (pins Python 3.12 and forces binary wheels).
...
- [ ] Settings -> Updates shows Riverside `0.85.0`.
...
- [ ] Configure the accounts mappings:
  ...
  - [ ] Mapped **Income Gift Card Breakage** and **Liability Gift Card** accounts to capture automated gift card breakage sweeps.
...
- [ ] Create Metabase User Groups:
  ...
  - [ ] Create group **`Reporting - Admin`** (grant view access to both `Staff Approved` and `Admin Financial`).
...
### 4.10 Auxiliary API Configurations
...
### 4.11 ROSIE Local AI Copilot & Voice Stack

ROSIE handles AI Help Center Q&A, documentation searching, and operations analysis.

- [ ] Confirm the Gemma LLM model is downloaded and located in the model folder: `C:\ProgramData\RiversideOS\rosie\gemma-e4b.gguf` (supports multi-user / SYSTEM server pathing).
- [ ] Verify the server `.env` contains `RIVERSIDE_LLAMA_MODEL_PATH=C:\ProgramData\RiversideOS\rosie\gemma-e4b.gguf`.
- [ ] Configure the AI Provider:
  - [ ] **LocalGemma**: Complete privacy local processing.
  - [ ] **GeminiApi**: Connects to the cloud Gemini API.
  - [ ] **Auto** (Recommended): Auto-redirects search and non-sensitive logic to Gemini (if API key is present) and isolates sensitive transactions locally.
- [ ] Open the Help Center search drawer and ask ROSIE a trial question. Confirm response is returned correctly.

### 4.12 Integration Troubleshooting Quick Reference
...
| **Constant Contact** | Sync status failed / oauth error | Verify Client ID and Secret in settings. Check RIVERSIDE_CC_REDIRECT_URI matches. |

### 4.13 Constant Contact Setup

Constant Contact synchronizes marketing-opted-in contacts to selected lists and tracks delivery events.

- [ ] In Back Office, navigate to **Settings -> Integrations -> Constant Contact**.
- [ ] Save the **OAuth Client ID** and **OAuth Client Secret** (values are encrypted using `RIVERSIDE_CREDENTIALS_KEY`).
- [ ] Click **"Authorize Account"** to initiate the OAuth2 authorization code redirect flow.
- [ ] Sign in to Constant Contact, grant permissions, and confirm redirect back to the success window.
- [ ] Configure the target mailing lists:
  - [ ] Select the **Default Contact List** (e.g. "General Newsletter").
  - [ ] Map any conditional segments (e.g., Tag "VIP" -> VIP mailing list; group code "Bridal" -> bridal list).
- [ ] Click **"Save Mapping settings"**.
- [ ] Trigger a manual sync push via **"Sync Marketing Contacts Now"** and check that success logs are recorded at the bottom of the dashboard.
- [ ] Copy the webhook endpoint URL: `http://SERVER-IP:3000/api/settings/constant-contact/webhooks/receive`.
- [ ] Save this URL in the Constant Contact Developer Portal webhooks section to capture email events (sent, bounced, opened, clicked, unsubscribed) on customer profiles.
...
- [ ] Confirm Settings -> Updates shows `0.85.0`.
...
- [ ] Printers & Scanners settings are saved on the server (persisted in `store_settings.pos_station_config`).
- [ ] Test receipt prints.
- [ ] PWA receipt test prints from the configured Epson IP/port.
- [ ] Register #1 receipt reprint path works.
- [ ] Receipt Print Failure Retry Queue is operational (verifies red badge appears in POS cart header on printing error).
...
- [ ] Confirm server starts and Settings -> Updates shows `0.85.0`.
- [ ] Update Register #1 desktop app.
- [ ] Relaunch Register #1 and confirm Settings -> Updates shows `0.85.0`.
...
4. Do not continue production work until all surfaces show `0.85.0`.
...
- **Bug reports** (Developer Errors) — pending bugs with server log snapshots
...
- **Auto-discovery** — scans Tailscale peers and local subnet for ROS servers via high-performance concurrent Rust sweeps (avoiding CORS and browser connection limit limitations)
