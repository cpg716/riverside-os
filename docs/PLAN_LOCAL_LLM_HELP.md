...
**Status:** **Active ROSIE program / product architecture** — architecture + **Help-only** maintainer automation (**nightly** AIDOCS/Playwright manual pipeline, `generate:help`, `ros_help`, and **Orphan Cleanup/Pruning**) ship **fully implemented**; governed ROSIE routes live under **`/api/help/rosie/v1/*`** and workstation availability depends on Settings + Host runtime configuration. Start with **[`AI.md`](AI.md)** for the documentation map. **Last updated:** 2026-04-11.
...
- [x] **Nightly ROSIE maintainer (Help Center only):** Ship an **unattended** job that runs **AIDOCS** + **Playwright** golden paths, **diffs** UI vs committed **`*-manual.md`**, **regenerates** help artifacts (`npm run generate:help`), **reindexes** `ros_help`, and **refreshes** any **help-manual-only** sidecar embeddings when that corpus changed. **IMPLEMENTED:** Added `generate:help:components:cleanup` for recursive orphan trashing and 60-day pruning to prevent drift — see [Authoring new Help manuals](#authoring-new-help-manuals-aidocs-playwright-and-governed-learning). PRs from this job **must** be limited to **`client/src/assets/docs/*-manual.md`**, **`client/src/assets/images/help/**`**, and generated help manifest artifacts — **no** other paths ([§ Absolute rule](#absolute-rule-learn-only-never-mutate-business-data)). **Merge policy** is org-defined; **never** embed raw production PII in commits or embeddings.
...
### Axum BFF (implementation contract)
...
2. **“Guardrails”** — if you need code first, implement **server-side** **`rosie_assistant_config`** (or reuse **`store_settings` JSON**) + **BFF validation** so toggles **cannot** contradict `transaction_checkout` / `inventory` / `weddings` invariants.
