...
| Placeholder model names (e.g. “Gemma 4 E4B”) | Pin an exact **GGUF / model card / license** before implementation. Quant size drives RAM/VRAM. |
...
| `transaction_summary` | `GET /api/transactions/{id}` (and list if scoped) | `orders.view` + composite session rules | Narrow scope; no broad export unless product approves. Use `record_scope=orders` only for Special, Custom, and Wedding fulfillment work. |
...
| **Models** | Gemma 4 E4B, Moonshine, Kokoro | **Pin** exact GGUF / ONNX artifacts + licenses before build; names move fast—validate **April 2026** availability. |
...
- **Brain:** Gemma 4 E4B Instruct (Local GGUF via `llama.cpp` or `mistral.rs`).
