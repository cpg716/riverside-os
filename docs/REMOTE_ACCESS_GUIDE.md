...
Riverside OS runs entirely on your own Windows server PC. It is **not** a cloud service. To reach it from outside the shop network, you need a secure private tunnel — Tailscale provides this with zero open ports on your internet router and no VPN configuration overhead.
...
## 2. One-time server PC setup (IT / owner — done once)

### 2.1 Install Tailscale on the server PC
...
3. The server PC appears in your Tailscale admin console at [login.tailscale.com/admin/machines](https://login.tailscale.com/admin/machines).
4. Note the server PC's **Tailscale IP** (`100.x.x.x`) and optionally its **MagicDNS name** (e.g. `ros-server.tail12345.ts.net`).
...
Run in an elevated PowerShell on the server PC:
...
Add to the server PC's `.env` (or Windows Machine environment):
...
- Replace `100.x.y.z` with the server PC's actual Tailscale IP.
...
For HTTPS without a reverse proxy, use **Tailscale Serve** on the server PC:
...
2. Install and sign in with the **same Tailscale account** as the server PC.
...
- **Rename devices** for clarity (e.g. "ROS Server PC", "Owner iPhone").
- **Set key expiry** — by default device keys expire after 90 days and require re-authentication; disable expiry for always-on machines like the server PC.

### Disabling key expiry on the server PC (recommended)

In the admin console → Machines → click the server PC → **Disable key expiry**. This prevents the server from dropping off the Tailscale network after 90 days without requiring a manual re-login.
...
|---|---|---|
| Cannot reach server over Tailscale | Is Tailscale running and connected on **both** devices? | Check server PC system tray; check remote device Tailscale app |
...
| Server PC dropped off Tailscale network | Key expired (90-day default) | Re-login on server PC or disable key expiry in admin console |
| MagicDNS name not resolving | MagicDNS not enabled in Tailscale admin | Admin console → DNS → Enable MagicDNS |
| HTTPS cert warning | Not using Tailscale Serve | Set up `tailscale serve` on the server PC (section 2.4) |
...
|---|---|
| Server PC Tailscale IP | Tailscale admin console → Machines, or hover tray icon |
| Server PC MagicDNS name | Admin console → Machines → click PC |
...
*Last updated: 2026-05-28. Reflects Riverside OS v0.80.9 in-app Tailscale address saver and connection failure hint.*
