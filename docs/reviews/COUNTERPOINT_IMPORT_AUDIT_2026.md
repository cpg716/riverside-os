...
4. **Counterpoint SQL Customization**: (April 9 Update) Final field mapping confirmed that some Counterpoint environments use local table names (**`SY_GFC`**, **`AR_LOY_PT_ADJ_HIST`**) instead of the documented standard. Additionally, the **`SY_STC`** table was located, enabling the activation of the Store Credit opening balance migration. The bridge has been tuned to support these overrides.
