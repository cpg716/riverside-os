...
**Policy pack:** `rosie-policy-pack-0.96.0` · **Last reviewed:** 2026-08-02. The v0.96.0 release changes staff wedding-order guidance and Help grounding, not the reporting route allowlist or financial basis rules in this catalog.
...
**Primary staff analytics UI (2026):** **Back Office → Insights** is a native ROSIE + Cube Core report builder. It supports governed natural-language reports, follow-up refinement, charts/tables, new periods, CSV, print, favorites, and automatically retained history. **`GET /api/insights/*`** remains for operational reads and the fixed **Back Office → Reports** library. Cube models human-readable **`reporting.*`** views and connects through **`cube_ro`**, which has no application-table access. The Rust gateway—not the model or browser—allowlists members and enforces **insights.view** plus Admin-only cost/margin. See **[`CUBE_INSIGHTS_REPORTING.md`](CUBE_INSIGHTS_REPORTING.md)**.
...
**Canonical router map:** [`server/src/api/mod.rs`](../server/src/api/mod.rs) (`build_router`). **§0** lists **staff-facing read nests** used for reporting, CRM depth, native Insights, **Curated Reports**, and **in-app Help** ( **`/api/help/*`** ). It is a **maintained checklist**, not a generated OpenAPI dump — validate with **`python3 scripts/scan_axum_get_routes_hint.py`** and `build_router` when adding routes. **POST**-only business actions (finalize commission, QBO sync, checkout, etc.) remain out of scope for passive NL reads; the native Insights POST routes are read-only report execution and saved-definition lifecycle endpoints.
...
| Method | Path | Permission / notes |
|--------|------|---------------------|
| POST | `/api/insights/reports/ask` | **`insights.view`**. Natural-language question plus optional previous validated spec; creates, validates, runs, and records a read-only Cube report. Cost and margin require Riverside Admin. |
| POST | `/api/insights/reports/run` | **`insights.view`**. Validates and reruns a saved spec, optionally with a replacement date range and history ID. |
| GET / POST / DELETE | `/api/insights/reports/favorites[/{id}]` | Signed-in staff member's named validated report definitions. Results are rerun live, not stored. |
| GET / POST | `/api/insights/reports/history[/{id}/archive|/{id}/restore]` | Automatic successful-run history; active/archive listing, manual archive, and restore. Default inactivity archive is 180 days. |
| GET | `/api/insights/semantic-catalog` | Staff-visible governed datasets and members. Admin-only members are omitted for non-Admin staff. |
| GET | `/api/insights/cube-health` | Staff-safe Cube readiness/configuration without secrets. |
...
| GET | `/api/insights/commission-ledger` | **`insights.view`**. **Unpaid** = booked window on open lines; **earned** = fully paid Transactions with commission events or audited fallback lines in the recognition window (`from`/`to`), plus manual adjustments. Includes commissionable sales volume. |
...
| GET | `/api/insights/rms-charges` | **`insights.view`**. Query: **`from`**, **`to`** (optional UTC **`NaiveDate`**; inclusive start, exclusive end — same semantics as other insights date ranges; default window **90 days** through tomorrow UTC). Returns up to **500** rows from **`pos_rms_charge_record`** with charge/payment and Transaction/customer context. **UI:** curated report, native Insights when modeled, or integrations; register-scoped views use **POS → Reports**; filtered lists use **Customers → RMS charge**. See [**`docs/POS_PARKED_SALES_AND_RMS_CHARGES.md`**](POS_PARKED_SALES_AND_RMS_CHARGES.md). |
...
| GET | `/api/insights/loyalty-velocity` | **`insights.view`**. Curated Reports tile for daily loyalty point movement: earned, used, and net velocity. Native Insights models customer loyalty through the governed reporting schema. |
| GET | `/api/insights/merchant-activity` | **`insights.view`**. Merchant/payment activity by business date, including gross volume, fees, refunds, and net amount where available. Native Insights payment measures use **`reporting.payment_ledger`**. |
...
**Coverage boundary — Reports vs native Insights:** Operational Reports endpoints are available through fixed tiles immediately. Do not promise that an operational topic is conversationally sliceable until a readable **`reporting.*`** view is added to the Cube model and the matching server catalog. The browser and model must never compensate by querying application tables or inventing SQL.

**Existing Insights API not surfaced as Reports tiles:** **`commission-lines`** and **`commission-trace/{line_id}`** intentionally belong under **Staff → Commissions** drilldown. **`commission-adjustments`** and saved-view create/delete routes are lifecycle actions, not passive reports.
...
These endpoints are built for **pivot-style** and **ops** reporting. Native Insights uses its own server-validated semantic catalog and never converts arbitrary text directly into one of these routes or into SQL; see [**`CUBE_INSIGHTS_REPORTING.md`**](CUBE_INSIGHTS_REPORTING.md).

| Endpoint | Method | Auth / permission (typical) | What it returns | Natural-language examples |
|----------|--------|----------------------------|-----------------|---------------------------|
| `/api/insights/reports/ask` | POST | **`insights.view`**; Admin for cost/margin | Validated report specification, business explanation, rows, labels, formats, generated time, and history ID | “Recognized sales by category last month”, “Inventory below reorder point” |
| `/api/insights/sales-pivot` | GET | Should be gated as **`insights.view`** for NL reporting | Rows: **bucket**, **gross_revenue**, **tax_collected**, **order_count**, **line_units**; optional **weather_snapshot**, **closing_comments** (when `group_by=date`); **customer_id** when `group_by=customer`. **truncated** if capped at 200 rows. **Back Office → Reports** exposes this weather path as **Daily Sales Weather**. Native Insights models weather through **`reporting.daily_sales_weather`**. | “Sales by brand last month”, “Tax by category”, “Units by salesperson”, “Daily sales with weather” |
...
| `/api/insights/nys-tax-audit` | GET | **`insights.view`** | Reconciled legal tax summary using the Completed/Fulfilled basis: paid completed Transactions and settled refund/exchange events in the store-local range. Returns saved gross, taxable, nontaxable, state tax, local tax, and total tax; no source cutoff, tax-engine, current-catalog, or Z-close input. Native Insights exposes the same source as approved dataset **`sales_tax`**. | “New York sales tax report for July”, “How much sales tax did we collect?” |
...
| `/api/insights/register-sessions` | GET | **`insights.view`** | Closed sessions: **ordinal**, **opened_at**, **closed_at**, **cashier**, **opening_float**, **expected/actual cash**, **discrepancy**. History lists use `compact=true` to omit large saved-detail arrays; omitted detail is not treated as missing report evidence. | “Drawer over/short trends”, “Who closed last week” |
...
### 15.6 Native Insights vs curated REST reports

- **Ad-hoc exploration, charts, saved favorites, reusable history, and new periods:** native **Insights** through the validated Cube report specification — subject to Riverside permissions.
...
**A:** “**`GET /api/insights/margin-pivot`** and native Insights cost or margin measures are **Admin-only** in Riverside, not merely **`insights.view`**. The server checks that permission on every generated and rerun specification.”
