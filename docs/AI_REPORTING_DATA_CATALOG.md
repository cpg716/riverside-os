...
**Primary staff analytics UI (2026):** Interactive exploration and dashboards for most reporting are **Metabase** inside **Back Office → Insights** (same-origin **`/metabase/`** iframe). **`GET /api/insights/*`** remains for **operational** reads (commission ledger, register/RMS exports, wedding saved views, etc.), **Back Office → Reports**, **Customers → RMS charge**, and NL-reporting executors — not as a parallel full chart UI. **Phase 2 (migrations 90 + 96 + 106 + 107):** PostgreSQL schema **`reporting`** — **`orders_core`**, **`order_lines`** (migration **107**: checkout-frozen **`unit_cost`**, **`line_extended_cost`**, **`line_gross_margin_pre_tax`** — aligned with **`GET /api/insights/margin-pivot`**), **`daily_order_totals`** (store **booked** business date via **`effective_store_timezone()`**), **`daily_order_totals_fulfilled`** (pickup/ fulfillment day — migration **106**), **`order_recognition_at`** / **`order_recognition_business_date`** on header/line views, **`reporting.order_recognition_at()`**, **`loyalty_point_ledger`**, **`order_loyalty_accrual`**, **`loyalty_reward_issuances`**; customer ZIP/city/state + display name + staff names on order views. DB role **`metabase_ro`** (**`SELECT`** on **`reporting`** + **`EXECUTE`** on **`reporting.effective_store_timezone()`** and **`reporting.order_recognition_at`**). Policy toggles and JWT handoff settings: **`GET`/`PATCH /api/settings/insights`**. **`docs/METABASE_REPORTING.md`**, **`docs/REPORTING_BOOKED_AND_FULFILLED.md`**, **`docs/PLAN_METABASE_INSIGHTS_EMBED.md`**.
...
- **Gift / loyalty:** `gift_cards`, **`gift_card_events`**, loyalty settings tables, **`loyalty_point_ledger`**
- **Integrations:** QBO staging tables (as exposed by API); **`counterpoint_sync_runs`** (written on CP bridge batches — **no first-party read API** today; add one if NL reporting needs sync health)
