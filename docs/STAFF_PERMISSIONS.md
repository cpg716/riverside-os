...
| `reviews.view` | View review-request Outbox, sent, failed, cancelled, and suppressed history. |
| `reviews.manage` | Send an audited review-delivery test, or cancel a review request while it is still scheduled in Outbox with a required audited reason. |
...
| `customers.rms_charge.reporting` | RMS reporting and broader reconciliation visibility. |
| `pos.rms_charge.use` | Use the unified RMS Charge tender in POS. |
| `pos.rms_charge.lookup` | POS masked account lookup and disambiguation support. |
| `pos.rms_charge.history_basic` | POS slim RMS history and status visibility. |
| `pos.rms_charge.payment_collect` | RMS payment collection tools in POS. |
...
### Staff Access session integrity

`POST /api/staff/session` verifies the selected staff UUID and Access PIN once, then returns a random opaque token. The database stores only its SHA-256 hash. Every token is bound to both the stable workstation identity and a per-tab/window connection identity, expires after 16 hours by default (`RIVERSIDE_STAFF_SESSION_HOURS`, allowed range 1–24), and can be revoked independently. Sign-out revokes the current token; PIN change or staff deactivation revokes every active token for that staff member. `last_seen_at` is refreshed at most once per minute to preserve live Station Fleet evidence without turning every API read into a database write. Legacy code/PIN headers remain temporarily accepted for existing integrations and focused API tests, but the Riverside UI no longer stores or repeatedly sends the raw PIN.
...
| Location | Role |
|----------|------|
| `client/src/context/BackofficeAuthContext.tsx` | Holds the opaque Staff Access token and expiry, emits staff-session + station + connection headers, sends the station heartbeat, revokes on sign-out, and clears locally at server-issued expiry. Exposes **`backofficeHeaders()`**, permission/profile state, and refresh helpers. |
| `client/src/components/layout/BackofficeSignInGate.tsx` | Verifies the selected identity and Access PIN through **`POST /api/staff/session`**, adopts the returned permissions, and never retains the raw PIN. |
| `client/src/lib/backofficeSessionPersistence.ts` | Stores only the authenticated staff UUID, opaque token, and expiry in per-tab **`sessionStorage`** key **`ros.backoffice.session.v2`**; it deletes the legacy v1 raw-PIN payload. |
| `client/src/lib/posRegisterAuth.ts` | Merges the opaque Staff Access token, station/connection identity, and independently scoped open-Register token. Secret headers are stripped before any offline-queue persistence. |
...
Any `fetch` to a permission-gated API must pass **`backofficeHeaders()`** so the server receives the Staff Access token plus `x-riverside-station-key` and `x-riverside-connection-key`. POS/register authority separately requires the Register session id, station-issued POS token, and station key; each physical device that attaches to an open Register receives its own token. For **`GET /api/sessions/current`**, prefer **`mergedPosStaffHeaders(backofficeHeaders)`** whenever context is available; otherwise use **`sessionPollAuthHeaders()`**. **404** = no open till with valid auth; **401** = missing, expired, revoked, or incorrectly bound Staff/Register session.
