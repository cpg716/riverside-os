...
- **Identity Isolation (v0.2.0):** The staff member signed into the **Back Office / Shell** is independent of the **Checkout Operator** at the register. Logging into the computer as a Manager does **not** automatically attribute future sales to that Manager.
- **Register PIN Enlistment:** Every transaction requires the active salesperson to enter their unique **Access PIN** on the register keypad. This ensures that sales, returns, and cash adjustments are correctly attributed in audit logs, regardless of who is signed into the overall shell.
...
| `inventory.view_cost` | Cost fields in product / Inventory Overview paths. |
...
| **Show product costs** (role toggle) | **`inventory.view_cost`** — unit cost in POS Inventory Overview paths (see catalog table above). |
