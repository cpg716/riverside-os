...
| `client/src/lib/posRegisterAuth.ts` | **`mergedPosStaffHeaders(backofficeHeaders)`** — BO staff headers from context + POS session id/token from **`sessionStorage`** + the current device station key from **`localStorage`**. **`sessionPollAuthHeaders()`** — persisted BO session + POS token + current station key only (no React context); for **`GET /api/sessions/current`** outside the provider tree. **`hasRegisterSessionPollCredentials()`** — skip polls when neither staff nor POS id/station key would be sent (avoids pointless **401** in DevTools). |
