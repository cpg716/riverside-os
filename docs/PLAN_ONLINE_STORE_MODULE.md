...
| Area | Location / notes |
|------|------------------|
| **Schema** | **`migrations/legacy_prelaunch_history/73_online_store_module.sql`**: `orders.sale_channel`, `product_variants.web_*`, `store_pages`, `store_coupons`, `order_coupon_redemptions`, `store_tax_state_rate`, RBAC **`online_store.manage`**, seed **`home`** page. **`migrations/legacy_prelaunch_history/74_shippo_shipping_foundation.sql`**: Shippo columns on **`orders`**, **`shippo_config`**, **`store_shipping_rate_quote`**. **`migrations/legacy_prelaunch_history/75_unified_shipments_hub.sql`**: **`shipment`**, **`shipment_event`**, **`shipments.*`** — **`docs/SHIPPING_AND_SHIPMENTS_HUB.md`**, **`PLAN_SHIPPO_SHIPPING.md`**. **`migrations/legacy_prelaunch_history/76_store_guest_cart_and_media_assets.sql`**: **`store_guest_cart`**, **`store_guest_cart_line`**, **`store_media_asset`**. **`migrations/legacy_prelaunch_history/77_customer_online_account.sql`**: **`customers.customer_created_source`**, **`customer_online_credential`**. |
...
- Reuse patterns from **[`discount_events`](../migrations/legacy_prelaunch_history/41_discount_events.sql)** only if product wants **same** rules for web and events; otherwise **keep web coupons separate** to avoid conflating trunk-show automation with **code WELCOME10**.
...
**POS / fulfillment shipping:** `POST /api/pos/shipping/rates`, `POST /api/shipments/{id}/rates`, `POST /api/shipments/{id}/purchase-label` — defined in Shippo plan.
