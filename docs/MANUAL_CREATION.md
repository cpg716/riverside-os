...
### Screenshot coverage standard

- Approved manuals must reference at least **2 unique local screenshots**.
- Approved manuals should have **3 dedicated workflow screenshots** under `client/src/assets/images/help/<manual-id>/`.
- Major workflows should target **4 dedicated screenshots** when the screen has multiple phases, drawers, or review states.
- Reused overview screenshots are acceptable for context, but they do not replace dedicated screenshots that show the manual's actual workflow.
...
- `npm run check:help-impact` flags user-facing ROS changes that did not update Help/ROSIE corpus files
...
## Supported manual pipeline

Riverside's supported in-app Help pipeline is repo-owned:

- Markdown lives under **`client/src/assets/docs/<id>-manual.md`** (must end with **`-manual.md`**).
- Screenshots live under **`client/src/assets/images/help/<id>/`** and are captured by Playwright.
- Manifest and corpus artifacts are generated by **`npm run generate:help`**.
- ROSIE/Gemma draft output is maintainer-reviewed before becoming approved staff-facing manual content.

After adding or changing manual files:
...
1. auto-boots the local E2E stack if the UI/API are not already running
2. captures deterministic Help screenshots with Playwright
3. runs `npm run generate:help`
...
| **Screenshots** | Add/update Playwright specs in `client/scripts/help-screenshot-specs.mjs`, write PNGs under `client/src/assets/images/help/<id>/`, then run `npm run generate:help:refresh`. |
