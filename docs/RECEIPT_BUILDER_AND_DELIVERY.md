...
Receipt identity and financial fields are protected. The builder will not apply, test-print, or test-deliver a template that omits a required field or repeats a token. Protected fields include the receipt type, Transaction #, date/time, customer, salesperson, staff, Register #, items, order-payment activity, subtotal, separately stated sales tax, total, paid amount, balance, tender on standard receipts, and status. Picked Up receipts intentionally use the single `{{PAYMENT_BLOCK}}` section for prior payment, today's tender, balance, and payment status; `{{PAYMENT_HISTORY_BLOCK}}` is not required because the production renderer suppresses that separate legacy section for pickup and payment-on-order receipts. Template tabs show issue counts and provide a one-action **Restore Riverside default** recovery. Optional branding and operational sections remain editable.

The builder also exposes **Delivery tests**. A test email sends the current ReceiptLine preview through the configured Store Email mailbox. A test text requires the saved **Text receipts enabled** switch, applies the saved/default **Receipt MMS caption**, rasterizes the current preview to PNG, and sends it as a Podium MMS attachment. ReceiptLine layout tests use the unsaved editor state; Digital receipt delivery changes must be saved before testing their switch or caption. The Digital receipt delivery card reuses the established `ui-pill` readiness indicator and the existing Store Email and Podium settings and health endpoints. **Ready** requires live SMTP reachability for receipt email or an authenticated reachable Podium API plus a saved sending location and receipt-text switch. It shows each provider separately as **Checking**, **Ready**, **Setup needed**, **Needs attention**, or **Unavailable**, and shows **Save pending** when the receipt-text switch differs from its saved server value.
...
- The normal and gift-receipt email subjects are editable in **Settings → Receipt Settings → Digital receipt delivery**. Supported subject values are `{store_name}`, `{receipt_ref}`, `{receipt_type}`, `{customer_name}`, and `{customer_code}`. The receipt HTML itself remains controlled by Receipt Settings.
...
- **Enablement:** **Settings → Receipt Settings → Digital receipt delivery → Text receipts enabled** controls receipt SMS/MMS independently from staff-authored, pickup, alteration, appointment, and new-sender texts. Review requests retain their own review-policy switch.
...
- Normal and gift-receipt MMS captions are editable in **Settings → Receipt Settings → Digital receipt delivery** with `{store_name}`, `{receipt_ref}`, `{receipt_type}`, `{customer_name}`, and `{customer_code}` values.
