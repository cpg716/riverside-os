...
**Thermal ZPL:** **`GET /api/transactions/{transaction_id}/receipt.zpl`** supports the same **`gift`** and **`order_item_ids`** query parameters (full transaction is the default when omitted).
...
- **`POST /api/transactions/{transaction_id}/receipt/send-email`** — JSON body optional **`to_email`**; if omitted, uses the customer email on the transaction. Optional **`gift`** (bool) and **`order_item_ids`** (UUID array; empty = all lines) — same semantics as the HTML route.
