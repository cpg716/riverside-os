...
**`receipt_studio_layout_available`** on **`GET /api/transactions/{id}`** is `true` only when **`receipt_studio_exported_html`** is non-empty after trim.
...
| Value | Behavior |
|--------|----------|
| **`zpl`** (default) | **`GET /api/transactions/{id}/receipt.zpl`** — legacy Zebra-oriented builder; **Tauri** `printZplReceipt` (does not use GrapesJS HTML). |
...
- Server loads **`receipt_studio_exported_html`**, runs **`merge_receipt_studio_html(tpl, order, cfg, gift)`** with the same order snapshot shape as ZPL (`ReceiptOrderForZpl`, filtered when `transaction_line_ids` is set).
...
**Thermal ZPL:** **`GET /api/transactions/{transaction_id}/receipt.zpl`** supports the same **`gift`** and **`transaction_line_ids`** query parameters (full order is the default when omitted).
...
- **`POST /api/transactions/{transaction_id}/receipt/send-email`** — JSON body optional **`to_email`**; if omitted, uses the customer email on the order. Optional **`gift`** (bool) and **`transaction_line_ids`** (UUID array; empty = all lines) — same semantics as the HTML route.
