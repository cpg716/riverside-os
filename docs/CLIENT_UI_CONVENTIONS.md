...
- **Progressive Settings Navigation**: Keep **Settings Hub** pinned as the searchable entry point. Inside the Hub, use one compact category navigator and one consistent settings grid; present one category at a time while search spans every area. The Back Office sidebar exposes the same task-based groups as accordions and automatically opens the group containing the active deep link; do not restore a second provider overview, an always-expanded list, or independent category cards that create masonry gaps.
...
- **Compound-control labels**: A native `<label>` must identify one labelable form control and must never wrap an action button, suggestion menu, or shared selector. Use a neutral `<div>` for the visual group, then connect the field label with `htmlFor` and a stable or `useId()`-generated input id; use `aria-label` when the compact control has no visible field label.
- **Dropdown selection**: Selecting a `StaffMiniSelector` option must close the menu before publishing the new value and must prevent the option click from reactivating an ancestor trigger. Contract coverage in `client/e2e/staff-mini-selector-contract.spec.ts` rejects interactive controls nested in native labels.
...
- **Guided selection drawers**: Every multi-step side-panel picker must keep the item being configured and all completed choices visible. It must provide a clearly labeled **Back** action at every step, including confirmation or pricing review, and completed choices should be directly editable when the shared picker supports it. At the first step, Back returns to the originating workspace without applying the item. Reuse the shared picker so Cart additions, existing-line changes, Wedding Builder choices, and Customer Orders updates do not drift.
...
- **Helcim Unified Branding**: To ensure operational clarity, integrated payment methods must be explicitly labeled as **HELCIM CARD**, **CARD NOT PRESENT**, or **HELCIM VAULT**. This branding signal prioritizes the processor or card-entry mode as the anchor.
