...
- **Card close review**: Helcim terminal outcomes needing review remain visible in the POS close flow. Recording an outcome creates a **`helcim_terminal_recovery_actions`** audit row; it does not create a payment, refund, or ledger mutation. If an outcome remains unresolved, ordinary authorized close stays available and retains that warning in Payments Health and the audited close record rather than printing it on the financial Z-Report.
