...
Main Hub installs and standard updates run the pinned ROSIE stack installer. Existing assets whose hashes match the current pins are verified in place without recopying bundled runtime/speech assets or downloading them again. STT and TTS markers include the repository and immutable revision; individual assets remain SHA-256 checked, so a missing or damaged file is repaired even when the pin itself is unchanged. When a model pin changes, the update downloads and verifies the new text model and matching projector, activates them together, and runs deterministic, bounded text, SSE, native-tool, image, STT, and TTS certification before reporting readiness. Cold startup allows up to three minutes for a multi-gigabyte model to become healthy. A failed model certificate prints the individual HTTP, LLM, TTS, and STT results, restores the prior configured model when available, records ROSIE as degraded, and leaves the watchdog to retry; it does not fail or roll back an otherwise healthy Riverside OS update. Superseded managed model files remain available for rollback until the entire Main Hub update passes `/api/ready`, then are removed best-effort. `-PreserveExistingRosie` remains an explicit diagnostic/maintenance option, but it is not used by normal update workflows. If ROSIE itself needs repair, `Install-RosieAiStack.cmd` runs the same pinned setup directly.
