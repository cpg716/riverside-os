...
```text
RiversideOS-v0.60.2-Windows-Deployment/
...
```powershell
.\deployment\windows\build-deployment-package.ps1 -Version "0.60.2"
```

If the Tauri register bundle is coming from GitHub Actions instead of the local machine, copy the downloaded MSI into the package's `register/` folder before running `install-register.ps1`. For v0.60.2 and later, do not mix the `server/`, `client-dist/`, `register/`, or `updater/` folders from different release zips.
...
Hotfix/support actions included in v0.60.2 packages:
