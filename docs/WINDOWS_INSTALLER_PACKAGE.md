...
```text
RiversideOS-v0.70.7-Windows-Deployment/
...
```powershell
.\deployment\windows\build-deployment-package.ps1 -Version "0.70.7"
```

If the Tauri register bundle is coming from GitHub Actions instead of the local machine, copy the downloaded MSI into the package's `register/` folder before running `install-register.ps1`. For v0.70.7 and later, do not mix the `server/`, `client-dist/`, `register/`, or `updater/` folders from different release zips.
...
Hotfix/support actions included in v0.70.7 packages:
