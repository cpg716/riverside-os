...
Main Hub deployment packages stamp the installed server and database as `production` (`RIVERSIDE_MODE=production` and `store_settings.environment_mode='production'`). `RIVERSIDE_STRICT_PRODUCTION` is a separate hardening gate. It may remain `false` only during initial onboarding so staff can finish configuration; production diagnostics must show a go-live blocker until it is explicitly enabled. Before enabling it, run **System Audit** and verify the absolute backup directory, current migration ledger, exact browser origins, integration encryption key, live payment credentials, configured search service, and deployed web bundle. System Audit also requires the live API version and full build SHA to exactly match the verified deployment-package manifest retained under the installed `release` directory. Any audit `[FAIL]` exits non-zero. Routine updates preserve the installed Main Hub configuration instead of replacing it with the package example.

System Audit verifies the effective live CORS response when strict production is configured, so a persisted allowlist cannot pass while the running server still returns a wildcard. It does not require `COUNTERPOINT_SYNC_TOKEN`; direct Counterpoint Bridge sync is retired and backend Counterpoint operations use their own authenticated server paths.

The default installed config is `C:\RiversideOS\riverside-deployment.config.json`. A custom install root must remain discoverable through `server.installRoot` in the package config, or an operator launching the PowerShell manager/audit directly must pass `-ConfigPath <absolute-installed-config-path>`. The compiled Deployment Manager passes its resolved config path to System Audit automatically.
...
- **Production safeguards**: `server.strictProduction` remains an explicit operator-controlled setting. The Deployment Manager does not disable it because package environment fields are blank; Helcim and other integration credentials may be stored encrypted in the production database. Do not enable safeguards until **System Audit** reports that the strict-startup prerequisites are ready.
- **Runtime backup directory**: Main Hub config normalization fills a missing `RIVERSIDE_BACKUP_DIR` with the absolute `<installRoot>\\backups` path. This also runs inside `install-server.ps1`, so direct LAN/full-package updates cannot bypass it. Existing non-empty custom paths are preserved.
- **Integration credential encryption key**: the standard installer writes `RIVERSIDE_CREDENTIALS_KEY` to `C:\RiversideOS\server\.env`; the repair command may also establish a Windows Machine fallback. System Audit reads the effective server file first. The key must be present before Backoffice Settings can save Helcim, QBO, Counterpoint, or other encrypted integration credentials.
- **Meilisearch**: the Main Hub installer copies the pinned Community Edition `meilisearch\meilisearch.exe` to `C:\RiversideOS\meilisearch`, generates a unique master key on new installs, registers the **Riverside OS Meilisearch** loopback-only startup task on `http://127.0.0.1:7700`, disables Meilisearch analytics, schedules a daily local snapshot, and writes `RIVERSIDE_MEILISEARCH_URL` plus `RIVERSIDE_MEILISEARCH_API_KEY` to the server `.env`. After a reset, restore, or Counterpoint import, rebuild search from **Settings → Integrations → Meilisearch**.
...
- **Server update**: copies the new server and web files for the same Riverside release, applies pending migrations, refreshes the firewall/task setup, and restarts Riverside. Before replacing `riverside-server.exe`, the installer stops the scheduled task and processes, then requires an exclusive file handle so a delayed Windows handle release cannot fail midway through replacement. Before replacing the bundled Meilisearch executable, the updater removes its scheduled-task restart policy and verifies the process has stopped so Windows cannot relock the file during the update.
- **Pre-update safety**: the in-app GitHub path requires the selected release asset to have a valid SHA-256 digest, verifies the downloaded ZIP against it, and then verifies the embedded build identity before extraction can proceed to installation. Existing Main Hub installs, pushed-LAN updates, and fleet updates create and verify the database backup with the configured PostgreSQL administrator before the running server is stopped, installed files are replaced, or resolved config changes are persisted. They never fall back to the limited Riverside app account for a full dump. Every custom-format backup must pass `pg_restore --list`; incomplete or unreadable dumps are removed. A package or backup failure leaves the current server and installed config untouched. Failures after shutdown independently restore prior files and the scheduled task, keep both the restored server environment and installed config synchronized with any completed database app-role credential change, then attempt to restart the previous server.
- **Installed environment serialization**: `C:\RiversideOS\server\.env` remains generated from Deployment Manager and feature-owned settings. The installer writes dotenv-safe quoted values, including Windows paths, spaces, quotes, dollar signs, URLs, and credentials. Packaged ROSIE, System Audit, and credential-repair scripts decode that same representation; operators should not manually add shell quoting or move integration credentials out of their owning Settings pages.
...
Main Hub installs run the complete pinned ROSIE stack installer. Routine exact-build Main Hub updates compare deterministic component fingerprints from the new package with the last successfully installed manifest. An unchanged ROSIE fingerprint preserves its files, environment, processes, and scheduled task without repeating foreground certification; a changed or previously untracked fingerprint runs the complete pinned verification path. Existing assets whose hashes match the current pins are verified in place without unnecessary downloads. STT and TTS markers include the repository and immutable revision; individual assets remain SHA-256 checked, so a missing or damaged file is repaired when reconciliation is required. When a model pin changes, the update downloads and verifies the new text model and matching projector, activates them together, and runs deterministic, bounded text, SSE, native-tool, and image certification before accepting the model. Cold startup allows up to three minutes for a multi-gigabyte model to become healthy. The foreground updater does not run or print the Kokoro-to-SenseVoice loopback. After the exact Riverside build passes `/api/ready`, the scheduled ROSIE watchdog runs speech certification without a user-visible console and records exact TTS/STT status for support. A failed model certificate restores the prior configured model when available; a later speech failure records ROSIE as degraded for watchdog retry without failing or rolling back an otherwise healthy Riverside OS update. Superseded managed model files remain available for rollback until the entire Main Hub update passes `/api/ready`, then are removed best-effort. `-PreserveExistingRosie` remains an explicit diagnostic/maintenance option. If ROSIE itself needs repair, `Install-RosieAiStack.cmd` runs the complete pinned setup and certification directly.

Tagged releases publish two exact-build packages. `Windows-Deployment.zip` remains the complete installation and recovery source. `MainHub-Update.zip` excludes unrelated deployment/support applications and is preferred automatically by Settings → Updates on an existing Main Hub. The update manifest fingerprints ROSIE, Meilisearch, and Cube Core and performs a fast installed-runtime check; unchanged, healthy components stay running, while changed, unhealthy, or untracked components follow their existing verified installer path. Package and per-file checksum verification, the pre-migration database backup, migration checksum enforcement, rollback files, exact build validation, and `/api/ready` remain mandatory. The server is restarted once for the guarded cutover and is not stopped again after the local desktop application update.
