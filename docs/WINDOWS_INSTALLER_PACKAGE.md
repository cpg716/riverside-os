...
Main Hub installs and standard updates run the pinned ROSIE stack installer. Existing assets whose hashes match the current pins are verified in place without recopying bundled runtime/speech assets or downloading them again. STT and TTS markers include the repository and immutable revision; individual assets remain SHA-256 checked, so a missing or damaged file is repaired even when the pin itself is unchanged. When a model pin changes, the update downloads and verifies the new text model and matching projector, activates them together, and runs deterministic, bounded text, SSE, native-tool, and image certification before accepting the model. Cold startup allows up to three minutes for a multi-gigabyte model to become healthy. The foreground updater does not run or print the Kokoro-to-SenseVoice loopback. After the exact Riverside build passes `/api/ready`, the scheduled ROSIE watchdog runs that speech certification without a user-visible console and records exact TTS/STT status for support. SenseVoice uses the explicit English language profile with inverse text normalization in both certification and live transcription. A failed model certificate restores the prior configured model when available; a later speech failure records ROSIE as degraded for watchdog retry without failing or rolling back an otherwise healthy Riverside OS update. Superseded managed model files remain available for rollback until the entire Main Hub update passes `/api/ready`, then are removed best-effort. `-PreserveExistingRosie` remains an explicit diagnostic/maintenance option, but it is not used by normal update workflows. If ROSIE itself needs repair, `Install-RosieAiStack.cmd` runs the complete pinned setup and certification directly.
