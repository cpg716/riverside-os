...
| 3 | **Podium** | **SMS, reviews, and web chat** | Podium remains the customer texting/review/web-chat rail, including email delivery of Podium review requests. Other Store Email uses the ROS first-party IONOS mailbox. ROS stores OAuth credentials in encrypted Settings, uses a public HTTPS callback/webhook URL for Podium delivery, and ingests message webhooks into the Podium Inbox/customer message thread. |
...
| 13 | **Meilisearch** (self-hosted search) | **Optional in-repo** | Free Community Edition sidecar in **`docker-compose.yml`** or the matching Main Hub Windows binary; no Meilisearch Cloud or Enterprise dependency. ROS indexes variants, store catalog, customers, weddings, Back Office Orders, Transaction Records, alterations, and help from PostgreSQL with SQL hydration + **ILIKE** fallback. Configure URL and API key in **Settings → Integrations → Meilisearch**; admin reindex in **Settings → Integrations** or **`POST /api/settings/meilisearch/reindex`**. See [`SEARCH_AND_PAGINATION.md`](./SEARCH_AND_PAGINATION.md), [`STORE_DEPLOYMENT_GUIDE.md`](./STORE_DEPLOYMENT_GUIDE.md). |
