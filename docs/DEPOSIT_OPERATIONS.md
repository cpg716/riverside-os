...
5. Select each beneficiary and amount, then explicitly choose `held_for_future_order` or `existing_transaction` with one exact target Transaction Record. For equal deposits, enter **Deposit amount per selected member** once and select only the intended members. A newly selected member receives that amount immediately; **Apply to Selected** updates only the current selection, individual member amounts remain editable, and unselected members receive nothing. A selected `$0` member is visibly excluded and does not block other funded members; enter an amount to include that member.
...
7. In **Collect & Build Orders**, choose **Start Building Member Orders**. Riverside preserves any merchandise already staged for the payer and its salesperson attribution, then opens each funded member inside one Wedding Builder before showing payer Payment. The deposit salesperson is captured separately. Employee-price payer merchandise is permitted in the combined checkout, remains noncommissionable, and does not inherit the deposit salesperson. Party-level Builder parent products are filtered by their **All**, **Groom Only**, **Groomsmen Only**, **Any**, or **Other** member rule before appearing as a checklist; choose that member's exact variation, skip an item, or use product search for a different parent. Optional alteration intakes may be added to the member draft. Mark merchandise **Order (Wedding)**, choose a salesperson with **Salesperson for all member Transactions** or override that member individually, and explicitly select **No Tax** with a required reason when applicable. Each **Save Member Order & Next** is a nonfinancial draft only: it creates no Transaction Record, deposit, payment allocation, booked activity, fulfillment record, tax record, or receipt.
...
9. Only an approved tender followed by successful atomic checkout creates the payer Transaction, payment allocations, held credits, workflow audit records, and booked activity. In **Collect & Build Orders**, that same database transaction also saves every reviewed member draft and its stable checkout identity on the funded workflow. The draft snapshot is nonfinancial, but it is now recoverable from Main Hub even if the Register closes after the payer receipt. A declined card creates none of those records; the reviewed allocations and member drafts remain staged for retry or another tender, while the declined provider attempt remains in Payments Health.
10. After the approved payer receipt, the heading remains **Deposit complete · orders pending**. Choose **Continue to Member Transactions**, review all prepared member rows, and choose **Create All Member Transactions** once. Riverside creates one separate, server-validated atomic Transaction and Wedding Fulfillment Order for each member, applies only that member's exact source-tracked held deposit, and never collects a second payer tender. A later **Deposit Only** build saves each completed member draft to Main Hub before advancing to the next member. The final screen lists the exact items/variations, salesperson, tax treatment, Transaction total, deposit applied, balance due, and an individual **View / Print Receipt** action for every member.
11. The final action is restart-safe and idempotent, not a financially false all-or-nothing wrapper. Each required held-credit redemption must use the stable checkout identity saved with that member's reviewed draft. If one member fails, that member's atomic checkout rolls back; already successful members remain truthfully posted once, and retry resumes only unfinished members. Riverside reloads the funded workflow from Main Hub after posting and does not show **Wedding Builder Complete** until every expected member has an exact server-confirmed Transaction. Staff may also resume later through the original payer's **Wedding Deposit → Orders & Receipts** entry. That payer-scoped history reloads the durable drafts, funded member allocations, exact held-source balances, posted member Transactions, and receipt links; choose **Build All Remaining Orders** to complete the same Builder without visiting member accounts individually.
...
Card provenance remains attached to the payer tender and its source allocations. A wedding allocation funded by multiple payments retains each contributing payment Transaction, amount, payer, and provider transaction ID. A member refund resolves those exact sources and refundable capacities; it does not treat one account-level Helcim ID as the source and cannot redirect another payer's funds. Original-card money returns to the originating payer(s), never to the beneficiary member.

When an individual wedding member is selected in Register, the nonfinancial **Part of the Wedding Order?** prompt uses the Wedding Hub purchase context and the same parent-product variation drawer as Deposit & Build. Accepting activates the exact party/member and makes new searched or scanned merchandise default to `wedding_order`; each parent item still requires an exact resolved variation. The prompt may display the available held source and contributing payer, but it never consumes the credit. Only an explicit Pay-screen application followed by normal atomic checkout creates the member Transaction, Wedding Fulfillment Order, open-deposit redemption, salesperson attribution, tax records, reporting activity, and receipt. Declining the prompt leaves a regular sale and creates no wedding or financial write.
