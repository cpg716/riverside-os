...
2. Choose the workflow first: **Deposit Only** finishes after the successful payer receipt, while **Collect & Build Orders** returns directly to funded-member item selection after that receipt.
...
6. Review the chosen workflow, payer, party, funded members, destinations, amounts, total, and responsible salesperson before adding the deposits to the Cart.
7. Before the first tender, `/api/weddings/deposit-workflows/preflight` rechecks membership, payer ownership, open target status, and current balance. Checkout revalidates the same financial facts under database locks.
8. Only an approved tender followed by successful atomic checkout creates the payer Transaction, payment allocations, held credits, workflow audit records, and booked activity. A declined card creates none of those records; the reviewed Cart allocations stay staged for retry or another tender, while the declined provider attempt remains in Payments Health.
9. In **Collect & Build Orders**, **Continue to Payment & Build Orders** stages the reviewed allocations and opens payer Payment in one action. The payer tender is still collected exactly once. After the approved payer receipt, **Continue Wedding Orders** immediately opens the first funded member without a posted Transaction Record.
10. Add items from the Wedding Checklist or product search, mark deferred lines **Order (Wedding)**, and visibly confirm the salesperson. These actions only stage that member's Cart. Only a successful **Complete Sale / Record Sale** atomic checkout may create that member's Transaction Record and redeem the exact source-tracked held deposit; a failed checkout rolls those writes back.
11. After each member receipt, **Continue Wedding Orders** immediately advances to the next funded member without a posted Transaction Record. When no unposted funded member remains, Riverside shows the payer-centered **Orders & Receipts** dashboard. Staff may also resume later through the original payer's **Wedding Deposit → Orders & Receipts** entry and view or print payer/member receipts there.
12. Removing or changing the selected Customer clears the linked Wedding member banner and any unposted deposit allocations that belonged to the former payer. Stale Wedding context never carries into another Customer's Cart.
