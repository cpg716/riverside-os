...
| **Webhooks** | **`POST /api/webhooks/podium`** verifies **`podium-timestamp`** + **`podium-signature`**, then durably stores verified JSON before returning `200`. Migration **183** adds pending/processing/processed/skipped/failed lifecycle, leases, backoff, crash reclamation, and terminal alerts. The worker applies message failures, contact create/update/delete/merge/unchanged lifecycle events, and message ingest unless **`RIVERSIDE_PODIUM_INBOUND_DISABLED`** is truthy. API pull remains missed-history recovery, not a replacement for an enabled webhook. |
| **Public API** | **`GET /api/public/storefront-embeds`** — unauthenticated JSON for public builds to inject widget snippet when enabled. |
| **Client** | **Settings → Integrations → Podium** — credentials-missing callout, required OAuth scopes including `read_contacts`, health and full contact reconciliation controls, conflict evidence, and the editable message catalog. **Operations/POS → Podium Inbox** — direct text composer, collision-safe unmatched queue with explicit customer selection, channel/state filters, quick replies, auto-scroll, outbound sender, and assignee display. **Customer Hub** shows durable contact status, exact provider ID, retry/error, and last success next to manual sync. |
...
**Polish / operational boundary:** **`sms.templates`** RBAC split still uses **`settings.admin`**. Dedicated online-store work remains out of scope. A code-complete integration is not live proof: deploy migration/code, reconnect OAuth for `read_contacts`, configure the public contact/message webhook events, send provider tests, and verify the exact deployed build before operational certification.
