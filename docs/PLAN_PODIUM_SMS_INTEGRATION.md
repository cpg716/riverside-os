...
| **Webhooks** | **`POST /api/webhooks/podium`** verifies **`podium-timestamp`** + **`podium-signature`**, then durably stores verified JSON before returning `200`. Migration **183** adds pending/processing/processed/skipped/failed lifecycle, leases, backoff, crash reclamation, and terminal alerts. The worker applies failure events and runs **`podium_inbound::ingest_from_webhook`** unless **`RIVERSIDE_PODIUM_INBOUND_DISABLED`** is truthy. Customer-originated messages can create a stub, capture names, persist the thread, and fan out notifications; staff-originated events do not create unmatched stubs. API pull remains missed-history recovery, not a replacement for an enabled webhook. |
| **Public API** | **`GET /api/public/storefront-embeds`** — unauthenticated JSON for public builds to inject widget snippet when enabled. |
| **Client** | **Settings → Integrations → Podium** — credentials-missing callout, parallel **readiness** strip, expanded OAuth scopes (`read_users`, `write_contacts`), and one editable catalog for operational SMS/Store Email, Podium review SMS/email, and receipt delivery envelopes; **`data-testid="podium-sms-settings-section"`** for E2E. **Operations/POS → Podium Inbox** — direct text composer, unmatched provider queue, channel/state filters, quick replies, auto-scroll to newest message, **Sent** badge on outbound, assignee display in thread header. **Staff → Edit** — Podium user dropdown for identity linking. **Customer Hub → Messages** — **Sync to Podium Contacts** button. Customer Hub → Communication preferences: **review requests opt-out** checkbox. **`StorefrontEmbedHost`** when **`VITE_STOREFRONT_EMBEDS=true`**. Playwright: **`client/e2e/podium-settings.spec.ts`**. |
...
**Polish / gaps:** **`sms.templates`** RBAC split still uses **`settings.admin`**. Dedicated **online store** route in-repo (beyond embed) remains **out of scope** unless **`PLAN_ONLINE_STORE_MODULE.md`** expands storefront SMS. All core Podium API endpoints are now wired; no remaining integration gaps.
...
| **Store settings** | **`podium_sms_config`** JSONB: **`sms_send_enabled`**, **`location_uid`**, widget fields, operational **`templates`**, operational **`email_templates`**, **`review_templates`**, and **`receipt_templates`** |
