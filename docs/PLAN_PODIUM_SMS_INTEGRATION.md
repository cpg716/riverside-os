...
| **Server** | **`server/src/logic/podium.rs`** — refresh-token OAuth using encrypted integration credentials, cached access token on **`AppState.podium_token_cache`**, E.164 normalization, **`POST {RIVERSIDE_PODIUM_API_BASE}/v4/messages`** (default `https://api.podium.com`), **`POST …/v4/messages/attachment`** (multipart image for SMS/MMS), cursor-paged **`GET …/v4/conversations`** sync for current inbox rows, **`GET …/v4/conversations/{uid}/messages`** for profile/history hydration, and a background missed-history pull every 30 hours by default. `podium_send_ok` / `podium_send_err` tracing does not log phone/body. All Podium API requests include **`podium-version`** from **`RIVERSIDE_PODIUM_API_VERSION`** or the shipped default **`2021.04.01`**. **`wiremock`** unit test exercises token + message POST against a mock base URL. |
