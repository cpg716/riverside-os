...
| **Server** | **`server/src/logic/podium.rs`** — refresh-token OAuth using encrypted integration credentials, cached access token on **`AppState.podium_token_cache`**, E.164 normalization, **`POST {RIVERSIDE_PODIUM_API_BASE}/v4/messages`** (default `https://api.podium.com`), **`POST …/v4/messages/attachment`** (multipart image for SMS/MMS), **`GET …/v4/conversations/{uid}/messages`** for profile/history hydration, `podium_send_ok` / `podium_send_err` tracing (no phone/body in those lines). All Podium API requests include **`podium-version`** from **`RIVERSIDE_PODIUM_API_VERSION`** or the shipped default **`2021.04.01`**. **`wiremock`** unit test exercises token + message POST against a mock base URL. |
...
| **Still open / nice-to-have** | Dedicated Settings thread browser; throttle/metrics (**Phase 2** ops). Historical thread backfill is supported through Podium conversation-message reads when the OAuth grant includes **`read_messages`**. If that scope/product access is missing, or webhooks were disabled when the activity occurred, ROS can show a matched conversation in the inbox but cannot show message bodies in the customer profile until credentials/scopes are corrected and sync/webhooks run. |
