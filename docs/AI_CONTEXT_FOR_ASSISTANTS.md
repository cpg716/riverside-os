...
| **Live help retrieval (server)** | **`GET /api/help/search`** + **`PLAN_HELP_CENTER.md`**, [`ROS_AI_HELP_CORPUS.md`](ROS_AI_HELP_CORPUS.md) | Current: **`ros_help`** corpus, optional Meilisearch, generated from bundled in-app manuals — **`docs/MANUAL_CREATION.md`**. Retired **`/api/ai/*`** and **`ai_doc_chunk`** are not current. |
