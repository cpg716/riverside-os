...
### 2.1 Current deployment status snapshot (2026-05-17)

This is the current repo/deployment status to verify before a live install:

| Item | Current status | Deployment impact |
|------|----------------|-------------------|
| Target release version | **`v0.60.0`** | Root, client/PWA bundle, server, and Tauri metadata must all match. Run `npm run check:version` before publishing artifacts. |
| Latest published GitHub release | **`v0.60.0`** | Use the release workflow output for the current release; do not mix installer assets from older releases. |
...
| Latest Playwright E2E on `main` | Must pass on the final `v0.60.0` release commit | Rerun GitHub checks on the release commit before calling the code gate green. |
| Latest Lint Checks on `main` | Must pass on the final `v0.60.0` release commit | Rerun GitHub checks on the release commit before calling the code gate green. |
