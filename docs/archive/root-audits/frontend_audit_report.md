...
| `TransactionAttributionModal` headers | **OK** — Uses `mergedPosStaffHeaders` so register **POS session token** is sent for `GET /api/transactions/:id` and staff list; matches server **staff or POS session** gates. |
...
`StaffWorkspace.tsx`, `OrdersWorkspace.tsx`, `CommissionPayoutsPanel.tsx`, `QboWorkspace.tsx`, `StaffAccessPanels.tsx`, `PhysicalInventoryWorkspace.tsx`, `TransactionAttributionModal.tsx`, `BackofficeAuthContext.tsx` (definition / permissions fetch).

*Interpretation:* BO workspaces generally do not need POS session tokens; POS-facing flows use merge. **`TransactionAttributionModal`** is the main candidate to confirm against backend expectations when opened from the register.
...
4. ~~**Align** `TransactionAttributionModal` headers~~ — **Verified** (merged headers for register order reads).  
...
client/src/components/pos/TransactionAttributionModal.tsx
