...
- **Void vs cancel** — **`orders.void_sale`** (migration **49**) allows **`PATCH`** to **`cancelled`** when the transaction has **no** payment allocations; **`orders.cancel`** remains required when money was allocated (refund queue). See **[`docs/TRANSACTION_RETURNS_EXCHANGES.md`](./TRANSACTION_RETURNS_EXCHANGES.md)**.
...
| Theme | What shipped |
|--------|----------------|
| **Suit / component swap** | **`POST /api/transactions/{id}/items/{line}/suit-swap`**, `suit_component_swap_events`, inventory `adjustment` rows for fulfilled takeaway swaps; QBO journal **warnings** when swaps exist on the activity date; Back Office **Orders → Swap component**; permission **`orders.suit_component_swap`** (**50**). |
...
| **POS exchange + link** | **`PosExchangeWizard`** (returns + order totals/tenders); replacement checkout triggers **`POST /api/transactions/{original}/exchange-link?register_session_id=…`** from the cart when an exchange is in progress (`Cart.tsx`). |
