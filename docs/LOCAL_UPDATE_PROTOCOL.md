...
Fresh installs and pre-launch reset builds use the schema-contract baseline in `migrations/001_core_identity_staff.sql` through `migrations/009_promo_gift_cards.sql`. Migration filenames are tracked in **`public.ros_schema_migrations`**. **Never** skip the ledger inserts: the apply scripts record each applied filename.
