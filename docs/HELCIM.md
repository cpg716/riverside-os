...
- Hosted Card Not Present does not send a synthetic ROS value in the top-level `invoiceNumber` field, which links only an existing Helcim invoice. ROS instead uses HelcimPay.js `invoiceRequest` to create a minimal provider invoice whose unique `ROS-<attempt>` number and single line equal the exact payment amount. The signed browser response remains the immediate confirmation path; the signed Helcim webhook can independently fetch that exact invoice number and bind the provider transaction to the same ROS attempt if the browser confirmation is lost. Amount and time alone are never treated as transaction identity.
...
| Card terminals, devices, device detail, ping | Configuration and operator-requested readiness diagnostics. The live purchase path does not send a separate ping that could disturb the reader immediately before payment. |
| Hardware start purchase | Card-present checkout, dispatched once directly to the Register's assigned terminal. |
...
| HelcimPay.js checkout initialization, invoice request, iframe events, and response validation | Card Not Present entry on the whitelisted public HTTPS origin; each attempt creates an exact ROS-correlated provider invoice, signed approvals are validated server-side, and known decline/close events finalize the exact attempt. |
...
The current Riverside sale model does not use Payment API preauthorization/capture, card verification, ACH, recurring plans, itemized Helcim invoicing as a second sale ledger, provider-side batch settlement, Fee Saver, or full-card-number processing. The minimal HelcimPay.js invoice request used for exact Card Not Present correlation is provider evidence only; the ROS Transaction remains the financial source of truth. Adding other provider workflows simply because an endpoint exists would create extra checkout states, PCI scope, or a competing ledger authority. They require a separate approved business workflow before implementation. HelcimPay logo upload is presentation-only and does not affect transaction reliability.
...
- **Direct terminal dispatch**: ROS sends one purchase directly to the assigned terminal after customer-profile enrichment. It does not precede that purchase with the Settings diagnostic ping. A `409 device ... not listening` is a definitive rejection of that purchase request: ROS records the failed attempt, sends no retry, and leaves the sale ready after staff wake or sign in to the reader.
...
- **Checkout isolation without Register hostage state**: A repeated Card Reader request for the same open checkout returns and reconciles its existing pending attempt instead of dispatching a second purchase. A confirmed approval for that exact checkout remains protected from a duplicate charge and must be attached before that checkout is recorded. Earlier pending, failed, timed-out, expired, canceled, or unresolved attempts from a different checkout remain Payments Health evidence but do not disable cash, check, gift card, store credit, Manual Card, sale clearing, or a later checkout. Starting a new Card Reader request replaces the earlier ROS routing reservation on that Register's assigned terminal without deleting its provider identifiers or audit history.
- **Exact build pre-dispatch gate**: Current Register clients send their full source build SHA on authenticated POS requests. If a current client and Main Hub report different immutable builds, Card Reader, Card Not Present, and Saved Card stop before Helcim receives a request and show both build prefixes. Missing identity from a legacy client remains compatible during rollout, while deployment auditing still requires the installed Main Hub and packaged Register to match the exact package SHA.
- **Recovery scope**: The active Register badge reads recovery with the POS Register session credential and shows only that session's open work. Staff-wide and prior-session recovery remains available in Payments Health and Register close history, but it is never imported into a new checkout or used to disable that sale.
...
- Terminal purchases create a pending provider attempt immediately before the single hardware dispatch; hosted Card Not Present payments create one after Helcim returns the secure checkout token. Provider completion remains a separate event.
...
- Card Not Present does not require staff to enter an invoice number. ROS creates a unique provider invoice request for the hosted attempt and accepts only Helcim evidence whose invoice, amount, currency, and purchase purpose match that exact attempt. HelcimPay.js `ABORTED` (declined) and `HIDE` (closed before processing because ROS disables Helcim's confirmation screen) are posted back to the same server-side attempt. If the signed browser handoff is lost, the signed webhook or **Recover payment** can resolve the exact provider invoice without charging again; amount and time are never used as a substitute identity. A delayed exact approval can also replace one matching Manual Card placeholder on that completed checkout without creating a second payment.
...
- Register routing uses Terminal 1 / Terminal 2 naming and a one-to-one live assignment: one Register owns one terminal during payment. A terminal is not shared between simultaneous Register checkouts.
