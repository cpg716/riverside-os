...
2. **Process cash-out**
   ...
   - **Helcim:** `card_present` and `card_manual` both mean refund to the original linked Helcim card. Selecting **Original Card** stages the tender as pending; it is not an approval. The server creates a deterministic provider attempt and resolves the current Helcim V2 transaction ID before calling the provider. For a full refund while the card batch is open, ROS uses Helcim **Reverse**; for a closed/settled batch or a partial refund, ROS uses Helcim **Refund**. ROS commits the negative payment and refund queue update only when Helcim returns approved/captured. Successful completion explicitly names Helcim, the exact approved amount, and the masked card so staff can distinguish provider approval from a staged refund. Definite declines and safe pre-dispatch failures persist a failed provider attempt and leave the refund queue unchanged. A network/5xx outcome that may have reached Helcim remains pending and may be replayed only with the same key inside ROS's conservative safe-retry deadline; afterward it requires exact provider reconciliation and cannot be replaced by a new refund. Standalone Payments API refund/reverse actions are rejected; all processor money movement remains bound to this authorized Transaction Record workflow.
