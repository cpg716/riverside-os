...
2. **Process cash-out**
   ...
   - **Already-processed Helcim refund:** `card_terminal_manual` is only for a refund that staff already completed in Helcim. It requires Manager Access, a reason, and the Helcim refund reference. ROS records a canonical `card_manual` negative payment with `payment_provider = helcim` and the provider reference, so it remains visible in the built-in Payments workspace without creating a second provider refund.
