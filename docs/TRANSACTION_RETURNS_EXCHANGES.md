...
**Receipts:** Customer receipts never hide returned or exchanged selected lines. Active quantities print in their normal sale/pickup/shipping sections. A refund receipt prints each returned item as negative merchandise, prints the exact negative tax and refund tender, and totals to the real refund amount. Historical sale receipts preserve the original sale proof without re-applying later return events.
...
2. **Process cash-out**
   ...
   - **Reporting event:** each completed return stores one immutable refund event with its exact merchandise subtotal, state tax, local tax, total, tender, and register session. Daily Sales, Z-Reports, receipts, refund queues, and QBO all read that same event instead of recomputing a return from the current catalog price or the original undiscounted line.
