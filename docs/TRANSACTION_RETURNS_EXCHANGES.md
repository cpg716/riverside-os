...
**Receipts:** A completed return or exchange produces an event-scoped receipt for today's customer activity, not a reprint of the entire original sale. It prints only today's returned items as negative merchandise, today's new merchandise or services as positive lines, the exact event tax, and the current tender outcome. Items the customer kept from the original sale and historical tenders are omitted. A mixed exchange totals to the exact net refund or amount due after the return credit and new lines are combined. Historical sale receipts remain available as the original sale proof without re-applying later return events.
...
1. **Queue sources**
   ...
   - **Partial unfulfilled Order-item cancellation:** use **Customer Orders → Cancel Order Items**, not Return / Exchange. `POST .../order-line-cancellations/preview` locks and validates the selected open deferred lines, then returns the cancellation merchandise/tax credit, current paid and balance figures, revised Transaction total, balance remaining, and actual refund due. Cancellation credit first reduces the existing unpaid balance; only `max(cancellation credit - prior positive balance due, 0)` becomes a refund obligation. Confirmation records append-only line removals with `restock = false`, recalculates the Transaction, updates Wedding/Fulfillment tracking through the canonical Transaction lines, and opens the existing refund queue only when the revised Transaction is overpaid. A true overpayment continues to multi-source Pay; a linked Helcim source must be approved and committed in the refund ledger before the refund can finish.
   - Fulfilled or takeaway merchandise cannot enter the Order-item cancellation route. Use Return / Exchange for goods already given to the customer. Conversely, partial refund endpoints reject unfulfilled Order items so gross item value cannot be mistaken for refundable cash.
   - Procurement state is preserved rather than silently erased. A vendor/PO commitment remains visible for receiving follow-up. Received or Ready-for-Pickup Special, Custom, or Wedding goods remain in `stock_on_hand` while the cancelled customer's `reserved_stock` quantity is released; Layaway cancellation releases `on_layaway`. The cancellation audit records the exact inventory bucket and quantity released.
...
2. **Process cash-out**
   ...
   - **Helcim:** `card_present` and `card_manual` both mean refund to the original linked Helcim card. On a refund-only return, selecting **Original Card** and applying it calls the canonical refund processor before final UI completion. The drawer adds the tender and enables **Record Sale** only after the response contains approved/captured Helcim status, a provider refund ID, and the committed ROS `refund_event_id`; the final record request replays the same stable `refund_tender_id` idempotently. Exchange card remainders remain deferred until their replacement Transaction and inventory link exist. The server reads the linked original payment's `provider_transaction_id`, verifies the current Helcim V2 transaction and batch, and sends the resulting original charge ID to Helcim as **`originalTransactionId`**. Staff-supplied transaction IDs are not accepted. For a full refund while the card batch is open, ROS uses Helcim **Reverse**; for a closed/settled batch or a partial refund, ROS uses Helcim **Refund**. ROS commits the negative payment and refund queue update only when Helcim returns approved/captured. Successful completion explicitly names Helcim, the exact approved amount, and the masked card so staff can distinguish provider approval from a staged refund. Definite declines and safe pre-dispatch failures persist a failed provider attempt and leave the refund queue unchanged. A network/5xx outcome that may have reached Helcim remains pending and may be replayed only with the same key inside ROS's conservative safe-retry deadline; afterward it requires exact provider reconciliation and cannot be replaced by a new refund. Standalone Payments API refund/reverse actions are rejected; all processor money movement remains bound to this authorized Transaction Record workflow.
   - **Restore after an interrupted approval:** keep the return or cancellation open in Register Pay and select **Restore**. **Record Sale** retries the same durable refund attempt and idempotency key. If Helcim already approved the refund/reversal, ROS attaches that exact provider movement before committing returned quantities, inventory, cancellation status, balances, audit, or receipt. When Helcim cannot provide stable automatic evidence, staff may use the existing manual external-card path in Pay only after verifying the exact provider reference and completing Manager Access; ROS never sends a second refund merely to clear the blocker.
   - **Staff Access handoff:** a Transaction Record opened for cancellation, return, or refund remains pending while the Register requests Staff Access. The Register consumes that handoff only after sale hydration and an authorized sale operator are both ready, so the required lock screen cannot discard the negative lines or refund settlement.
   - **Already-processed external card refund:** `card_terminal_manual` is for a real refund staff already completed in a prior processor, Helcim dashboard, or another external card system. It requires Manager Access, a reason, card last four, and the external refund/approval reference. ROS records a negative `card_terminal_manual` payment with `payment_provider = external`; it never invents a Helcim approval or calls Helcim a second time.
   - **Check:** `check_number` is required and is written to both the refund payment and allocation so receipts, register reconciliation, search, and QBO evidence retain the instrument number.
   - **Staff Account:** `staff_account_charge` is allowed only up to the remaining original Staff Account tender. The negative payment and Staff Account receivable reduction commit together.
   - **RMS Charge:** `on_account_rms` / `on_account_rms90` are allowed only up to the remaining original RMS-financed tender. Staff must first complete the real refund in RMS/R2S, then record its reference, reason, and Manager Access approval. ROS commits the negative payment and updates the linked RMS Charge record(s) together, including split RMS tenders.
   - **Open Deposit:** cannot be selected as a generic refund tender. Cancellation/void restores the original held deposit through the Open Deposit ledger.
   - **Tender validation:** the API accepts only the supported refund methods above plus cash, gift card, and store credit. Unknown, donation, or synthetic method strings are rejected before any financial or inventory mutation.
   - **QBO:** every real negative refund tender clears the matching return liability. RMS financing refunds credit the RMS financing clearing account, and Staff Account refunds credit Staff Accounts Receivable; RMS/Staff payment-collection reversals remain separate from merchandise refunds.
   - **Reporting event:** each completed return or exchange stores one immutable event with its exact returned merchandise, new merchandise or services, state tax, local tax, net refund or amount due, tender, and register session. When that settlement produces a customer refund, Daily Sales shows it once as one return/exchange event; opening or reprinting it uses the same event-scoped receipt instead of a separate zero-detail payment row or the full original sale. Z-Reports, refund queues, and QBO read the same event instead of recomputing a return from the current catalog price or the original undiscounted line.
   - **Split refunds:** every transaction type accepts multiple payment methods. The Register submits each refund source with a stable `refund_tender_id` under one `refund_event_id`. The first source may be less than the full itemized return total; later sources consume the remaining open obligation. Replaying a completed source is idempotent, while a failure leaves the remaining obligation open and auditable.
...
3. **UI**
   ...
   - Before recording a replacement Transaction that includes an **Original Card** refund remainder, the Register performs a read-only Helcim preflight against the server-selected linked charge and its current batch. A known blocked partial refund, insufficient linked-card capacity, debit source, missing linked charge, or unverifiable provider lookup stops before the replacement sale, return lines, inventory, exchange credit, or provider refund changes. The canonical refund processor repeats all capacity and batch checks immediately before provider dispatch; the preflight never replaces settlement validation and never writes a provider attempt.
   - After a successful preflight, card-funded exchanges settle the replacement link, returned lines, inventory, and exchange credit first. The remaining linked-card refund then runs through the canonical transaction refund processor. If Helcim changes state or becomes unavailable after preflight, the exchange remains correctly recorded and the unpaid remainder stays open in the refund queue for idempotent retry; ROS does not discard the exchange or send a duplicate card refund.
   - A same-day partial original-card refund cannot use Helcim's open-batch full reversal because that would reverse the retained merchandise too. The Register must issue the exact difference to **Cash**, **Original Card**, **Gift Card**, or **Store Credit** before showing **Sale Complete**. If the read-only Original Card preflight finds that the batch is still open, the exchange is not recorded; staff can wait until the batch closes or choose Cash, Gift Card, or Store Credit. Until the refund is confirmed, ROS shows **Refund Due / Refund pending** and must not print **Settled** or **Complete** for the exchange event.
   - The completion view and event receipt show the exact current return/exchange result. For an approved original-card refund, staff see the provider, approved amount, and masked card before handing the receipt to the customer.
...
- **`POST /api/transactions/{id}/void`**
  ...
  Without `external_helcim_reference`, voiding does not call Helcim or zero the
  original approved card payment. It opens or updates the refund queue and
  returns `pending_refund`; the authorized transaction refund processor later
  performs the linked Reverse/Refund and writes the negative payment ledger only
...
- **Exchange/Return Wizard** (register): [`PosExchangeWizard`](../client/src/components/pos/PosExchangeWizard.tsx) — A high-fidelity, "WowDash" themed workspace for processing post-sale adjustments at the till.
   ...
   - **Salesperson and commission attribution**: Loading the original Transaction Record automatically selects its original salesperson for the replacement Transaction Record. The server re-derives that default from the original record at checkout, and replacement lines without an explicit line override inherit it. Return commission adjustments remain assigned to the salesperson stored on each original returned line.
   ...
   - **Credit basis**: Register return/exchange handoff uses the original selected line subtotal plus the original selected line tax split. The returned value first reduces any positive balance due; only the excess is available as exchange credit or refund, capped by paid money on the Transaction Record. This prevents a partial deposit from being presented as refundable while the original Transaction Record still has an unpaid balance.
   ...
   - **Existing Transaction destination**: staged exchange credit may fund replacement merchandise and/or a positive `order_payments[]` allocation to an existing customer Transaction Record. The Register must not clear that payment when return lines are added. Checkout records the payment movement and target allocation normally; exchange settlement validates the exchange-credit source by the checkout Transaction identity even when the allocation target is the existing Transaction Record.
   - **Blocked-credit support recovery**: `apply_blocked_exchange_recovery_repair` is a preview-first, explicit-confirmation support command for the narrow case where an empty replacement checkout committed internal exchange credit but settlement never completed. It verifies the immutable recovery identity and exact credit allocation, records an offsetting internal payment without touching the processor or inventory, restores the original balance, cancels the empty replacement Transaction Record, resolves the recovery job, and writes Transaction and Staff Access audit evidence.
