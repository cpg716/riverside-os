...
2. **Process cash-out**
   ...
   - **Helcim:** `card_present` and `card_manual` both mean refund to the original linked Helcim card. The server creates a deterministic provider attempt and attempts to resolve the current Helcim V2 transaction ID before calling the provider. For a full refund while the card batch is open, ROS uses Helcim **Reverse**; for a closed/settled batch or a partial refund, ROS uses Helcim **Refund**. ROS commits the negative payment and refund queue update only when Helcim returns approved/captured. Provider request errors, rate limits, or declines persist the failed provider attempt for audit and leave ROS refund state unchanged.
