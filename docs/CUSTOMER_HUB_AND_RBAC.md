...
Many browse/search/create paths use **`require_customer_access`** (signed-in staff or valid POS register session). The customer address suggestion helper (`GET /api/customers/address-suggestions`) follows this same pattern and returns no suggestions rather than blocking customer save when the upstream lookup is unavailable. **Merge** (`POST /api/customers/merge`), **duplicate review queue** (list / enqueue / dismiss), **group membership** changes, and **store credit adjust** are gated by **dedicated permission keys** — see [`server/src/api/customers.rs`](../server/src/api/customers.rs) and [`docs/STAFF_PERMISSIONS.md`](STAFF_PERMISSIONS.md).
