...
For a controlled one-shot repair, keep the normal full source preflight enabled and set `CP_RUN_ONCE_ENTITIES` to the comma-separated write steps approved for that run, for example `tickets,historical_booking_events`. The resulting `fix_rerun` keeps complete Counterpoint source proof while skipping unrelated entity writes. It does not bypass ROS validation, protected-dependency checks, source proof, or import-run audit. When immediately retrying an interrupted run without any source or configuration change, `CP_REUSE_PREFLIGHT_IMPORT_RUN_ID` may reference the exact passed preflight UUID instead of repeating the source scan; ROS still validates that preflight before creating a new audited run.
...
Ticket-history and open-document rows without a valid source booking timestamp are rejected into import review. ROS does not substitute the Bridge run time, because doing so would turn historical activity into false current-day Booked Sales. Legacy backfill timestamp corruption is corrected automatically inside ROS by a numbered migration; it is not reimported and requires no operator workflow. The repair preserves prior evidence and cannot force totals or tenders to match.
...
| `SY_GFC.ORIG_CUST_NO` | `gift_cards.customer_id` through the unique matching `customers.customer_code` |
...
**Imported-card metadata repair:** The repair endpoint has separate `preview` and `apply` modes. Each request declares the complete ROS-matched Counterpoint repair-scope count and balance; zero-balance source rows that were never imported stay outside that scope. Preview reports source-backed `original_value` corrections plus gift-card payments whose saved subtype/card-kind metadata disagrees with the reviewed source classification, their absolute amount, and the QBO activity dates that must be regenerated. Apply runs in one database transaction and aborts if a scoped code is duplicated or missing, a balance changed, an original value is missing or below the current balance, the verified totals differ, or authenticated staff/reason audit context is absent. It updates `card_kind`, liability classification, original value, issue date, expiration, and—when the request requires source customer ownership—the uniquely resolved `customer_id`; it never changes `current_balance`, status, payment amount, or payment allocation. Every original-value correction and matched payment reclassification writes an exact before/after `staff_access_log` entry in the same transaction. When a matched card has no source issue event, the transaction adds one idempotent `issued` audit row from Counterpoint's original value and issue date. Existing events are preserved, repeated repairs do not duplicate issue rows or repair audit, and a missing original value blocks the repair rather than inventing activity. Missing or ambiguous customer numbers likewise block a customer-required repair instead of guessing ownership.

For attended Main Hub maintenance, use the staff-authorized `POST /api/settings/counterpoint-sync/gift-card-metadata-repair/preview` and `/apply` routes. They accept the directly queried Counterpoint source rows and do not depend on the bridge process or its machine token. Send `customer_code` from `ORIG_CUST_NO` and set `require_customer_links: true` when customer ownership is part of the repair. Apply additionally requires the confirmation phrase; the reviewed `would_reclassify_payments` / `payment_repair_digest`; the reviewed `would_repair_original_values` / `original_value_repair_digest`; and a written reason. Any payment or original-value fingerprint change after preview blocks the transaction. The requesting staff member and exact before/after corrections are recorded. After Apply, regenerate the QBO proposal for every returned `qbo_activity_dates_to_regenerate` date after the required Gift Card expense/liability mappings are configured in QuickBooks Settings; do not approve a stale proposal. The machine-authenticated `/api/sync/counterpoint/gift-cards` route remains for unattended imports and cannot supply the staff audit context required to apply a historical metadata repair.
