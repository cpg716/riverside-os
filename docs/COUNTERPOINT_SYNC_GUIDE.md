...
**Imported-card metadata repair:** The repair endpoint has separate `preview` and `apply` modes. Each request declares the complete ROS-matched Counterpoint repair-scope count and balance; zero-balance source rows that were never imported stay outside that scope. Preview also reports gift-card payments whose saved subtype/card-kind metadata disagrees with the reviewed source classification, their absolute amount, and the QBO activity dates that must be regenerated. Apply runs in one database transaction and aborts if a scoped code is duplicated or missing, a balance changed, the verified totals differ, or authenticated staff/reason audit context is absent. It updates `card_kind`, liability classification, issue date, expiration, and—when the request requires source customer ownership—the uniquely resolved `customer_id`; it never changes `current_balance`, `original_value`, status, payment amount, or payment allocation. For matched payments, it corrects only `metadata.sub_type` and `metadata.gift_card_card_kind` and writes an exact before/after `staff_access_log` entry for every changed payment in the same transaction. When a matched card has no source issue event, the same transaction adds one idempotent `issued` audit row from Counterpoint's original value and issue date. Existing events are preserved, repeated repairs do not duplicate the issue row or payment audit, and a missing original value blocks the repair rather than inventing activity. Missing or ambiguous customer numbers likewise block a customer-required repair instead of guessing ownership.

For attended Main Hub maintenance, use the staff-authorized `POST /api/settings/counterpoint-sync/gift-card-metadata-repair/preview` and `/apply` routes. They accept the directly queried Counterpoint source rows and do not depend on the bridge process or its machine token. Send `customer_code` from `ORIG_CUST_NO` and set `require_customer_links: true` when customer ownership is part of the repair. Apply additionally requires the confirmation phrase, `would_reclassify_payments` count, and `payment_repair_digest` returned by the reviewed preview plus a written reason; any payment change after preview blocks the transaction. The requesting staff member and exact before/after payment classifications are recorded. After Apply, regenerate the QBO proposal for every returned `qbo_activity_dates_to_regenerate` date after the required Gift Card expense/liability mappings are configured in QuickBooks Settings; do not approve a stale proposal. The machine-authenticated `/api/sync/counterpoint/gift-cards` route remains for unattended imports and cannot supply the staff audit context required to apply a historical metadata repair.
