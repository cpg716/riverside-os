...
At server startup, an installed `.env` beside `riverside-server.exe` overrides inherited Windows task variables. This keeps the installed configuration authoritative even when Task Scheduler retains stale environment values from an earlier configuration.

Deployment Manager serializes installed environment values with dotenv-safe quoting and escaping. Windows paths and settings-owned credentials therefore retain their exact semantic value for the Rust server, ROSIE tasks, System Audit, and credential repair. During an update, the installer also verifies that the stopped server executable can be opened exclusively before replacing it; a lingering Windows handle fails the update before any server file is overwritten.
