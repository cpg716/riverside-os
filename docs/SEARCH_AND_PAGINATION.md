...
- **Automatic updates:** Meilisearch does **not** subscribe to PostgreSQL or update itself. ROS updates search when specific server write paths spawn an incremental Meilisearch upsert or delete after saving PostgreSQL state. Each completed incremental task advances the persisted index revision; a background verifier then compares the index's live document count with PostgreSQL and marks that exact revision verified. A failed task leaves revisions unequal and fails closed to PostgreSQL. If PostgreSQL cannot be read during an incremental upsert, ROS records the failure and preserves the existing Meilisearch document instead of interpreting the read error as a deletion. As a safety net, when Meilisearch is configured, the API runs one full staged rebuild per store-local day after **3 AM** by default. An incomplete scheduled rebuild gets at most three attempts per scheduled worker run with 30-second then 120-second backoff; the aggregate success marker remains false when all attempts fail. Set **`RIVERSIDE_MEILISEARCH_DAILY_REINDEX_ENABLED=false`** to disable it or **`RIVERSIDE_MEILISEARCH_DAILY_REINDEX_HOUR_LOCAL=0..23`** to change the local hour. PostgreSQL remains authoritative and search falls back to SQL when Meilisearch is unavailable.
- **When a warning is actionable:** A warning identifies its concrete cause: stale full rebuild, revision pending or failed, count mismatch, active indexing, unavailable stats, or connection failure. Refresh only rechecks those facts. Rebuild when the proof remains failed; fix the connection first when live status is unavailable.
