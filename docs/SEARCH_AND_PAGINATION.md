...
- **Credential source:** The key saved in **Settings → Integrations → Meilisearch** is stored as an encrypted integration credential and can override the `RIVERSIDE_MEILISEARCH_API_KEY` fallback from `server/.env`. That saved API key, `RIVERSIDE_MEILISEARCH_API_KEY`, and Docker Compose `MEILI_MASTER_KEY` must all refer to the same live Meilisearch master/API key. If the health panel reports `invalid_api_key`, re-enter the current key in Settings and restart the API if the failure persists.
