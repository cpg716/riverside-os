...
## Interactive search performance contract

- Typed remote searches wait for a short pause before requesting results and cancel superseded requests. A response for an older query must never replace the newest query.
- Customer Hub and Inventory Find retain the last successful rows while the next page or filter result is loading, visibly mark those rows as updating, and replace them only after the current request succeeds.
- High-traffic first pages stay deliberately bounded: Customer Hub loads 100 customers per page in Back Office (25 in POS), Inventory Find loads 200 variation rows, and POS Inventory loads 80 variation rows. Staff can use the existing **Load more** action without losing the active query or filters.
- `GET /api/products/control-board` and its `/api/inventory/control-board` alias accept `include_stats=false` for row-only pickers and search results. The default remains `true`; Inventory summary consumers must keep the default so catalog-wide asset, out-of-stock, vendor, label, and replenishment counts remain authoritative.
- Product Hub loads its product, timeline, and shared reference lists independently with cancellation. ROSIE catalog analysis is optional and starts only when staff select **Analyze product**.
...
- **Stale Protection:** An index is not reported ready when its last successful full rebuild is older than 36 hours, an incremental task failed, its indexed revision differs from the source revision, or a recent background count verification is absent or mismatched. Search never performs a live index-stat request on the staff request path; it uses PostgreSQL until the background proof covers the current revision.
...
- **Automatic updates and self-repair:** ROS pushes incremental Meilisearch upserts/deletes after PostgreSQL writes. Each completed task advances the persisted index revision; a background verifier then compares PostgreSQL and live Meilisearch document counts (including Help Center chunks) and marks that exact revision verified. A failed task remains visibly untrusted instead of being cleared by a later unrelated write. The maintenance worker checks once per minute for an initial-build gap, sticky task failure, revision divergence, verified count mismatch, failed aggregate rebuild, or a full rebuild approaching the 36-hour authority limit. When Meilisearch is healthy and idle, ROS automatically builds temporary indexes, verifies them, and atomically swaps them live. Repair retries are single-flight, bounded to three attempts per run, and held to a 15-minute cooldown after an unsuccessful run. The existing store-local daily rebuild after **3 AM** remains a second layer. Set **`RIVERSIDE_MEILISEARCH_AUTO_REPAIR_ENABLED=false`** only for isolated test/maintenance environments; set **`RIVERSIDE_MEILISEARCH_DAILY_REINDEX_ENABLED=false`** to disable the scheduled daily layer or **`RIVERSIDE_MEILISEARCH_DAILY_REINDEX_HOUR_LOCAL=0..23`** to change its hour.
- **When a warning is actionable:** A warning identifies its concrete cause: stale full rebuild, revision pending or failed, count mismatch, active indexing, unavailable stats, or connection failure. Normal revision/count failures queue automatic repair and need no staff babysitting. Use the manual rebuild only when Support directs it or automatic repair remains unable to complete; a credential/version problem still requires fixing the connection/runtime first.
- **Operations health truth:** A configured or reachable Meilisearch process is not by itself
  ...
  `/api/ready` exposes the same condition and whether automatic staged repair is queued. The
  Settings view refreshes itself every 30 seconds while idle and every 3 seconds during indexing.
...
- **Self-hosted update discipline:** Keep the Community Edition runtime pinned to an explicit Meilisearch version. Current ROS pin: **`getmeili/meilisearch:v1.49.0`** (Windows Main Hub package runtime: **`1.49.0`**). Strict-production startup requires Meilisearch configuration and refuses an unavailable or mismatched runtime. Non-production startup retains a configured client through a temporary outage so its maintenance worker can recover without an API restart. The Windows Main Hub task starts at boot/when available and retries a crashed runtime once per minute with a high bounded restart count. Runtime binaries still update through the governed Main Hub package; search data updates and repairs themselves through ROS. Before changing `getmeili/meilisearch:<version>`, review Meilisearch's update notes for the current-to-target version, stop ROS writes if needed, create a dump for cross-version migration, and preserve a snapshot for same-version recovery. Do not downgrade a reused `/meili_data` directory.
...
**Customer name shorthand:** Multi-token customer searches combine a strict all-terms query with name-only prefix queries for every token. This lets Register, Customer Hub, universal search, and other customer pickers resolve initials and partial names such as `C Garcia`, `Chris Gar`, or `Gar C` while still ranking customers who satisfy every entered name fragment. Meilisearch executes the component searches in one multi-search request. Every PostgreSQL fallback uses the same per-name-token prefix contract, including Customer Hub lifecycle-filter searches; a healthy search index is therefore an acceleration and ranking layer, not a requirement for shorthand to work.

**Product shorthand and mixed fields:** A multi-token product query may combine fragments from different indexed fields. For example, `Ral 409`, `nav bla 42`, or a partial product name plus the beginning of a SKU/catalog number requires every meaningful fragment to match somewhere across SKU, barcode/UPC, product name, brand, variation, or catalog/style number. Meilisearch runs one strict all-terms query plus a prefix/typo-tolerant query for each fragment, intersects those candidate sets, and keeps the strict ranking first. Register supplements its deterministic product-family matching with those verified candidates. Inventory and universal PostgreSQL fallbacks apply the same all-fragment-across-fields rule using literal substring patterns. An empty Meilisearch candidate set falls through to that broader SQL match so mid-word fragments and SQL-only catalog context are not silently hidden.

**ROSIE boundary:** ROSIE may explain results or suggest an optional navigation shortcut, but Gemma does not decide customer identity, product identity, SKU, price, stock, or eligibility. Those searches remain deterministic, typo/prefix aware, and PostgreSQL-hydrated. This avoids hallucinated records and LLM latency while still allowing ROSIE to help staff interpret what to do next.
