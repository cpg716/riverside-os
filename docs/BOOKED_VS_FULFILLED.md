...
Adding merchandise to or removing merchandise from an older open Transaction creates a line-booking event on the store-local amendment date; it does not rewrite the parent Transaction's original booked date. This remains true when Counterpoint supplied the original Transaction: Counterpoint owns the historical booking, while ROS owns the later amendment. Daily Sales and the booked Z-Report net all same-day line additions, removals, and price/quantity/tax adjustments for that Transaction. A positive net change increases that amendment day's Booked Subtotal, while a negative net change reduces it by the signed pre-tax amount. The detail groups both sides as **Order Edited (Increase)**, **Order Edited (Decrease)**, or **Order Edited (No Net Change)** and distinguishes the primary salesperson from the edit operator. A no-tender edit never reports a payment. An Order item cancellation without a matching same-day edit, or another return credit that reduces the summary without a successful refund tender, remains visible once as signed return activity; payment-backed refunds continue to use their one event-scoped refund row.
