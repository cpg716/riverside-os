...
test("shared variation drawer keeps item progress visible and supports back/edit", () => {
  ...
  expect(picker).toContain('isSelectionComplete ? "overflow-hidden" : "overflow-y-auto"');
  expect(picker).toContain('data-testid="variant-pricing-pinpad"');
  expect(picker).toContain("mt-auto shrink-0");
  expect(picker).toContain("rounded-t-2xl border border-b-0");
  expect(picker).toContain("rounded-b-2xl border border-app-border");
