...
const OUTCOMES = [
    ['completed_outside_ros', 'Completed outside ROS'],
    ['cancelled', 'Wedding cancelled'],
    ['not_completed', 'Wedding did not proceed'],
    ['legacy_record', 'Legacy record / outcome already known'],
...
export default function WeddingCloseoutModal({ isOpen, party, onClose, onClosed }) {
    ...
    useEffect(() => {
        ...
        void api.getWeddingCloseoutSummary(party.id)
            ...
            .catch((err) => { if (!ignore) setError(err instanceof Error ? err.message : 'Could not load closeout review.'); });
        return () => { ignore = true; };
    }, [isOpen, party?.id]);
    ...
    const submit = async (event) => {
        ...
        try {
            ...
            setError(err instanceof Error ? err.message : 'Wedding closeout failed.');
        } finally {
            setBusy(false);
        }
    };
        ...
        >
            <form onSubmit={submit} className="w-full max-w-2xl overflow-hidden rounded-2xl border border-app-border bg-app-surface shadow-2xl">
                <div className="flex items-start justify-between border-b border-app-border bg-app-surface-2 px-6 py-5">
                    <div>
                        <p className="text-[10px] font-black uppercase tracking-widest text-amber-600">Manager closeout</p>
                        <h2 id="wedding-closeout-title" className="mt-1 text-xl font-black text-app-text">Close out {party?.trackingLabel || party?.name}</h2>
                        <p className="mt-1 text-xs font-semibold text-app-text-muted">Records the outcome and removes this wedding from active work without fabricating workflow completion.</p>
                    </div>
                    <button type="button" onClick={onClose} className="rounded-full p-2 text-app-text-muted hover:bg-app-border/50" aria-label="Close wedding closeout">
                        <Icon name="X" size={20} />
                    </button>
                </div>
                ...
                <div className="space-y-5 p-6">
                    ...
                    <div>
                        <label className="mb-1 block text-xs font-black uppercase tracking-wide text-app-text-muted">Recorded outcome</label>
                    ...
                    <div>
                        <label className="mb-1 block text-xs font-black uppercase tracking-wide text-app-text-muted">Why is this wedding being closed?</label>
                    ...
                    <div>
                        <label className="mb-1 block text-xs font-black uppercase tracking-wide text-app-text-muted">Additional historical notes</label>
                    ...
                    <section className={`rounded-xl border p-4 ${hasOpenWork ? 'border-amber-300 bg-amber-50' : 'border-emerald-200 bg-emerald-50'}`}>
                        <h3 className="text-xs font-black uppercase tracking-wide text-app-text">ROS records that will be preserved</h3>
                        {!summary ? <p className="mt-2 text-sm font-semibold text-app-text-muted">Reviewing linked records…</p> : (
                            <div className="mt-3 grid grid-cols-2 gap-2 text-sm sm:grid-cols-4">
                                <div><b>{summary.transaction_count}</b><br />Transactions</div>
                                <div><b>{summary.open_transaction_count}</b><br />Open Transactions</div>
                                <div><b>{formatMoney(summary.balance_due)}</b><br />Balance due</div>
                                <div><b>{formatMoney(summary.held_deposit_balance)}</b><br />Held deposits</div>
                                <div><b>{summary.open_fulfillment_line_count}</b><br />Open fulfillment lines</div>
                                <div><b>{summary.scheduled_appointment_count}</b><br />Scheduled appointments</div>
                                <div><b>{summary.open_alteration_count}</b><br />Open alterations</div>
                            </div>
                        )}
                        <p className="mt-3 text-xs font-semibold text-app-text-muted">Closeout does not cancel, fulfill, refund, write off, or alter any linked record. Those records remain available in their owning workspaces.</p>
                    </section>
                    ...
                    {hasOpenWork ? (
                        <label className="flex items-start gap-3 rounded-xl border-2 border-amber-400 bg-amber-50 p-4 text-sm font-bold text-amber-950">
                            ...
                            <span>I understand that open balances and operational records remain open. This closeout only removes the wedding from active wedding tracking.</span>
                        </label>
                    ) : null}
                </div>
                ...
                <div className="flex justify-end gap-3 border-t border-app-border bg-app-surface-2 px-6 py-4">
                    ...
                    <button type="submit" disabled={!summary || busy || reason.trim().length < 10 || (hasOpenWork && !acknowledge)} className="min-h-11 rounded-xl bg-amber-600 px-5 text-sm font-black text-white shadow-sm disabled:cursor-not-allowed disabled:opacity-50">
                        {busy ? 'Closing…' : 'Record & Close Wedding'}
                    </button>
                </div>
            </form>
        </WeddingModalPortal>
