...
import VariationsBuilder, { type GeneratedVariationRow } from "./VariationsBuilder";
...
export default function ProductMasterForm({ onCreated }: ProductMasterFormProps) {
  ...
  const StepIndicator = ({ id, current, label, icon: Icon }: { id: FormStep, current: FormStep, label: string, icon: any }) => (
  ...
  return (
    <div className="flex flex-col gap-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <nav className="flex items-center gap-2 p-2 rounded-[2rem] border border-app-border bg-app-surface-2 self-start">
        <StepIndicator id="shell" current={step} label="Identity" icon={Sparkles} />
        <div className="h-4 w-px bg-app-border" />
        <StepIndicator id="financials" current={step} label="Revenue" icon={DollarSign} />
        <div className="h-4 w-px bg-app-border" />
        <StepIndicator id="matrix" current={step} label="Configuration" icon={Settings2} />
        <div className="h-4 w-px bg-app-border" />
        <StepIndicator id="review" current={step} label="Validation" icon={CheckCircle2} />
      </nav>
        ...
        <div className="space-y-6">
          ...
          {step === "shell" && (
            <section className="rounded-[2.5rem] border border-app-border bg-app-surface p-8 shadow-sm">
              <div className="mb-6 flex items-center justify-between">
                 <div>
                   <h2 className="text-2xl font-black italic tracking-tighter text-app-text uppercase">Catalog Identity</h2>
                   <p className="text-xs font-bold text-app-text-muted mt-1 uppercase tracking-widest">Base template definition and branding</p>
                 </div>
              ...
              <div className="grid gap-4 md:grid-cols-2">
                <div className="space-y-1.5">
                  <label className="text-[10px] font-black uppercase tracking-widest text-app-text-muted ml-1">Product Name</label>
                  <input
                    value={name}
                ...
                <div className="space-y-1.5">
                  <label className="text-[10px] font-black uppercase tracking-widest text-app-text-muted ml-1">Brand Portfolio</label>
                  <input
                    value={brand}
                ...
                <div className="space-y-1.5">
                   <label className="text-[10px] font-black uppercase tracking-widest text-app-text-muted ml-1">Classification</label>
                   <select
                    value={categoryId}
                ...
                <div className="space-y-1.5">
                  <label className="text-[10px] font-black uppercase tracking-widest text-app-text-muted ml-1">Asset Description</label>
                  <textarea
                    value={description}
          ...
          {step === "financials" && (
            <section className="rounded-[2.5rem] border border-app-border bg-app-surface p-8 shadow-sm">
               <div className="mb-6 flex items-center justify-between">
                 <div>
                   <h2 className="text-2xl font-black italic tracking-tighter text-app-text uppercase">Financial Architecture</h2>
                   <p className="text-xs font-bold text-app-text-muted mt-1 uppercase tracking-widest">Pricing targets and channel visibility</p>
                 </div>
              ...
              <div className="grid gap-6 md:grid-cols-2">
                 <div className="space-y-4">
                    <div className="space-y-1.5">
                      <label className="text-[10px] font-black uppercase tracking-widest text-app-text-muted ml-1 italic">Benchmark Retail (USD)</label>
                      <input
                        value={baseRetail}
                    ...
                    <div className="space-y-1.5">
                      <label className="text-[10px] font-black uppercase tracking-widest text-app-text-muted ml-1 italic">Average Acquisition Cost (USD)</label>
                      <input
                        value={baseCost}
                 ...
                 <div className="space-y-3 bg-app-surface-2 rounded-3xl p-6 border border-app-border">
                    <h4 className="text-[10px] font-black uppercase tracking-widest text-app-text-muted mb-2">Automated Rules</h4>
                    <label className="flex items-start gap-4 p-3 rounded-2xl bg-app-surface border border-app-border/40 cursor-pointer group hover:border-app-accent/40 transition-all">
                      ...
                      <div className="flex flex-col">
                        ...
                        <span className="text-[10px] text-app-text-muted font-bold mt-0.5 leading-tight">Generate alerts in the morning digest when inventory depletes.</span>
                      </div>
                    </label>
                    ...
                    <label className="flex items-start gap-4 p-3 rounded-2xl bg-app-surface border border-app-border/40 cursor-pointer group hover:border-app-accent/40 transition-all">
                      ...
                      <div className="flex flex-col">
                        ...
                        <span className="text-[10px] text-app-text-muted font-bold mt-0.5 leading-tight">Mark all generated SKUs as published in the online boutique.</span>
                      </div>
                    </label>
                 </div>
              </div>
          ...
          {step === "review" && (
            <section className="rounded-[2.5rem] border border-app-border bg-app-surface p-8 shadow-sm">
               <div className="mb-8 flex items-center justify-between">
                 <div>
                   <h2 className="text-2xl font-black italic tracking-tighter text-app-border uppercase">Final Validation</h2>
                   <p className="text-xs font-bold text-app-text-muted mt-1 uppercase tracking-widest">Reviewing {rows.length} SKU definitions for commit</p>
                 </div>
              ...
              <div className="rounded-[2rem] border border-app-border overflow-hidden bg-app-surface-2 shadow-inner mb-8">
                <div className="max-h-[400px] overflow-auto">
                    ...
                    <thead className="sticky top-0 bg-app-surface border-b border-app-border z-10">
                        <tr>
                        <th className="px-6 py-4 font-black uppercase tracking-widest text-app-text-muted opacity-60">SKU Code</th>
                        <th className="px-6 py-4 font-black uppercase tracking-widest text-app-text-muted opacity-60">Matrix Position</th>
                        <th className="px-6 py-4 text-right font-black uppercase tracking-widest text-app-text-muted opacity-60">Initial SOH</th>
                        </tr>
                    </thead>
                    <tbody className="divide-y divide-app-border/40">
                        {rows.map((r, i) => (
                        <tr key={`${r.sku}-${i}`} className="hover:bg-app-surface/50 transition-colors group">
                            <td className="px-6 py-4 font-mono font-black text-app-text group-hover:text-app-accent">{r.sku}</td>
                            ...
                            <td className="px-6 py-4 text-right font-black tabular-nums text-app-text-muted">{r.stock_on_hand}</td>
                        </tr>
                        ))}
                    </tbody>
                    </table>
              ...
              <div className="flex items-center justify-between gap-6 p-6 rounded-3xl bg-emerald-50 border border-emerald-200">
                  <div className="flex items-center gap-4">
                     ...
                     <div>
                        <p className="text-xs font-black uppercase tracking-widest text-emerald-900">Master Form Ready</p>
                        <p className="text-[10px] font-bold text-emerald-700/80">All validations passed. Click below to commit these models to the registry.</p>
                     </div>
                  </div>
                  <button
                    type="button"
                  ...
                  >
                    {busy ? "Committing Models..." : `Sync ${rows.length} Definitions`}
                  </button>
              </div>
        ...
        <aside className="space-y-6">
           <section className="rounded-[2rem] border border-app-border bg-app-surface p-6 shadow-sm">
              <div className="flex items-center gap-2 text-app-accent mb-4">
                 <Info size={18} />
                 <h4 className="text-[10px] font-black uppercase tracking-[0.2em]">Contextual Help</h4>
              </div>
              
              <div className="space-y-1.5 p-4 rounded-2xl bg-app-surface-2 border border-app-border/40">
                 <p className="text-[10px] font-black uppercase tracking-widest text-app-text mb-1">Taxation Logic</p>
                 <div className="flex items-center gap-2">
                    <span className={`h-2 w-2 rounded-full ${categoryBadge?.is_clothing_footwear ? 'bg-emerald-500' : 'bg-app-text-muted'}`} />
              ...
              <div className="space-y-1.5 p-4 rounded-2xl bg-app-surface-2 border border-app-border/40 mt-4">
                 <p className="text-[10px] font-black uppercase tracking-widest text-app-text mb-1">Matrix Engine</p>
                 <p className="text-[10px] font-bold text-app-text-muted leading-relaxed">
                    Constructor creates a Cartesian product of all attributes. 3 Sizes x 3 Colors = 9 SKUs. 
                    Riverside OS automatically generates sequential SKU codes using the template prefix.
                 </p>
              </div>
           </section>
           ...
           <section className="rounded-[2rem] border border-app-border bg-violet-600 p-6 shadow-xl shadow-violet-600/20 text-white">
              <div className="flex items-center gap-2 mb-4 opacity-80">
                 <Layers size={18} />
                 <h4 className="text-[10px] font-black uppercase tracking-[0.2em]">Audit Readiness</h4>
              </div>
              <p className="text-[11px] font-medium leading-relaxed opacity-90">
                Every product creation is logged with the current staff ID and a system correlation ID. 
                Ensure your descriptions are customer-friendly as they will propogate to the Online Store and printed receipts.
              </p>
           </section>
        </aside>
      </main>
