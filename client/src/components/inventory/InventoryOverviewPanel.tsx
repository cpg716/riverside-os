...
const InventoryOverviewPanel = () => {
  ...
  const fetchRecommendations = useCallback(async () => {
    ...
    try {
      ...
      console.error("Stock overview fetch failed:", err);
      toast("Connection error to stock server", "error");
    } finally {
      setLoading(false);
    }
    ...
    <div className="space-y-10 animate-in fade-in slide-in-from-bottom-6 duration-1000">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <DashboardStatsCard
          ...
          icon={BrainCircuit}
          trend={{ value: recommendations.length, label: "Total alerts" }}
        />
        <DashboardStatsCard
        ...
        <DashboardStatsCard
          ...
          icon={Percent}
          trend={{ value: clearances.length, label: "Potential markdowns", isUp: false }}
          color="orange"
        />
      </div>
    ...
    >
      <div className="relative z-10">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-6 mb-8 p-6 bg-app-bg/20 rounded-[2rem] border border-app-border/20 backdrop-blur-sm">
          ...
          <Metric label="Selling Velocity" value={`${rec.daily_velocity.toFixed(2)}/d`} />
          <Metric label="Times Sold" value={`${rec.sale_frequency}`} />
          <Metric
            label="Available"
