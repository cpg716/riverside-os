...
import { LayoutGrid, List, Search, Printer, SlidersHorizontal, ShieldAlert, DollarSign } from "lucide-react";
...
// X, Trash2 removed (ShieldAlert kept for maintenance)
...
export const VariationsWorkspace: React.FC<VariationsWorkspaceProps> = ({
  ...
  const { backofficeHeaders, staffCode: _staffCode } = useBackofficeAuth();
  ...
  const [batchPriceMode, setBatchPriceMode] = useState<"fixed" | "offset">("fixed");
  const [batchPriceSubmitting, setBatchPriceSubmitting] = useState(false);

  // Maintenance State
  const [maintenanceTarget, setMaintenanceTarget] = useState<{ variantId: string; sku: string; type: "damaged" | "return_to_vendor" } | null>(null);
  ...
  const rowAxis = matrixRowAxisKey || variationAxes[0] || detectedAxes[0] || "Attribute";
  const colAxis = matrixColAxisKey || variationAxes[1] || detectedAxes[1] || (detectedAxes.length > 1 ? detectedAxes[1] : null);
  ...
  // API Call Handlers
  const patchVariant = useCallback(async (variantId: string, patch: any) => {
    const isStock = "quantity_delta" in patch;
    const endpoint = isStock ? "stock-adjust" : "pricing";

    const res = await fetch(`${baseUrl}/api/products/variants/${variantId}/${endpoint}`, {
      method: "PATCH",
  ...
  const adjustStock = useCallback(async (variantId: string, delta: number) => {
    const res = await fetch(`${baseUrl}/api/products/variants/${variantId}/stock-adjust`, {
      method: "PATCH",
      headers: { "Content-Type": "application/json", ...apiAuth() },
  ...
  const handleBatchWeb = async (status: boolean) => {
    ...
    try {
      await Promise.all([...selectedIds].map(id => patchVariant(id, { web_published: status })));
  ...
  const handleBatchTrackLow = async (status: boolean) => {
    try {
      await Promise.all([...selectedIds].map(id => patchVariant(id, { track_low_stock: status })));
  ...
  const handleMaintenanceSubmit = async () => {
    ...
    try {
        const res = await fetch(`${baseUrl}/api/products/variants/${maintenanceTarget.variantId}/stock-adjust`, {
          method: "PATCH",
          headers: { "Content-Type": "application/json", ...apiAuth() },
          body: JSON.stringify({ 
            quantity_delta: -qty,
        ...
        toast(`Successfully moved ${qty} to ${maintenanceTarget.type === 'damaged' ? "Damaged" : "RTV"}`, "success");
    ...
    <div className="flex flex-col gap-6 animate-in fade-in duration-500">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div className="flex flex-col gap-1">
            <h2 className="text-xl font-black tracking-tight text-app-text flex items-center gap-3">
                <span className="opacity-40 font-mono text-sm leading-none pt-1">#{productId.slice(0, 8)}</span>
                {productName} 
            </h2>
            <div className="flex items-center gap-3">
                <div className={`flex rounded-xl bg-app-surface shadow-sm border border-app-border p-1 ${viewMode === "grid" ? "ring-1 ring-app-accent/20" : ""}`}>
                    <button
                ...
                <p className="text-[10px] font-black uppercase tracking-widest text-app-text-muted opacity-60">
                    {variants.length} SKU{variants.length !== 1 ? 's' : ''} · {categoryName || "Uncategorized"}
                </p>
                {productTrackLowStock && (
      ...
      {viewMode === "grid" ? (
              ...
              <tr className="sticky top-0 z-30">
                <th className="sticky left-0 z-40 border-b border-r border-app-border bg-app-surface-2/95 backdrop-blur-md p-4 text-left">
                  <div className="flex items-center gap-2 text-[10px] font-black uppercase tracking-widest text-app-text-muted">
                    ...
                    <span>{rowAxis} \ {actualColAxis}</span>
                  </div>
                </th>
                {colKeys.map(ck => (
                  <th key={ck} className="border-b border-app-border bg-app-surface-2/95 backdrop-blur-md p-4 text-center">
                    <span className="text-[11px] font-black uppercase tracking-widest text-app-text">{ck}</span>
                  </th>
                ))}
              </tr>
            </thead>
              ...
              {rowKeys.map(rk => (
                <tr key={rk}>
                  <td className="sticky left-0 z-20 border-b border-r border-app-border bg-app-surface/95 backdrop-blur-md p-4">
                    <span className="text-sm font-black text-app-text">{rk}</span>
                  </td>
                  {colKeys.map(ck => {
                    const v = cellMap.get(`${rk}\0${ck}`);
                    if (!v) return <td key={ck} className="border-b border-app-border bg-app-surface-2/20" />;
                    return (
                      <td key={ck} className="border-b border-app-border p-1.5 align-top">
                        <VariationGridCell
                          variant={v}
                          templateBaseRetail={templateBaseRetail}
                          isOutOfStock={v.stock_on_hand <= 0}
                          isLowStock={v.stock_on_hand > 0 && v.stock_on_hand <= v.reorder_point}
                          hasPriceOverride={!!v.retail_price_override}
                          onUpdateStock={(delta) => adjustStock(v.id, delta)}
                          onUpdatePrice={(cents) => patchVariant(v.id, { retail_price_override: cents ? centsToFixed2(cents) : null, clear_retail_override: cents === null })}
                          onUpdateTrackLow={(next) => patchVariant(v.id, { track_low_stock: next })}
                          onUpdateWeb={(next) => patchVariant(v.id, { web_published: next })}
                          onShowMaintenance={(type) => setMaintenanceTarget({ variantId: v.id, sku: v.sku, type })}
                        />
                      </td>
                    );
                  })}
        ...
        <VariationsList
          ...
          onUpdateVariant={patchVariant}
          onShowMaintenance={(id, sku, type) => setMaintenanceTarget({ variantId: id, sku, type })}
        />
      )}
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 p-4 backdrop-blur-sm animate-in fade-in duration-300">
            <div className="w-full max-w-md rounded-[32px] border border-app-border bg-app-surface p-8 shadow-2xl animate-in zoom-in-95 duration-300">
                <div className="mb-6 flex items-center justify-between">
                    <div className="flex items-center gap-3">
                        <div className={`flex h-12 w-12 items-center justify-center rounded-2xl ${maintenanceTarget.type === 'damaged' ? "bg-red-500/10 text-red-500" : "bg-app-accent/10 text-app-accent"}`}>
                            <ShieldAlert size={24} />
                        </div>
                        <div>
                            ...
                            <p className="text-[10px] font-bold uppercase tracking-widest text-app-text-muted">Variation Maintenance</p>
                        </div>
                    </div>
                </div>
                ...
                <div className="space-y-6">
                    <div className="rounded-2xl border border-app-border bg-app-surface-2 p-4">
                        <p className="text-[10px] font-black uppercase tracking-[0.2em] text-app-text-muted mb-1">Target SKU</p>
                        <p className="font-mono text-sm font-bold text-app-text">{maintenanceTarget.sku}</p>
                    </div>

                    <div className="space-y-2">
                        <label className="text-[10px] font-black uppercase tracking-[0.2em] text-app-text-muted">Quantity to Remove</label>
                        <input
                            type="number"
                            value={mtQty}
                    ...
                    <div className="space-y-2">
                        <label className="text-[10px] font-black uppercase tracking-[0.2em] text-app-text-muted">Note / Reason</label>
                        <textarea
                            value={mtNote}
                            onChange={(e) => setMtNote(e.target.value)}
                            className="ui-input min-h-[100px] w-full p-4 text-sm"
                            placeholder={maintenanceTarget.type === 'damaged' ? "Describe damage..." : "Reason for RTV..."}
                        />
                    </div>
                </div>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/60 p-4 backdrop-blur-sm animate-in fade-in duration-300">
          <div className="w-full max-w-sm rounded-[32px] border border-app-border bg-app-surface p-8 shadow-2xl animate-in zoom-in-95 duration-300">
            <div className="mb-6 flex flex-col items-center text-center">
              ...
              <h3 className="text-xl font-black uppercase tracking-tight text-app-text italic">Batch Pricing</h3>
              <p className="text-[10px] font-bold uppercase tracking-widest text-app-text-muted">Adjusting {selectedIds.size} variants</p>
            </div>
            ...
            <div className="space-y-6">
              ...
              <div className="space-y-2">
                <label className="text-[10px] font-black uppercase tracking-[0.2em] text-app-text-muted">
                  {batchPriceMode === "fixed" ? "New Retail Price" : "Price Offset ($)"}
                </label>
                <div className="relative">
                  <span className="absolute left-4 top-1/2 -translate-y-1/2 font-bold text-app-text-muted">$</span>
                  <input
                    type="text"
                    autoFocus
            ...
            <div className="mt-8 flex gap-3">
              ...
              <button
                ...
                onClick={async () => {
                  ...
                  try {
                    const cents = parseMoneyToCents(batchPriceInput.replace("+", ""));
                    ...
                    const updates = [...selectedIds].map(async (id) => {
                      ...
                      return patchVariant(id, { retail_price_override: centsToFixed2(finalPriceCents) });
                    });

                    await Promise.all(updates);
                    toast(`Batch price ${batchPriceMode === "fixed" ? "updated" : "adjusted"} successfully`, "success");
                    setShowBatchPriceModal(false);
                    setBatchPriceInput("");
                  } catch (e: any) {
                    toast(e.message || "Failed to batch update price", "error");
                  } finally {
      ...
      <BatchCommandBar
        ...
        onBatchStock={() => {
            const q = prompt("Add/Subtract stock for all selected (e.g. +5 or -2):");
            if (!q) return;
            const delta = parseInt(q, 10);
            if (isNaN(delta) || delta === 0) return;
            toast("Applying stock adjustments...", "info");
            Promise.all([...selectedIds].map(id => adjustStock(id, delta)))
                .then(() => toast("Batch stock updated", "success"));
        }}
