...
> = {
  card_terminal: {
    ...
    icon: CreditCard,
    idle:
      "border-2 border-blue-400/80 bg-blue-100 text-blue-950 shadow-sm hover:border-blue-500 hover:bg-blue-50",
    active:
  ...
  cash: {
    ...
    icon: Banknote,
    idle:
      "border-2 border-emerald-500/80 bg-emerald-100 text-emerald-950 shadow-sm hover:border-emerald-600 hover:bg-emerald-50",
    active:
  ...
  check: {
    ...
    icon: ScrollText,
    idle:
      "border-2 border-sky-500/80 bg-sky-100 text-sky-950 shadow-sm hover:border-sky-600 hover:bg-sky-50",
    active:
  ...
  on_account_rms: {
    ...
    icon: Landmark,
    idle:
      "border-2 border-amber-500/80 bg-amber-100 text-amber-950 shadow-sm hover:border-amber-600 hover:bg-amber-50",
    active:
  ...
  on_account_rms90: {
    ...
    icon: CalendarDays,
    idle:
      "border-2 border-indigo-500/80 bg-indigo-100 text-indigo-950 shadow-sm hover:border-indigo-600 hover:bg-indigo-50",
    active:
  ...
  gift_card: {
    ...
    icon: Gift,
    idle:
      "border-2 border-fuchsia-500/80 bg-fuchsia-100 text-fuchsia-950 shadow-sm hover:border-fuchsia-600 hover:bg-fuchsia-50",
    active:
  ...
  store_credit: {
    ...
    icon: Wallet,
    idle:
      "border-2 border-violet-500/80 bg-violet-100 text-violet-950 shadow-sm hover:border-violet-600 hover:bg-violet-50",
    active:
...
export default function NexoCheckoutDrawer({
  ...
  const tenderTabIds = useMemo(() => {
    ...
    return base;
  }, [allowStoreCredit, rmsPaymentCollectionMode]);
      ...
      if (tab === "card_terminal") {
        ...
        try {
          const res = await fetch(`${baseUrl}/api/payments/intent`, {
            ...
            body: JSON.stringify({ amount_due: centsToFixed2(amtCents) }),
          });
          if (!res.ok) {
            if (allowOfflineCardSim) {
            ...
            throw new Error(apiMsg || "Card terminal could not be started");
          }
          const data = (await res.json()) as { intent_id?: string };
          ...
          const msg =
            e instanceof Error && e.message
              ? e.message
              : "Could not start card terminal. Check connection and try again.";
          toast(msg, "error");
          pendingStripeCentsRef.current = 0;
        }
        return;
  ...
  const handleStripeSuccess = () => {
    ...
    const meta = TAB_META["card_terminal"];
    setApplied((prev) => [
      ...prev,
      {
        id: newId(),
        ...
        amountCents: amtCents,
        label: meta.label,
        metadata: { stripe_intent_id: stripeIntent?.intent_id },
      },
    ]);
    setKeypad("");
    setShowStripeSimulation(false);
        ...
        <div className="no-scrollbar min-h-0 flex-1 overflow-y-auto overscroll-y-contain pb-2 pt-0.5">
          ...
          <div className="rounded-2xl border border-app-border bg-app-surface p-3 shadow-sm">
            ...
            <p className="ui-type-instruction-muted mt-1.5 hidden text-xs leading-snug md:block">
              Totals match POS line math (NYS Pub. 718-C: eligible clothing/footwear
              under $110 may shift state vs local on the receipt).
            </p>
            {allowDepositKeypad ? (
              <div className="mt-2 border-t border-app-border pt-2">
                ...
                <p className="ui-type-instruction-muted mt-1 text-xs leading-snug">
                  ...
                  <span className="font-semibold ui-caution-text">Apply deposit</span>{" "}
            ...
            {hasLaterItems && remainingCents <= 0 && (
              <div className="mt-2 border-t-2 border-emerald-500/30 bg-emerald-50/50 p-2.5 dark:bg-emerald-950/20">
                <label className="flex cursor-pointer items-center justify-between gap-3">
                  ...
                  <div 
                    onClick={() => onPickupConfirmedChange?.(!pickupConfirmed)}
                  ...
                  >
                    <div className={`absolute left-1 top-1 h-4 w-4 transform rounded-full bg-white shadow-sm transition-transform duration-200 ease-in-out ${pickupConfirmed ? "translate-x-5" : "translate-x-0"}`} />
                  </div>
                </label>
              </div>
            )}
          ...
          <div className="isolate grid shrink-0 grid-cols-1 gap-2 sm:grid-cols-[minmax(220px,1fr)_11rem] sm:gap-3 lg:grid-cols-[minmax(260px,1fr)_12rem]">
            <div className="flex min-w-0 flex-col space-y-2">
              <div className="group relative shrink-0">
                ...
                <div
                  className="flex h-[3.75rem] items-center justify-end rounded-xl bg-app-surface-2 px-4 ring-2 ring-app-border transition-all group-focus-within:bg-app-surface group-focus-within:ring-app-accent sm:h-[4.25rem]"
                >
                ...
                {tab === "cash" &&
                  ...
                  remainingCents > 0 && (
                    <div className="mt-1 flex justify-end text-[10px] font-black uppercase tracking-widest text-emerald-600">
                      Change due: $
                      {centsToFixed2(keypadCents - remainingCents)}
                    </div>
                  )}
              </div>
