...
interface ProductConfirmDrawerProps {
...
export default function ProductConfirmDrawer({
  variantId,
  onClose,
  onAddToSale,
}: ProductConfirmDrawerProps) {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const res = await fetch(`${baseUrl}/api/inventory/snapshot/${variantId}`, { headers });
        if (!res.ok) throw new Error("Failed to fetch product snapshot");
