...
import ProductConfirmDrawer from "./ProductConfirmDrawer";
...
import { type WeddingMember } from "./types";
...
import { CartLedger } from "./cart/CartLedger";
import { CartKeypad } from "./cart/CartKeypad";
import { CartToolRow } from "./cart/CartToolRow";
import { CartCheckoutButton } from "./cart/CartCheckoutButton";
...
export default function Cart({
  ...
  const [weddingDrawerIsSelectionMode, setWeddingDrawerIsSelectionMode] =
    useState(false);
  ...
  const [openDepositPrompt, setOpenDepositPrompt] =
    useState<OpenDepositPrompt | null>(null);
  ...
  useEffect(() => {
    ...
    fetch(`${baseUrl}/api/discount-events/active`, {
      ...
      .then((data) => {
        if (!cancelled)
          setActiveDiscountEvents(Array.isArray(data) ? data : []);
      })
      .catch(() => {});
    return () => {
      cancelled = true;
  ...
  const {
    ...
    apiAuth,
    onNeedsSignIn: () => {
      // Primary trigger is now !checkoutOperator state
    },
  });
  ...
  const openLineProductBrowser = useCallback((line: CartLineItem) => {
    setConfirmVariantId(line.variant_id);
  }, []);
  ...
  const verifySalePin = useCallback(async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (res.status === 404) {
          setSalePinError(
            "Authentication service endpoint not found (404). Contact support.",
          );
  ...
  const commissionStaff = useMemo(
    () =>
      posStaffList.filter(
        (s) => !s.role || s.role === "salesperson" || s.role === "admin",
      ),
    [posStaffList],
  );
      ...
      const next = prev.map((l) => {
        if (activeWeddingMember) {
          if (l.fulfillment === "special_order") {
            changed = true;
            return { ...l, fulfillment: "wedding_order" as FulfillmentKind };
          }
          return l;
        }
        if (l.fulfillment === "wedding_order") {
          changed = true;
          return { ...l, fulfillment: "special_order" as FulfillmentKind };
        }
        return l;
      });
  ...
  useEffect(() => {
    ...
    const run = async () => {
      ...
      setActiveWeddingMember({
        ...
        received: Boolean(wm.received),
        fitting: Boolean(wm.fitting),
        pickup: Boolean(wm.pickup),
  ...
  return (
    <div className="relative flex h-full w-full overflow-hidden bg-app-bg">
      ...
      <div className="relative z-0 flex min-w-0 flex-1 flex-col border-r border-app-border">
        <div className="shrink-0 border-b border-app-border bg-app-surface px-3 py-2 shadow-sm sm:px-4 lg:px-6 lg:py-3">
          <div className="space-y-2 rounded-2xl border border-app-border/90 bg-gradient-to-br from-app-surface via-app-surface to-app-surface-2/40 p-2.5 shadow-[0_14px_48px_-24px_rgba(0,0,0,0.18)] ring-1 ring-black/[0.04] dark:from-app-surface dark:via-app-surface dark:to-app-surface-2/25 dark:ring-white/[0.06]">
            ...
            <CartToolRow
              activeWeddingMember={activeWeddingMember}
              onWeddingClick={() => setWeddingDrawerOpen(true)}
              onExchangeClick={() => setExchangeWizardOpen(true)}
              onLayawayToggle={() => {
                setLines((prev) =>
                  prev.map((l) => ({
                    ...l,
              ...
              isLayawayActive={lines.some((l) => l.fulfillment === "layaway")}
              onGiftCardClick={() => setGiftCardLoadOpen(true)}
              onParkSaleClick={() => {
                const label = selectedCustomer
                  ? `Sale for ${selectedCustomer.first_name} ${selectedCustomer.last_name}`
                  : "Untitled Sale";
                const res = window.prompt(
                  "Enter a label for this parked sale:",
                  label,
                );
                if (res !== null) void parkSale(res);
              }}
              onClearSaleClick={() => setShowClearConfirm(true)}
              onOptionsClick={() => setOrderReviewOpen(true)}
              onOrdersClick={() => setOrderLoadOpen(true)}
          ...
          {disbursementMembers.length > 0 && (
            <div className="space-y-3">
              ...
              {disbursementMembers.map((m) => (
                <div
                  key={m.id}
                  className="relative flex items-center justify-between gap-4 rounded-3xl border-2 border-blue-100 bg-blue-50/30 p-5 group animate-in slide-in-from-left duration-300"
                >
                  ...
                  <div className="text-right">
                    <p className="text-[9px] font-black uppercase tracking-widest text-app-text-muted opacity-60 mb-1">
                      Applying Amount
                    </p>
      ...
      <aside
        className={`relative z-0 flex h-full w-[clamp(332px,25vw,376px)] min-w-[clamp(332px,25vw,376px)] max-w-[clamp(332px,25vw,376px)] shrink-0 flex-none overflow-hidden border-l border-app-border/80 bg-gradient-to-b from-app-surface via-app-surface-2/25 to-app-bg shadow-[-8px_0_32px_-12px_rgba(0,0,0,0.12)] ${checkoutDrawerOpen ? "pointer-events-none select-none opacity-40" : ""}`}
        aria-label="Customer, sale totals and keypad"
      >
        {/* ── Customer selector (payment rail) ── */}
        <div className="shrink-0 border-b border-app-border/60 px-2 pb-1.5 pt-1.5">
          <CustomerSelector
            variant="posStrip"
        ...
        <div className="shrink-0 px-2 pt-1.5">
          <CartLedger
            ...
            isRmsPaymentCart={isRmsPaymentCart}
            sessionId={sessionId}
            isGiftCardOnlyCart={isGiftCardOnlyCart}
            activeDiscountEvents={activeDiscountEvents}
            selectedDiscountEventId={selectedDiscountEventId}
            setSelectedDiscountEventId={setSelectedDiscountEventId}
            applyDiscountEventToSelectedLine={applyDiscountEventToSelectedLine}
            selectedLineKey={selectedLineKey}
            managerMode={managerMode}
            hasAccess={hasPermission("register.void_all")}
            linesLength={lines.length}
            clearCart={clearCart}
            setCheckoutOperator={setCheckoutOperator}
            setSalePinCredential={setSalePinCredential}
            setSalePinError={setSalePinError}
            setShowVoidAllConfirm={setShowVoidAllConfirm}
            toast={toast}
            posShipping={posShipping}
            setShippingModalOpen={setShippingModalOpen}
            setPosShipping={setPosShipping}
          />
        </div>
        ...
        <CartKeypad
          ...
          onKeypadKey={handleNumpadKey}
        />

        <div className="shrink-0 border-t border-app-border/70 bg-app-surface/95 p-2 shadow-[0_-10px_40px_-18px_rgba(0,0,0,0.15)] backdrop-blur-sm">
          <CartCheckoutButton
            collectTotalCents={totals.totalCents}
            onCheckoutClick={() => {
              ...
              prev.map((l) => ({
                ...
                fulfillment:
                  l.fulfillment === "takeaway"
                    ? ("special_order" as const)
                    : l.fulfillment,
              })),
            );
            toast(
              "Switched takeaway items to Special Order for shipping.",
              "info",
            );
          }
        }}
      ...
      <NexoCheckoutDrawer
        ...
        onOpenSplitDeposit={() => {
          setWeddingDrawerIsSelectionMode(true);
      ...
      <WeddingDetailDrawer
        ...
        onClose={() => {
          ...
          setWeddingDrawerIsSelectionMode(false);
        }}
        memberId={activeWeddingMember?.id}
        isSelectionMode={weddingDrawerIsSelectionMode}
        onAddDisbursements={(ms) => {
          setDisbursementMembers((prev) => {
            // Prevent duplicates
            const existingIds = new Set(prev.map((p) => p.id));
            const uniqueMs = ms.filter((m) => !existingIds.has(m.id));
            return [...prev, ...uniqueMs];
          });
          setWeddingDrawerOpen(false);
          setWeddingDrawerIsSelectionMode(false);
        }}
      ...
      {confirmVariantId && (
        <ProductConfirmDrawer
          variantId={confirmVariantId}
          onClose={() => setConfirmVariantId(null)}
      ...
      <PosSaleCashierSignInOverlay
        ...
        onCancel={() => {
          if (onCancelSignIn) onCancelSignIn();
          else onExitPosMode?.();
        }}
      />
            ...
            onSelectOrder={(order, mode) => {
              void (async () => {
                try {
                    ...
                    addItem(
                      {
                        ...
                        standard_retail_price: parseMoneyToCents(
                          item.unit_price,
                        ),
                        unit_cost: 0,
                        state_tax: 0,
                  ...
                  toast(
                    `Order ${order.id.slice(-6)} recalled for ${mode}`,
                    "success",
                  );
                } catch (e) {
                  toast(
            ...
            onComplete={(options) => {
              setCheckoutOrderOptions({
                ...
                stripe_payment_method_id:
                  options.storeCardForBalance?.stripe_payment_method_id || null,
              });
