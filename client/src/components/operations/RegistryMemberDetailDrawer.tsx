...
import type { RegistryActionRow } from "../../lib/registryPriorityFeed";
...
interface RegistryMemberDetailDrawerProps {
  row: RegistryActionRow | null;
  onClose: () => void;
  onOpenFullParty: (partyId: string) => void;
}

export default function RegistryMemberDetailDrawer({
  row,
  onClose,
  onOpenFullParty,
}: RegistryMemberDetailDrawerProps) {
  ...
  const load = useCallback(async (r: RegistryActionRow) => {
    setLoading(true);
    setErr(null);
    try {
                      ...
                      try {
                        await weddingApi.updateMember(
                          member.id,
                          {
                            ...
                            activity_description: "Retail sizing updated (Priority Feed)",
                          },
