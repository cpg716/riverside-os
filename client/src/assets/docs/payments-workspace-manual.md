...
6. If **Recover Payment** cannot determine a final provider result, use **Restore → Release from sale** and confirm the warning. ROS releases only the sale lock; it does not call the attempt declined or canceled, and the provider evidence remains in Payments Health. Continue with another permitted tender only after explaining that the original card result is still unknown, and do not retry that card until it is verified.

The Card Not Present flow must never be routed to a physical terminal. A successful approval is not complete in Riverside until the payment appears in the checkout ledger and the resulting Transaction Record shows the Helcim provider reference. After the approval is attached—or provider recovery proves a definitive decline/cancel—completing or clearing the sale resets the hosted handoff. A later exact approval automatically releases older unresolved hosted attempts from sale-blocking status. Any unresolved result stays visible in Payments Health but does not lock the current or a later sale, and a late signed confirmation can still promote it to an approval for recovery.
...
- Approved provider payments cannot be removed, parked, or transferred to another customer. **Clear Sale**, another tender, and **Ready to Save** remain unavailable while a current card request is actively processing or while an approval is still unattached. For an unresolved Card Not Present request, use **Restore** directly in Pay to recover the result, reopen secure entry, or explicitly **Release from sale** while retaining the attempt for Payments Health review. For a physical reader, confirm cancellation only after the reader itself ended without approval. Payments Health remains the historical review surface; staff do not leave the current sale to resolve its blocker.
