...
- `{{CUSTOMER_LINE}}` prints the customer name, phone, and Customer # when present. `{{ITEM_LINES}}` groups merchandise as Taken Today, PICKED UP, SHIPPED, Special Order, Custom Order, Wedding Order, or Layaway and includes product title, SKU, variation, quantity when greater than one, order date for pickup lines, line pricing, discount comparison when applicable, and three compact saved tax rows: **4.75%**, **4.00%**, and **Total Tax**. Zero components remain visible, so clothing below the state threshold shows $0.00 for 4.00%, full-tax merchandise shows both amounts, and non-taxable lines show $0.00 for all three rows. A Wedding Order group also prints the Party Name and Wedding Date directly under its heading. Pickup receipts only print the items picked up in that pickup event.
