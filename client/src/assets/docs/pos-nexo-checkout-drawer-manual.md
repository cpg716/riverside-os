...
- **Card reader** sends the payment directly to the selected Helcim terminal. Riverside does not run the Settings connection ping before a live payment because that diagnostic displays a message on the reader and can interrupt its ready state. Each terminal request is bound to the sale that opened it. An approval from another sale never appears as tender for the current customer. Select **Restore** beside Terminal in Pay to refresh or safely resolve the exact blocking attempt without leaving the sale; do not run the card again.
...
- After every card request is attached or has a server-confirmed final decline/cancel, the preservation ends at the sale boundary. Recording the sale or using an authorized **Clear Sale** then resets every sale-local drawer input and Card Not Present handoff. A live pending attempt that reached secure card entry cannot be released locally or moved to a new checkout/customer; its processor evidence remains in **Payments Health** until recovery resolves it. Only an abandoned pre-entry setup with no Helcim token or transaction evidence is released automatically.
...
If Terminal 1 has a pending request whose checkout reference does not match the open drawer, ROS labels it **Terminal request needs review** instead of assuming it belongs to an earlier sale. ROS automatically checks Helcim without importing that payment into the open sale. A verified final result releases the terminal immediately. If the physical request ended without approval because it was canceled, timed out, or the reader restarted, choose **Reader stopped / no approval** only after confirming the reader is idle. ROS checks Helcim once more, releases the blocked attempt, retains it for reconciliation, and unlocks Card Reader plus every other allowed tender.
...
Staff can resolve the same condition without leaving checkout: select **Recover terminal** in the Payment screen. After the active dispatch window, ROS checks the exact Helcim invoice and clears the reservation only when Helcim returns no matching provider transaction, even if the terminal API originally acknowledged the request. The Payment screen refreshes terminal availability immediately so the customer can retry. If Helcim returns a real request, approval, or an unresolved lookup, ROS keeps it protected and explains the required recovery instead of force-releasing it.
...
If payment or Register state blocks the current sale, select **Restore** in the Pay header. Restore keeps the current cart and presents the safe action for the exact blocker: refresh Helcim health, **Re-verify & Attach** the current approved payment, release a proven evidence-free Riverside reservation, confirm a physical terminal cancellation, reopen the secure Card Not Present entry, select an available terminal, request audited Manager Access for a non-default terminal, use another allowed tender when no card outcome is unresolved, close Pay to rejoin the Register, or continue with **Record Sale** once the ledger is ready. The cashier does not need to leave Pay for a current-sale blocker.
...
**Reader stopped / no approval** is available only for a physical terminal request, never for Card Not Present. It appears directly in Payment Status and in Restore so the cashier does not have to leave the sale. Use it only when the reader is idle and the request definitively ended without approval. After the audited release, Card Reader is ready to run again and Manual Card, cash, check, and every other allowed tender are unlocked. A request owned by another Register cannot be force-released. Riverside retains the attempt, Register, checkout reference, acting staff member, and restore action in audit history. An approved or provider-evidenced payment is never discarded, moved to another customer, or bypassed with a second tender.
