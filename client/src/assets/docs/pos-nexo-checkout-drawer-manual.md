...
A checkout-reference mismatch or stale local ROS row alone does not reserve a terminal. After the short in-flight window, ROS checks the exact Helcim invoice and automatically expires an orphaned local reservation only when Helcim returns no matching payment evidence and ROS has no provider payment ID, transaction ID, dispatch audit reference, or unknown-outcome error. Verified pending requests and approvals remain protected.

Staff can resolve the same condition without leaving checkout: select **Recover terminal** in the Payment screen. ROS checks the exact Helcim invoice and clears the reservation only when it proves the row is an evidence-free ROS error. The Payment screen refreshes terminal availability immediately so the customer can retry. If Helcim returns a real request or approval, ROS keeps it protected and explains the required recovery instead of force-releasing it.
