...
**I Canceled Terminal** is available only for a physical terminal request, never for Card Not Present. A request owned by another Register cannot be force-released; Restore instead offers an available terminal or another tender. Riverside retains the attempt, Register, checkout reference, acting staff member, Manager Access approval, and restore action in audit history. An approved or provider-evidenced payment is never discarded, moved to another customer, or bypassed with a second tender.
