...
If Terminal is blocked by a request from this Register, select **Restore** in the Pay header. The Restore panel shows the exact attempt and provides the safe action available now: **Recover Payment** checks Helcim and attaches a matching approval; **Restore Terminal** releases only a proven evidence-free Riverside reservation; **I Canceled Terminal** is used only after the physical terminal confirms cancellation. Riverside retains the attempt, Register, checkout reference, acting staff member, and restore action in audit history. A request owned by another Register cannot be force-released here.
