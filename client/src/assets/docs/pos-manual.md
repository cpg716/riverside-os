...
The action grid below search keeps the most common workflows visible in this order: **Customer Orders**, **Wedding Manager**, **Return / Exchange**, **Start Alteration**, **More Actions**, and **Clear Sale**. Keeping Clear Sale beside More Actions separates the reset control from the customer and wedding workflows. **Wedding Manager** is the only Register toolbar path for party deposits and member orders; the selected Customer's Wedding Party panel in the side rail can also start that member's order directly. The old generic Wedding Sale lookup was removed because it did not create a member order and duplicated the party selector. More Actions is a compact four-by-two grid containing Suit Swap, Custom Order, Gift Card loads, RMS or Staff Account payments, Layaway, Order options, and Park Sale. Selecting a More action closes that panel before the selected workflow opens, so closing the workflow returns directly to the Cart.
...
**Parked Sales** remains visible above product search even when the count is zero. Its count shows unfinished sales available on the current Register; select it to review, recall, or remove a parked sale without opening customer search first.
