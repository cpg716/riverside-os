...
The action grid below search keeps the most common workflows visible, with **Customer Orders**, **Clear Sale**, **Wedding Manager**, **Start Alteration**, and **Return / Exchange** available before **More Actions**. **Wedding Manager** is the only Register toolbar path for party deposits and member orders; the selected Customer's Wedding Party panel in the side rail can also start that member's order directly. The old generic Wedding Sale lookup was removed because it did not create a member order and duplicated the party selector. More Actions is a compact four-by-two grid containing Suit Swap, Custom Order, Gift Card loads, RMS or Staff Account payments, Layaway, Order options, and Park Sale. Selecting a More action closes that panel before the selected workflow opens, so closing the workflow returns directly to the Cart.
