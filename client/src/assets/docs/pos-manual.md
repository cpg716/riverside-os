...
The action grid below search keeps the most common workflows visible, with **Customer Orders** first, followed by **Start Wedding Sale**, **Collect Deposit**, **Start Alteration**, and **Return / Exchange**. The colors reinforce purpose: blue for customer records, Riverside fuchsia for Wedding Sale, violet for deposits, amber for alterations, and red for returns or exchanges. Choose **More Actions** for Custom Order, Gift Card loads, RMS or Staff Account payments, Layaway, Order options, Park Sale, and Clear Sale. Selecting a More action closes that panel before the selected workflow opens, so closing the workflow returns directly to the Cart. Nothing is hidden off-screen in a horizontal action ribbon.
...
When only some unfulfilled Order items are being dropped, open **Orders** in the POS navigation, open the customer's Order, and select **Cancel Order Items** beside the **Items** heading. You can also select **Cancel Ordered Item** beside one eligible line. Select the open lines, enter a specific reason, and choose **Review Financial Effect**. Riverside shows the cancellation credit, how much reduces the unpaid balance, the revised Order total, any balance still due, and the actual refund due. The item value itself is not automatically cash back. If payments exceed the revised total, confirmation creates an auditable refund obligation and moves staff to Pay, where the refund may use multiple valid sources and Original Card still requires verified Helcim approval. If no overpayment exists, no refund is sent. Fulfilled merchandise remains in Return / Exchange. Vendor-, PO-, receiving-, or ready-for-pickup lines must have that commitment resolved before they can be cancelled.
