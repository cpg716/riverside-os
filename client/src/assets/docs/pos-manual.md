...
4. On **Order / Layaway / Wedding** sales, any money paid before pickup is treated as a deposit even if you only use **Apply payment**. Use **Apply deposit** when you want the ledger to set a specific deposit target first. To collect deposits for other wedding members, first select the paying customer and then choose **Wedding Deposit** from the Cart action toolbar. Riverside opens that customer's wedding party directly in deposit mode. Select each beneficiary, enter the amount, and choose **Add Deposits to Cart**. Each deposit is a removable Cart row, and the Cart may contain only wedding deposits—merchandise is not required before continuing to **Payment**. The paying member is identified as **Payer** and cannot be selected as their own beneficiary. When you later select a member who has money held this way, a **Wedding deposit available** popup shows the balance and most recent payer. Open **Pay** and select **Apply $X** on the wedding-deposit card to add it to this member's sale, including in-stock **Takeaway** items that the member takes today. A newly entered deposit target cannot fund takeaway merchandise. Clear new party deposits or existing-order payment rows before applying the member's held deposit.
   - After Split deposit completion, Daily Sales shows the amount applied to the payer's own Transaction separately from **Wedding Deposits Placed** and **Total Tender Collected**. The payer's Customer History also records how much was placed for the party members. Sale Complete and the payer receipt identify each separate amount as **Wedding Party Deposit (party name)** so the collected tender is explained without treating the deposit as merchandise.
