...
3. The **Payment ledger** side panel opens. Enter amounts on the keypad, then **Apply payment** for each tender (card, cash, gift card, etc.) the way you were trained.
   - **Card Not Present** opens secure HelcimPay.js card entry for keyed payments. Riverside OS validates the Helcim approval response and does not store card numbers or CVV. On the desktop app, Riverside opens the one-time public HTTPS handoff outside the checkout drawer and attaches the approved payment back to the sale.
