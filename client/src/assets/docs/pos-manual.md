...
3. **Scan a barcode**, choose **Camera** beside product search, or **type a SKU** and press **Enter**. Camera scanning uses the device's rear camera when available and stays open for additional items. Exact SKUs, product barcodes, and active barcode aliases add the verified variation directly; close the scanner to review an unknown, name-only, or ambiguous match in product search. SKU-style entries such as `B-1626170`, `I-103881`, `CP-100001`, or `ROS-1001` must match exactly; if the SKU is missing, the Register shows **SKU NOT FOUND** instead of offering similar SKUs.
4. For parent-style searches, combine partial fragments from different fields. Part of the product or brand name plus the beginning of a style/SKU number can match the same item, and familiar terms such as `40901/1 suit`, `40901/1 slacks`, or `40901/1 blazer` remain supported. Riverside requires every meaningful fragment and ranks exact matches first.
...
The action grid below search keeps the most common workflows visible in this order: **Customer Orders**, **Wedding Manager**, **Return / Exchange**, **Start Alteration**, **More Actions**, and **Clear Sale**. Keeping Clear Sale beside More Actions separates the reset control from the customer and wedding workflows. **Wedding Manager** is the only Register toolbar path for party deposits and member orders; the selected Customer's Wedding Party panel in the side rail can also start that member's order directly. The Cart no longer repeats the selected Customer or Wedding identity already shown in that side rail. Staff, Salesperson, and Store Date/Time remain locked in the top strip. More Actions is a compact three-by-three grid containing Suit Swap, Custom Order, Gift Card loads, RMS or Staff Account payments, Layaway, Order options, **Resume Parked Sale**, and Park Sale. Selecting a More action closes that panel before the selected workflow opens, so closing the workflow returns directly to the Cart.

On a portrait tablet or phone, the Register keeps the Cart full-width and shows **Customer & Pay** at the bottom. Choose it to open the Customer, totals, keypad, and Pay workspace; choose **Back to cart** to return to scanning and line review. Desktop and landscape workstation layouts keep Cart and payment controls visible side by side.

Choose **More Actions → Resume Parked Sale** to open the current Register's parked-sale picker. The picker shows Customer, draft reference, item count, and subtotal; choose **Resume Sale**, delete a parked sale, or close the popup without changing the Cart.
...
- When an eligible discount event is applied, Riverside uses the SKU's configured sale price when one exists; otherwise it calculates the event percentage from retail. If the displayed sale price looks wrong, stop and have the SKU reviewed in Product Hub before recording the sale.
...
2. If a customer is loaded, choose from that customer's transaction list. Otherwise scan the receipt barcode or search by transaction number.
   Receipt lookup includes completed sales so fulfilled merchandise remains available for return or exchange. Long transaction details wrap inside each row; review the transaction number, date, item count, and amount before selecting it.
...
When only some unfulfilled Order items are being dropped, open **Customer Orders** from the Register toolbar, choose **View Order Details**, and select **Cancel Item** directly below **Update Item**. Enter a specific reason and choose **Review Balance & Refund**. Riverside shows the item credit, how much reduces the unpaid balance, the revised Order total, any balance still due, and the actual refund due. The item value itself is not automatically cash back. If payments exceed the revised total, confirmation creates an auditable refund obligation and moves staff to Pay, where the refund may use multiple valid sources and Original Card still requires verified Helcim approval. If no overpayment exists, no refund is sent. The Register keeps cancelled items visible in the Transaction Record with a **Cancelled** status so they do not look deleted. Fulfilled merchandise remains in Return / Exchange. Received or Ready-for-Pickup goods stay physically on hand while their customer reservation is released; Layaway holds return to available inventory. Existing vendor/PO commitments remain recorded for receiving follow-up.
...
3. The **Payment ledger** side panel opens. The compact top strip shows the terminal status. The payment types, amount/keypad workspace, and payment ledger each scroll independently when needed, above the fixed **Complete Sale** controls. Enter amounts on the keypad, then **Apply payment** for each tender (card, cash, gift card, etc.) the way you were trained.
   ...
   - **Cash** accepts the amount handed to you and shows the change due before checkout. When cash exceeds the sale amount, its Payment ledger entry separately shows **Payment** (cash received), **Paid to Sale Amount**, and **Change Due**. After checkout, Sale Complete repeats **Change due** in a large high-contrast panel beneath Receipt Contact. Give that amount back before beginning the next sale. The customer receipt prints both **Cash Tendered** and **Change**.
...
4. On **Order / Custom / Layaway / Wedding** Transactions, the compact Payment status strip shows **DEPOSIT MET** in green, or **NEED 25% DEPOSIT** / **NO DEPOSIT** in red. **NO DEPOSIT** requires the inline **Manager Access** action before Record Sale can continue. Any payment that leaves a balance is called and recorded as a **Deposit**; a payment that closes the balance is **Payment in Full**. The Manager approval is limited to that exact customer and checkout and creates no fake payment. Takeaway merchandise must still be paid in full. To collect deposits for other wedding members, select the paying customer and choose **Wedding Deposit** from the Cart action toolbar. Start by choosing **Deposit Only** or **Collect & Build Orders**. The guided workspace then makes you choose or start the party, verify the payer's party role, select each beneficiary and amount, choose **Hold for this member's future order** or one exact open Transaction Record, and review the full allocation before adding it to the Cart. For equal deposits, enter **Deposit amount per selected member** once and tap only the intended members; newly selected members receive that amount automatically, **Apply to Selected** updates the current selection, and individual amounts remain editable. Unselected members receive nothing. A selected `$0` member is clearly excluded and does not block valid funded members. You can start a party with only **Party Name** and **Wedding Date**, then link an existing customer or create a customer and assign a role without leaving the workflow. Each reviewed deposit remains a removable Cart row, and the Cart may contain only wedding deposits—merchandise is not required before continuing to **Payment**.
   ...
   - After the payer payment is approved, the completion heading is **Deposit complete · orders pending** until the prepared member drafts are posted. The funded workflow and every reviewed member draft are saved together on Main Hub with a stable retry identity; a later Deposit Only build saves each member draft before advancing. Choose **Continue to Member Transactions**, then **Create All Member Transactions** once. Closing that completion screen or leaving it unattended opens the same required final review instead of silently returning to an empty Cart. Riverside creates the separate member-owned Transactions and Wedding Fulfillment Orders through the normal atomic checkout service, applies each exact held source, and never asks for another payer payment. **Wedding Builder Complete** appears only after a fresh Main Hub check confirms every required member Transaction. The completion screen shows every member's exact items/variations, salesperson, tax treatment, Transaction total, deposit applied, balance due, and individual **View / Print Receipt**. If one member fails, successful members remain posted once and retry resumes only the unfinished rows with their original checkout identities.
...
At checkout, choose **RMS Charge**, explicitly select the touch-sized **Standard** or **90 Day** plan, enter the RMS approval number, and select **Add Payment**. Riverside does not preselect a plan and will not save an RMS Charge until one is chosen; the choice determines the tender reported on Daily Sales and the Z-Report. The compact account summary shows the masked account, available credit, current balance, and source. If the weekly account list shows **No Open to Buy**, Riverside keeps both plan choices available because RMS is the approval source, but **Manager Access** is required before the status changes to **Ready to Save**. Confirm the charge in RMS, record its approval number, and complete the on-screen Manager Access approval.

When Register search opens a parent product with variations, Riverside shows the full variation matrix for that parent in one grouped selection surface. **Item to Build** stays at the top with the product and every option already chosen. Choose the large option labels one step at a time. Options sort alphabetically for text, numerically for number-based sizes, and in merchandise order for apparel aliases such as **SMALL**, **MED**, **LG**, **XL**, and **2XL**. Gruppo Bravo vest sizes use the sellable labels **SMALL (36-38)**, **MEDIUM (40-42)**, **LARGE (44-46)**, **X-LARGE (48-50)**, and **2XL (52-54)**; Gruppo Bravo suit and tuxedo sizes retain their numeric size and length. Use the clearly labeled **Back** button to return one step, or select any completed option under **Item to Build** to edit from that point. These controls remain available on the final confirmation/pricing screen before **Add to Sale** or **Update Item**. The final step condenses the completed options into one fixed view with no scrolling or repeated product summary. **Line pricing** is attached directly above the full-size PIN pad as one calculator surface. Before an adjustment the price appears once; after an adjustment Riverside shows both **Regular unit price** and **Final unit price** for review. Reset the adjustment to restore regular price. At the first step, **Back** closes the variation panel and returns to the Cart, Wedding Builder, or Customer Orders view without adding or changing an item. The same variation panel, ordering, and back/edit behavior is used for ordinary Cart additions, Wedding Builder parent items, Customer Orders item updates, POS Inventory, and Product Hub. Barcode scans still add the exact scanned variation directly.
