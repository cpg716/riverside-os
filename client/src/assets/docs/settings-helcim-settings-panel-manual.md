...
- Card Not Present and public web checkout use HelcimPay.js. Add the public HTTPS ROS/PWA checkout origin to the Helcim API Access Configuration before live keyed entry. The desktop app's local `tauri.localhost` origin is not valid for HelcimPay.js, so Register Card Not Present opens a one-time public HTTPS handoff and then records the approved attempt back to the sale. If the public HTTPS base URL is not configured, use Offline CC only for an approval completed outside ROS.
