...
- Card Not Present uses the same server-owned recovery reference behind the scenes. If Helcim approves while the handoff or Main Hub connection is interrupted, keep the checkout open and use Recover payment; ROS will verify the approved attempt before showing Add Payment to Sale.
