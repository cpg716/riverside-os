...
- Manual Card and public web checkout use HelcimPay.js. Add the public HTTPS ROS/PWA checkout origin to the Helcim API Access Configuration before live keyed entry. The desktop app's local `tauri.localhost` origin is not valid for HelcimPay.js, so Register Manual Card opens a one-time public HTTPS handoff page and then records the approved attempt back in the checkout drawer. If the public HTTPS base URL is not configured, use the Helcim terminal path on the register.
