...
The normal private release is built and signed on the Main Hub. No GitHub account is required for staff to install it. Windows workstations download the signed app update from the Main Hub address already saved during station setup.
...
2. Confirm the release card says **Private Main Hub release** and review the target version and build.
3. Confirm no Register is in an active payment or unsynced checkout.
4. Select **Update Server & Client App** only when the release is ready and the update window is approved.
...
The guarded update verifies the package, takes the required pre-migration backup, runs the normal installer and migrations, and checks the exact build before the private workstation feed changes. If installation or readiness fails, the new feed is not published and the previous feed remains current.

### Register and Back Office PC update

Update the Main Hub first. Then, on each Windows station:
...
Browser and PWA stations do not install the Windows package. After the Main Hub update, use **Check app files** and **Reload app**, or follow the Riverside reload prompt. Finish active work first when the prompt allows **Later**. A confirmed exact-build mismatch remains blocking.
...
- A Main Hub update failure is not permission to copy files manually or bypass migrations. Review the promotion/update transcript or contact support.
