...
Daily Sales payment totals use the same store-local effective business-date window as the Z-Report. A payment made on another date toward a sale booked today is shown on that payment's date only, so card, cash, and other tender totals reconcile to the register close. Daily Sales and Z-Reports show every supported tender row even when its activity is zero. Net Cash, net CC, and net Checks are the primary reconciliation totals. Terminal, Card Not Present, saved-card, manual-card, and all-card-refund rows are CC detail; Deposit Applied, Exchange Credit, store credit, gift card, Staff Account, donation, **RMS Charge · Standard**, and **RMS Charge · 90 Day** are informational activity and are not additive to those primary totals. **RMS Payments** remains a separate business-activity metric because the actual tender collected is already counted under Cash or Check.
...
The Activity List includes every staff-facing transaction event in the selected period, including manager-approved complimentary transactions whose subtotal, tax, and tender are all `$0.00`. Those records remain visible for audit while contributing `$0.00` to sales and tender totals.

A **Wedding Deposit Disbursement** is held-liability activity, not a merchandise sale. Its card, printout, and export show the payer Transaction reference, funded-member count, exact deposit tender collected, and wedding party without inventing line items, subtotal, tax, merchandise balance, or a second copy of the same tender. Member suits and other merchandise appear only on the separate member Transactions after **Create All Member Transactions** succeeds. Use the Wedding party link or payer receipt to review the allocation; ordinary sale Detail, Void, and Refund controls do not apply to this deposit container.

Historical Counterpoint reconciliation and administrative repair work remain in the Transaction Record audit trail and do not create a new sale on the repair date or appear as customer-facing order amendments in Interaction Timeline. Merchandise changed in ROS on an existing open Transaction is different, including a Transaction that originally came from Counterpoint: its line-booking event belongs to the store-local amendment date, not the original Transaction date. **Booked** Daily Sales and Z-Reports apply the signed net value of all additions, removals, and price/quantity changes on that Transaction for the selected day. A positive net amendment increases Booked Subtotal; a negative net amendment reduces it without increasing the sale count. Same-day additions and cancellations are grouped into one **Order Edited (Increase)**, **Order Edited (Decrease)**, or **Order Edited (No Net Change)** record. The card keeps the primary Salesperson separate from **Edited by**, formats every price to cents, and shows a paid amount only when a tender actually moved. Amendment activity remains visible even when the last item was removed, and line details label the signed **Added**, **Change**, and **Removed** amounts so the report never substitutes the Transaction's full lifetime value. A cancellation without a matching same-day addition remains a separate **Order Item Cancellation** row with the negative item, subtotal, and tax even though no refund tender was issued. Any other return credit without a successful refund tender is likewise shown once as **Return / Credit Adjustment**.
...
A completed return or exchange that produces a customer refund appears once as one **Return / Exchange** event, not as separate refund and zero-detail payment entries. A refund completed on the original sale's business date remains visible as that event so its negative subtotal and tax reconcile before Register close. If the refund tender is assigned to another effective date or Register session, the return adjustment remains on the merchandise-return date instead of disappearing from that day's detail. Its detail and reprint are scoped to that event: today's returned items appear as negative lines, today's new merchandise or services appear as positive lines, and retained original items and historical tenders are omitted. Mixed exchanges show the exact net **Refund to customer** or **Amount due** on the event receipt. When **Original Card** is used, the event remains pending until the provider responds; successful completion identifies Helcim, the approved amount, and the masked card.
...
- **Tender Breakdown**: The on-screen Daily Sales view, Daily print, and Z-Report use one payment-day breakdown for Cash, Card, Checks, Gift Card, Store Credit, Deposit Applied, Exchange Credit, RMS Charge Standard, RMS Charge 90 Day, Staff Account, and Donation. Every supported row remains visible at zero so staff do not have to guess whether a tender was omitted. RMS Payment collections remain in their own activity metric while their Cash or Check tender appears once in this breakdown.
- **Business Summary Boxes**: New Orders, Orders Picked Up, Credit Card Total, RMS Payments, **RMS Charge · Standard**, and **RMS Charge · 90 Day** appear in the top summary so daily review focuses on register operations. **New Orders** counts Special and Custom Transactions whose initial booking event is in the selected business-date range; it does not recount every older open Order or a later amendment. Credit Card Total includes CC/Card Reader, Card Manual, Card Not Present, saved-card, and card refund/credit activity; it does not include Staff Account or exchange credit.
...
- **Transaction Audit**: A complete list of all matching `TXN-` transaction numbers, assigned Salesperson, payment-only activity, and amounts. Payment rows without merchandise lines remain present in CSV exports. Daily Sales and Z-Report transaction pages use the same booked-activity cards and labels: type first, Customer name as the largest label, then the bare `TXN-` number, Salesperson, fulfillment context, and date/time. A Transaction with multiple tenders remains one card with the tender lines inside it; it is never split into multiple report entries.
...
- **Financial reconciliation guard**: Before Riverside saves a Z-close snapshot or produces a complete unfiltered Daily/Z report, booked Net Sales and tax must equal the sum of the supporting activity detail. If they do not reconcile exactly to the cent, report output is blocked for review instead of presenting the mismatch as valid.
