...
Historical Counterpoint reconciliation and administrative repair work remain in the Transaction Record audit trail and do not create a new sale on the repair date or appear as customer-facing order amendments in Interaction Timeline. Merchandise changed on an existing open Transaction is different: its line-booking event belongs to the store-local amendment date, not the original Transaction date. **Booked** Daily Sales and Z-Reports apply the signed net value of all additions, removals, and price/quantity changes on that Transaction for the selected day. A positive net amendment increases Booked Subtotal; a negative net amendment reduces it without increasing the sale count. Amendment activity remains visible even when the last item was removed, and line details label the signed **Added**, **Change**, and **Removed** amounts so the report never substitutes the Transaction's full lifetime value. A same-price product, variation, fulfillment, or line-kind switch remains visible as **Order Adjustment (No Net Change)** without changing Net Sales or tax. If an Order item cancellation reduces only the unpaid balance, Daily Sales shows a separate **Order Item Cancellation** row with the negative item, subtotal, and tax even though no refund tender was issued. Any other return credit without a successful refund tender is likewise shown once as **Return / Credit Adjustment**.
