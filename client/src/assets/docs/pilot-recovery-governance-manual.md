...
| **wedding pickup**, **paid pickup incomplete**, or fulfillment question | [Orders Workspace](manual:orders-workspace) | Confirm transaction, balance, line readiness, and fulfillment status before marking pickup complete. If payment posted first, finish pickup from the source order when practical; otherwise assign the follow-up and verify it remains visible in the operational recovery workspace without re-ringing payment. |
