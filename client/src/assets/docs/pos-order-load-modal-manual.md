...
4. Find the open line and choose **Update Item**. The standard item option panel opens above Customer Orders so every choice remains visible and clickable. **Item to Build** stays at the top with the product and all choices already made. It shows the current SKU and variation, keeps any shared model or style choices selected, and then guides you through every remaining size, style, or other available option. Use **Back** to return one choice at a time, or select any completed option under **Item to Build** to edit from that point. Back and direct Edit remain available on the final pricing review before **Update Item**. At the first step, **Back** closes the panel without changing the order line. A product with only one option, such as shoe size, goes directly to that choice. **Standard** identifies a variation with no additional option, and an explicit **SKU** step appears if two variants otherwise have the same option description. Confirm **Update Item** only after the complete replacement variation and SKU are shown. The original customer price is preserved when only the item variation changes.
...
- One Register cart may apply payments to several open orders, pick up from several open orders, or combine those actions. Review the order chips under **Order work in this cart** before continuing. Starting pickup from another order adds its selected lines to the active Register cart and keeps the lines already staged from earlier orders. Reopening an order replaces only that order's staged pickup selection. An explicitly entered partial payment is preserved when pickup work is added; Riverside does not silently replace it with the full balance.
...
- Riverside repeats that same financial check before pickup or shipping. A Transaction whose header, charged lines, taxes, payments, and balance do not reconcile cannot release merchandise, even with Manager Access. A Manager payment override is only for an intentional payment shortage on an otherwise consistent Transaction; it never overrides inconsistent financial records.
