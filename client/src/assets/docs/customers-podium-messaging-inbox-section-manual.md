...
The screen refreshes the Riverside copy every minute while it is open. That background refresh keeps the open conversation on screen instead of clearing and reloading the message thread. Podium webhooks are still the fastest path for new inbound messages, calls, reviews, and review responses; Riverside stores verified webhook events in a retryable queue before acknowledging them. Inbound calls mark the conversation unread. A missed call, voicemail, or linked review that Podium marks as needing a response also marks it **Needs reply**. When a call is the newest activity, the list identifies the row as **Call** and the thread shows a call card. **Refresh** reloads only the Riverside copy. Open **Status** for provider diagnostics, accepted-delivery processing, stored-call count, and the last call Riverside received. **ROS webhook receiving** confirms Riverside's local signing and ingest setup; **Processing current** confirms accepted deliveries are not stuck or failed. Use **Pull from Podium** when message history is missing. The pull uses Podium's cursor-paged conversation and message APIs; call and published-review cards come from signed webhooks, not that history pull. Riverside also refreshes provider message history in the background when the last pull is more than 30 minutes old. A routine background refresh is not an error. **Pulling history** is a neutral progress state and does not show the needs-attention warning unless another Podium health problem is present. **History current** describes message history only. **Historical gap recorded** shows the exact number of stored conversation histories that are not marked complete, but it does not raise the general needs-attention banner while live webhook delivery and processing are healthy. Retry **Pull from Podium** once when older messages are actually missing; a returned pull failure remains an actionable warning for IT.
...
- The Inbox uses blue for the selected conversation, primary actions, and Riverside's outbound message bubbles.
...
- **Replying as** is conversation-specific and can differ from the staff member currently using the workstation. Only active staff with a linked Podium user appear as choices. Changing it requires the selected staff member's Access PIN; sending later replies does not.
...
- **ROS webhook receiving** means Riverside has its local signing secret and inbound ingest enabled. **Processing current** is the separate proof that accepted deliveries are being applied; a failed count and retained reason require IT attention. The provider subscription can still require an update; Settings → Connected Services → Podium is authoritative for that status. If **Stored calls** is zero while Podium has call activity, update that provider webhook. Message-history pulls cannot recover call events.
