...
- Riverside sends the approved tag batch to the configured Zebra station, using EPL for classic LP/TLP 2844 printers and ZPL II for newer Zebra printers. Desktop stations report the printer error if direct printing fails; browser/PWA sessions can open print preview as a fallback.
