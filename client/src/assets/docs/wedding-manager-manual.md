...
At the Register, **Wedding Deposit** is also a guided setup path. Choose **Deposit Only** or **Collect & Build Orders** first, then choose or start the party. Staff may start a party with Party Name and Wedding Date, link or create each Customer, assign roles, verify the payer, and choose an exact destination for every member amount. A selected `$0` member is visibly excluded instead of silently blocking the valid funded members. In **Collect & Build Orders**, **Continue to Payment & Build Orders** stages the reviewed deposits and opens Payment in one action. After the approved payer receipt, **Continue Wedding Orders** opens the first unposted funded member immediately; after each successful member Transaction and receipt it advances to the next unposted member. **Deposit Only** stops after funding. **Orders & Receipts** on the original payer remains the resume and receipt dashboard. Removing or changing the selected Customer clears stale Wedding member and unposted payer-allocation context. A declined payer tender posts no Transaction, allocation, held deposit, or booked sale; the reviewed Cart rows remain staged for retry.
