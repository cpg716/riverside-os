...
At the Register, **Wedding Deposit** is also a guided setup path. Choose **Deposit Only** or **Collect & Build Orders** first, then choose or start the party. Staff may start a party with Party Name and Wedding Date, link or create each Customer, assign roles, verify the payer, and choose an exact destination for every member amount. A selected `$0` member is visibly excluded instead of silently blocking valid funded members. In **Collect & Build Orders**, **Start Building Member Orders** preserves any merchandise already in the payer's Cart, including employee-price merchandise, then selects each funded member before payer Payment; staff add merchandise, confirm the salesperson, and choose **Save Member Order & Next**. Confirm the payer-merchandise salesperson before opening Wedding Deposit and confirm the deposit salesperson in Review; Riverside preserves them separately. Employee-price lines remain noncommissionable. These member selections are nonfinancial drafts. After all drafts are ready, **Collect & Build · Final Review** restores the payer's merchandise and enables **Final Payment** once. The payer receipt separates that merchandise from deposits placed for party members. A decline posts nothing and retains the reviewed drafts. After approval, **Continue Wedding Orders** loads each reviewed draft and automatically attaches its exact held source for separate member-owned Transaction completion without taking a second payer tender. **Deposit Only** stops after funding, and **Orders & Receipts** remains the resume and receipt dashboard. Removing or changing the selected Customer clears stale Wedding member and unposted payer-allocation context.
