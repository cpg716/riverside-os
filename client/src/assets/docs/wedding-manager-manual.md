...
At the Register, **Wedding Deposit** is also a guided setup path. Choose **Deposit Only** or **Collect & Build Orders** first, then choose or start the party. Staff may start a party with Party Name and Wedding Date, link or create each Customer, assign roles, verify the payer, and choose an exact destination for every member amount. A selected `$0` member is visibly excluded instead of silently blocking the valid funded members. **Collect & Build Orders** continues after the successful payer receipt; **Deposit Only** stops after funding. **Orders & Receipts** on the original payer shows each member's held/applied deposit and order status. Choose **Choose Member & Add Items** to select that member, then use the Register's Wedding Checklist or product search, mark deferred lines **Order (Wedding)**, confirm the salesperson, and complete the member Transaction. A declined payer tender posts no Transaction, allocation, held deposit, or booked sale; the reviewed Cart rows remain staged for retry.
