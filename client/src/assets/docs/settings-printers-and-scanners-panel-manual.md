...
4. For the Zebra clothing tag station, choose the installed Zebra printer or enter a non-loopback tag printer IP, then choose the exact **Printer language**: **EPL** for LP/TLP 2844-class printers or **ZPL II** for newer Zebra models.
...
6. Use **Print test tag** on the Clothing Tag Station card, or open **Tag Designer → Print test tag**, to send an actual test label using the current saved tag layout. The success message appears only after ROS dispatches to the selected target and names whether ROS used EPL or ZPL.
