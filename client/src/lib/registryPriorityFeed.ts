...
export interface RegistryFloorStaffRow {
  id: string;
  full_name: string;
  role: string;
  avatar_key: string;
...
export interface RegistryPriorityFeedBundle {
  stats: {
  ...
  needs_measure: RegistryActionRow[];
  needs_order: RegistryActionRow[];
  rush_orders: RushOrderRow[];
  overdue_pickups: RegistryActionRow[];
  today_floor_staff: RegistryFloorStaffRow[];
}

/** Wedding registry-priority-feed row (matches `GET /api/weddings/priority-feed` lists). */
export interface RegistryActionRow {
...
export type RegistryQueueBand = "overdue" | "needs_order" | "needs_measure";

export type RegistryPriorityFeedItem =
  | {
      kind: "wedding";
      id: string;
      sortKey: number;
      tier: "urgent" | "soon" | "normal";
      band: RegistryQueueBand;
      row: RegistryActionRow;
    }
  | {
      kind: "task";
      id: string;
...
function taskDueSortKey(due: string | null, now = new Date()): { sortKey: number; tier: RegistryPriorityFeedItem["tier"] } {
  if (!due || !due.trim()) {
...
function notificationSortKey(row: NotificationRow): { sortKey: number; tier: RegistryPriorityFeedItem["tier"] } {
...
function rushOrderSortKey(row: RushOrderRow, now = new Date()): { sortKey: number; tier: RegistryPriorityFeedItem["tier"] } {
  let base = 150;
  let tier: RegistryPriorityFeedItem["tier"] = "soon";
 ...
 */
export function buildRegistryPriorityFeed(input: {
  overduePickups: RegistryActionRow[];
  needsMeasure: RegistryActionRow[];
  needsOrder: RegistryActionRow[];
  ...
  limit: number;
}): RegistryPriorityFeedItem[] {
  const items: RegistryPriorityFeedItem[] = [];
  const now = new Date();
...
export function registryBandLabel(band: RegistryQueueBand): string {
  switch (band) {
    case "overdue":
