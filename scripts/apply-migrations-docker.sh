...
LEDGER_EXISTS="$(docker compose exec -T db psql -U postgres -d "$RIVERSIDE_DB_NAME" -tAc "SELECT EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'ros_schema_migrations');" | tr -d '[:space:]')"

if [ "$LEDGER_EXISTS" != "t" ]; then
  echo "Bootstrapping migration ledger (00_ros_migration_ledger.sql) on $RIVERSIDE_DB_NAME"
  docker compose exec -T db psql -U postgres -d "$RIVERSIDE_DB_NAME" -v ON_ERROR_STOP=1 < "$ROOT/migrations/00_ros_migration_ledger.sql"
  docker compose exec -T db psql -U postgres -d "$RIVERSIDE_DB_NAME" -v ON_ERROR_STOP=1 -c "INSERT INTO ros_schema_migrations (version) VALUES ('00_ros_migration_ledger.sql') ON CONFLICT (version) DO NOTHING;"
fi
...
for f in $(ls "$ROOT"/migrations/[0-9][0-9]*_*.sql 2>/dev/null | sort -V); do
  ...
  docker compose exec -T db psql -U postgres -d "$RIVERSIDE_DB_NAME" -v ON_ERROR_STOP=1 -c "INSERT INTO ros_schema_migrations (version) VALUES ('$base') ON CONFLICT (version) DO NOTHING;"
done
