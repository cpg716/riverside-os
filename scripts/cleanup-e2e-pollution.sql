...
DELETE FROM ops_action_audit WHERE reason ILIKE '%E2E%' OR action_key ILIKE '%E2E%' OR result_message ILIKE '%E2E%';
DELETE FROM task_instance WHERE title_snapshot ILIKE '%E2E%' OR title_snapshot ILIKE '%Test%';
DELETE FROM shipment WHERE internal_notes ILIKE '%E2E%' OR internal_notes ILIKE '%Test%' OR ship_to->>'name' ILIKE '%E2E%' OR ship_to->>'name' ILIKE '%Test%';
DELETE FROM nuorder_sync_logs WHERE sync_type ILIKE '%E2E%' OR sync_type ILIKE '%Test%' OR error_message ILIKE '%E2E%' OR error_message ILIKE '%Test%';
